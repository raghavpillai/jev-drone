...
def summarize(result):
    ...
    return {"id": result["id"], "case": result["world"]["config"]["case"],
            "rig": result["world"]["config"]["rig"], "memory": result["world"]["config"]["memory"],
            "status": result["status"], "stage": result["stage"],
            "strict_pass": complete and not result["violations"] and not result["contacts"] and
                           all(r["valid"] for r in result["reports"]),
            "sim_seconds": result["sim_seconds"], "wall_seconds": result["wall_seconds"],
            "calls": len(calls), "controls": len(controls), "accepted_controls": len(accepted),
            "median_latency_ms": float(np.median(latency)*1000) if latency else None,
            "p95_latency_ms": float(np.quantile(latency, .95)*1000) if latency else None,
            "local_hz": len(controls)/sum(max(.2, x) for x in latency) if latency else 0,
            "local_cycle_seconds": sum(max(.2, x) for x in latency),
            "wall_mission_hz": len(accepted)/result["wall_seconds"],
            "cost_usd": result["cost_usd"], "violations": len(result["violations"]),
            "violation_types": dict(Counter(x for v in result["violations"] for x in v["reasons"])),
            "contacts": len(result["contacts"]), "near_miss_seconds": result["near_miss_seconds"],
            "min_clearance": min(.5, result["min_clearance"]),
            "clearance_censored_at_0_5m": result["min_clearance"] >= .5,
...
def main():
    ...
    summary = {"episodes": len(rows), "statuses": dict(Counter(r["status"] for r in rows)),
               "strict_passes": sum(r["strict_pass"] for r in rows),
               "calls": sum(r["calls"] for r in rows), "cost_usd": sum(r["cost_usd"] for r in rows),
               "infrastructure_failures": failures, "rows": rows}
    ...
    for row in rows:
        print(f'{row["id"]}: {row["status"]}, stage={row["stage"]}, violations={row["violations"]}, {row["sim_seconds"]:.1f}s, ${row["cost_usd"]:.4f}')
