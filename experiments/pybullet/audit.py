...
def audit_episode(result):
    ...
    for call in result["calls"]:
        ...
        if sensor.get("predicted_risk_by_direction", {}).get(direction) or (
            sensor.get("current_course_risk") and np.linalg.norm(velocity) > 0.15
        ):
    ...
    for report in result["reports"]:
        ...
        valid = np.linalg.norm(sensor["velocity_estimate"]) <= 0.159
        if report["choice"] == "dock":
            valid &= (
                "dock" in objective and np.linalg.norm(actual - result["world"]["start"]) <= 0.55
            )
            valid &= np.linalg.norm(pose - result["world"]["start"]) <= 0.45
        else:
            ...
            if marker is None:
                ...
                if (
                    result["world"]["config"]["case"] == "moved_target"
                    and marker == "red"
                    and report["time"] >= 18
                ):
                ...
                valid &= any(
                    d["marker"] == marker
                    and np.linalg.norm(pose - d["position"]) <= 1.5
                    and report["time"] - d["time"] <= 0.65
    ...
    if result["status"] == "success" and (
        len(report_checks) != len(mission) or not all(report_checks) or result["contacts"]
    ):
    ...
    return {
        ...
        "audited_violations": len(violations),
        "moving_risk_ignored_count": len(motion_risk_ignored),
    }
