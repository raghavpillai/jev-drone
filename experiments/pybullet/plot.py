...
def main():
    ...
    for box in result["world"]["geometry"]:
        if box["name"] in (
            ...
            continue
        opacity = 0.10 if box["name"] == "wall" else 0.28
        ax.add_collection3d(
            Poly3DCollection(
                faces(box["center"], np.array(box["half"])),
                facecolors=[box["color"][:3]],
                edgecolors="#8495a3",
                linewidths=0.3,
                alpha=opacity,
            )
    ...
    speed.grid(alpha=0.15)
    for report in result["reports"]:
        height.axvline(report["time"], color="#80ef9a", alpha=0.6, linestyle=":")
        height.text(
            report["time"], upper[2] - 0.15, report["choice"], rotation=90, va="top", fontsize=8
        )
    figure.suptitle(result["id"] + " — " + result["status"], fontsize=18)
    figure.text(
        0.05,
        0.025,
        "Evaluator geometry shown for inspection only. Jev received rendered-sensor measurements; every flight movement was chosen by Jev.",
        fontsize=10,
        color="#a7b8c9",
    )
    figure.subplots_adjust(left=0.02, right=0.96, bottom=0.12, top=0.88, wspace=0.3, hspace=0.32)
