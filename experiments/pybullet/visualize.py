...
def replay(path, out):
    ...
    for time in np.linspace(0, result["sim_seconds"], min(100, max(2, int(result["sim_seconds"]*1.4)))):
        ...
        draw.text((28, 52), "Evaluator map · hidden geometry is NOT sent to Jev", font=font(14), fill="#a7b8c9")
        ...
        with np.load(snapshots[snapshot_index]) as data:
            ...
            rgb = data["rgb"][camera_index] if camera_index is not None else np.zeros_like(data["rgb"][0])
            depth = data["depth"][camera_index] if camera_index is not None else np.full_like(data["depth"][0], np.nan)
        canvas.paste(Image.fromarray(rgb).resize((384, 288), Image.Resampling.NEAREST), (686, 102))
        caption = "Front camera only · sampled every 2s" if result["world"]["config"].get("front_controls") else "Rendered RGB · sampled every 2s"
        draw.text((686, 76), caption, font=font(15), fill="#a7b8c9")
        finite = np.isfinite(depth)
        shade = np.nan_to_num(np.clip(depth/8., 0, 1))
        depth_rgb = np.stack([shade*120, (1-shade)*180, (1-shade)*230], axis=-1).astype(np.uint8)
        depth_rgb[~finite] = (225, 60, 120)
        canvas.paste(Image.fromarray(depth_rgb).resize((192, 144), Image.Resampling.NEAREST), (686, 420))
        ...
        if result["world"]["config"].get("control_schema") == "joystick-v1":
            ...
            published = [s for s in result.get("setpoints", []) if s["sim"]-epoch <= time and s.get("body_controls")]
            values = published[-1]["body_controls"] if published else {}
            draw.text((888, 483), "Held controls", font=font(15), fill="white")
            for row, (key, label, unit) in enumerate((("forward_mps", "Forward", "m/s"),
                    ("right_mps", "Right", "m/s"), ("up_mps", "Up", "m/s"), ("yaw_rate_rps", "Pan left", "rad/s"))):
                draw.text((888, 508+row*20), f"{label}: {values.get(key, 0.):+.2f} {unit}",
                          font=font(13), fill="#55d9e8")
        calls = [c for c in result["calls"] if c.get("response_time", c["time"]) <= time]
        latest = calls[-1] if calls else {}
        draw.text((30, 535), f't={time:.1f}s   altitude={point["position"][2]:.2f}m   speed={np.linalg.norm(point["velocity"]):.2f}m/s', font=font(18), fill="white")
        draw.text((30, 571), f'Jev: {latest.get("choice", "waiting")}  ({latest.get("role", "sensors")})', font=font(18), fill="#55d9e8")
        draw.text((30, 606), f'Final: {result["status"]} · contacts: {len(result["contacts"])} · rule violations: {len(result.get("violations", []))} · Jev / OpenRouter navigation', font=font(14), fill="#a7b8c9")
