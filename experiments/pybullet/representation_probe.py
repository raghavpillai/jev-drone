...
def action_facts(state):
    ...
    moving = np.linalg.norm(velocity) > 0.15
    facts = {}
    for name, direction in DIRECTIONS.items():
        clearance = sensor["clearance_m"][name]
        axis_ok = not moving or np.dot(velocity / np.linalg.norm(velocity), direction) >= 0.95
        facts[name] = {
            "clearance": clearance,
            "depth_label": "UNKNOWN"
            ...
            "slow_clearance_sufficient": bool(clearance is not None and clearance >= 0.4),
        }
    return facts
...
def modify(state, method):
    ...
    if method == "labels":
        state["sensors"]["clearance_m"] = {
            k: {"meters": v["clearance"], "status": v["depth_label"]} for k, v in facts.items()
        }
    elif method == "action_facts":
        state["movement_checks"] = facts
    elif method == "compact":
        state = {
            "goal_offset": state["goal_offset"],
            "at_position": state["at_position"],
            "motion_direction": state.get("motion_direction"),
            "motion_axis_needs_travel": state.get("motion_axis_needs_travel"),
            "stopped": state.get("stopped"),
            "movement_checks": facts,
...
def valid(choice, original):
    ...
    return (
        facts["axis_change_ready"]
        and facts[
            "cruise_clearance_sufficient" if speed == "cruise" else "slow_clearance_sufficient"
        ]
    )
...
def main():
    ...
    for path in args.episodes:
        ...
        controls = [
            ...
            if c["role"] == "control"
            and c.get("accepted")
            and not valid(c.get("choice"), c["state"])
        ]
    ...
    (args.out / "protocol.json").write_text(
        json.dumps(
            {
                "input_episodes": [str(p) for p in args.episodes],
                "methods": ["raw", "labels", "action_facts", "compact"],
                "count": len(jobs),
            },
            indent=2,
        )
    ...
    def run(job):
        ...
        try:
            result = gateway.choose(
                modify(call["state"], method),
                call["questions"]["selection"]["instructions"],
                call["questions"]["selection"]["criteria"],
            )
        finally:
            gateway.close()
        row = {
            ...
            "index": index,
            "method": method,
            "valid": valid(result.get("choice"), call["state"]),
            "original_choice": call.get("choice"),
            **result,
        }
    ...
    summary = {
        method: {
            "valid": sum(r["valid"] for r in rows if r["method"] == method),
