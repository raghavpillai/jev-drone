...
def main():
    ...
    for path in sorted(Path("results/px4-autonomy-modes-validation").glob("*/result.json")):
        ...
        for call in result["calls"]:
            state = call["state"]
            if (call["role"] == "control" and call.get("accepted")
                    and call["choice"] in ("brake", "keep_controls") and not state["at_position"]
                    and state["sensors"]["speed"] < .07
                    and abs(state["sensors"]["yaw_rate_rps"]) < .07
    ...
    result = json.loads(Path("results/px4-recovery-depth-v15/controller-4910/result.json").read_text())
    for name in ("forward_slow", "backward_slow", "strafe_left_slow", "strafe_right_slow", "pan_left", "pan_right", "up", "down"):
        call = next((c for c in result["calls"] if c["role"] == "control" and c.get("accepted") and c["choice"] == name), None)
        if call:
    ...
    try:
        for trial, kind, call in cases:
            ...
            for variant in ("baseline", "compact"):
                ...
                row = {"trial": trial, "kind": kind, "time": call["time"], "variant": variant,
                    "choice": result.get("choice"), "violations": effect.get("movement_violations"),
