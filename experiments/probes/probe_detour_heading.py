...
def candidate(state, criteria):
    ...
    if (state.get("active_detour") and state.get("required_heading_degrees") is None
            and state["horizontal_distance"] <= .35):
...
def main():
    ...
    selected = []
    for name in ("px4-recovery-validation-v23/occluded-4932", "px4-recovery-evidence-v20/occluded-4872"):
        ...
        for index, (call, raw) in enumerate(zip(result["calls"], journal)):
            state = raw["state"]
            if (call["role"] == "control" and not raw.get("error") and state.get("active_detour")
                    and state["task"].get("look_position") and state.get("required_heading_degrees") is None
                    and state["horizontal_distance"] <= .25 and abs(state["body_goal_offset_m"]["up"]) > .5
    ...
    variant_name = "defer_heading_and_altitude_facts" if args.altitude_facts else "defer_parent_heading"
    (args.out/"protocol.json").write_text(json.dumps({
        "states": [{"trial": n, "call_index": i, "sim_time": t, "vertical_direction": d}
                   for n,i,t,d,_ in selected],
        "repeats": 2, "variants": ["baseline", variant_name],
        ...
        for name, index, time, direction, raw in selected:
            for repeat in range(2):
                ...
                for variant in variants:
                    ...
                    row = dict(trial=name, call_index=index, sim_time=time, repeat=repeat, variant=variant,
                               choice=choice, vertical_progress=choice in (direction, direction+"_creep"),
                               violations=state["control_effects"][choice]["movement_violations"] if choice else None,
                               error=response.get("error"), latency_seconds=response["latency_seconds"])
                    rows.append(row)
