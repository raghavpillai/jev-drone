...
from jev_drone.planning.detour_route import observed_obstacles, INSTRUCTIONS
...
def main():
    ...
    for call in result["calls"]:
        ...
        facts = {name: observed_obstacles(effect["planned_route"], call["state"]["sensors"]["position_estimate"], [frame])
                 for name, effect in call["state"]["control_effects"].items() if "planned_route" in effect}
                ...
                for variant in ("baseline", "route_evidence"):
                    ...
                    if variant == "route_evidence":
                        ...
                        for name, legs in facts.items():
                            ...
                            criteria[name] += " Observed route obstacles: "+json.dumps(legs)+". Prefer a route without observed blocked legs; unseen space is not guaranteed clear."
                    response = gateway.choose(state, instructions, criteria)
                    choice = response.get("choice")
                    row = dict(time=call["time"], frame=str(snapshot), repeat=repeat, variant=variant,
                        choice=choice, selected_observed_blocked_route=any(leg["observed_blocked"] for leg in facts.get(choice, [])),
                        violations=state["control_effects"].get(choice, {}).get("movement_violations"), error=response.get("error"))
                    rows.append(row)
