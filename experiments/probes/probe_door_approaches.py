...
def near_options(state, cloud):
    ...
    for door, center in HOUSE.doors.items():
        ...
        position[HOUSE.door_axis(door)] -= 0.65 * HOUSE.door_direction(door, destination)
        gap = round(float(np.linalg.norm(cloud - position, axis=1).min()) - 0.4, 3)
        delta = position - state["sensors"]["position_estimate"]
        options["approach_" + door + "_near"] = {
            "position": position.tolist(),
            "door": door,
            "leads_to_room": destination,
...
def main():
    ...
    for cohort, trial in [
        ...
        for index, (call, raw) in enumerate(zip(result["calls"], journal)):
            ...
            bad = [
                ...
                and name[9:] in HOUSE.doors
                and option.get("observed_endpoint_unsafe")
            ]
            if not bad or (state["sensors"]["room"], state["stage"]) in selected:
                continue
            frame = max(
                (f for f in frames if float(f.stem) <= call["time"]), key=lambda f: float(f.stem)
            )
            ...
            useful = [
                name + "_near"
                for name in bad
                if not alternatives[name + "_near"]["observed_endpoint_unsafe"]
            ]
            if not useful:
                continue
            base = deepcopy(raw)
            ...
            for name, option in base["state"]["options"].items():
                if "position" not in option:
                    continue
                gap = round(
                    float(np.linalg.norm(cloud - option["position"], axis=1).min()) - 0.4, 3
                )
                option.update(observed_goal_gap_m=gap, observed_endpoint_unsafe=gap < 0.08)
                if name in base["questions"]["selection"]["criteria"]:
                    base["questions"]["selection"]["criteria"][name] = (
                        "DO NOT SELECT this observed unsafe endpoint. " if gap < 0.08 else ""
                    ) + json.dumps(option)
            base["state"]["offline_endpoint_depth_frame"] = {
                "path": str(frame),
            ...
            issue_remains = any(
                base["state"]["options"][name.removesuffix("_near")]["observed_endpoint_unsafe"]
                for name in useful
            )
            cases.append(
                {
                    ...
                    "index": index,
                    "probe_endpoint_status": "unsafe_base_with_clear_alternative"
                    if issue_remains
                    else "boundary_control_base_now_clear",
                    "useful_alternatives": useful,
    ...
    (args.out / "protocol.json").write_text(
        json.dumps(
            {
                "variants": ["single_approach", "alternate_approaches"],
                "repeats": 2,
                "cases": [
                ...
                "Positive gap is not route certification. No claims of physical completion.",
            },
            indent=2,
        )
        + "\n"
    ...
    try:
        for case in cases:
            for repeat in range(2):
                for variant in (
                    ["single_approach", "alternate_approaches"]
                    if repeat == 0
                    else ["alternate_approaches", "single_approach"]
                ):
                    ...
                    if variant == "alternate_approaches":
                        ...
                        for name, option in case["alternatives"].items():
                            question["criteria"][name] = (
                                "DO NOT SELECT this observed unsafe endpoint. "
