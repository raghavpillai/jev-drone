...
from jev_drone.control.doorway_phase import describe, INSTRUCTIONS
...
def main():
    ...
    for name in ('house_far-5101','house_reverse-5102'):
        ...
        for kind in ('misaligned','aligned'):
            eligible=[]
            for c in calls:
                s=c['state'];a=s.get('doorway_alignment')
                if a is None or s.get('active_detour') or not c.get('answers'):continue
                bad=abs(a['altitude_error_m'])>.25 and abs(a['heading_error_degrees'])<=8
                good=abs(a['altitude_error_m'])<.1 and abs(a['centerline_offset_m'])<.15 and abs(a['heading_error_degrees'])<5
                if (kind=='misaligned' and bad) or (kind=='aligned' and good):eligible.append(c)
    ...
    try:
        for index,(name,kind,call) in enumerate(cases):
            for repeat in range(2):
                for variant in (('baseline','door_phase') if repeat==0 else ('door_phase','baseline')):
                    state=deepcopy(call['state']);q=deepcopy(call['questions']['selection'])
                    if variant=='door_phase':
                        state,q['criteria']=describe(state,q['criteria']);q['instructions']+=INSTRUCTIONS
                    r=gateway.choose(state,q['instructions'],q['criteria']);choice=r.get('choice')
                    effect=state['control_effects'].get(choice,{})
                    row=dict(index=index,trial=name,kind=kind,repeat=repeat,variant=variant,choice=choice,
                        phase=state.get('doorway_phase'),violations=effect.get('movement_violations'),
                        vertical_toward=effect.get('travel_effect',{}).get('up')=='toward_goal',error=r.get('error'))
