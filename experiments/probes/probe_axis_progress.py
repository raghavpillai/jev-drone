...
from jev_drone.control.axis_progress import INSTRUCTIONS, describe
...
def main():
    ...
    sources = [
        ...
        ("px4-house-v30-survey", "house_hidden-5111"),
    ...
    for cohort, trial in sources:
        ...
        for index, line in enumerate(path.read_text().splitlines()):
            ...
            fact = updated.get("near_goal_axis_progress")
            if (
                len(selected) < 2
                and task not in selected
                and fact
                and s["goal_path_status"] in ("BLOCKED", "UNKNOWN")
                and choice not in fact["permitted_controls"]
                and choice
                and choice.startswith(("detour_", "brake", "keep_"))
            ):
                cases.append(
                    {
                        "label": cohort + "-" + task,
            ...
            negative = (
                "at_position"
                if s["at_position"]
                else "occupied_endpoint"
                if s.get("goal_endpoint_observed_occupied")
                else None
            )
    ...
    protocol = {
        ...
        "variants": ["original", "facts", "facts_and_rule"],
        "repeats": 2,
        ...
        for case in cases:
            for repeat in range(2):
                for variant in protocol["variants"] if repeat == 0 else protocol["variants"][::-1]:
                    ...
                    candidate, _ = describe(s, q["criteria"])
                    allowed = candidate.get("near_goal_axis_progress", {}).get(
                        "permitted_controls", []
                    )
                    if variant != "original":
                    ...
                    if variant == "facts_and_rule":
                        assert OLD_RULE in q["instructions"]
                        q["instructions"] = q["instructions"].replace(OLD_RULE, NEW_RULE)
                    ...
                    row = {
                        ...
                        "repeat": repeat,
                        "choice": choice,
                        "permitted_axis_progress": choice in allowed,
                        "violations": s["control_effects"]
                        .get(choice, {})
