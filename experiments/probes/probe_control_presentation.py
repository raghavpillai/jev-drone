...
from experiments.representations.altitude_access import INSTRUCTIONS as ALTITUDE_INSTRUCTIONS
from experiments.representations.altitude_access import describe as altitude_access
from experiments.representations.control_presentation import INSTRUCTIONS, describe
...
def main():
    ...
    p.add_argument("--native-order", action="store_true")
    a = p.parse_args()
    a.out.mkdir(parents=True, exist_ok=False)
    cases = []
    sources = [
        ("v34-full", "house_hidden-5111"),
        ("v36-validation", "house_reverse-5312"),
        ("v37-validation", "house_hidden-5411"),
        ("v38-validation", "house_reverse-5512"),
    ]
    for cohort, trial in sources:
        ...
        for i, (c, logged) in enumerate(zip(calls, result["calls"])):
            if logged.get("time") not in violations or "control_effects" not in c["state"]:
                continue
            if selected >= 2:
                break
            cases.append(
                dict(
                    label=f"{cohort}-{selected}",
                    kind="historical_violation",
                    source=str(path / "calls.jsonl"),
                    index=i,
                    call=c,
                )
            )
    ...
    for i, line in enumerate(path.read_text().splitlines()):
        ...
        if a.violations_only or "control_effects" not in s or choice not in wanted:
            continue
        if s["control_effects"][choice]["movement_violations"]:
            continue
        cases.append(
            dict(label="nominal-" + choice, kind="nominal", source=str(path), index=i, call=c)
        )
    ...
    variants = (
        ["original", "altitude"]
        if a.native_order
        else ["original", "altitude", "altitude_summary"]
        if a.altitude
        else ["original", "criteria", "summary"]
    )
    repeats = 2
    ...
    protocol = {
        ...
        "variants": variants,
        "repeats": repeats,
        "native_instruction_order": a.native_order,
            ...
            for repeat in range(repeats):
                for variant in variants if repeat == 0 else variants[::-1]:
                    ...
                    if variant.startswith("altitude"):
                        ...
                        if a.native_order:
                            ...
                            q["instructions"] = q["instructions"].replace(
                                anchor, ALTITUDE_INSTRUCTIONS + anchor, 1
                            )
                        else:
                            q["instructions"] += ALTITUDE_INSTRUCTIONS
                    result = gateway.choose(s, q["instructions"], q["criteria"])
                    ...
                    row = dict(
                        ...
                        travel=effect.get("travel_effect"),
                        turn=effect.get("turn_effect"),
                        controls=effect.get("resulting_controls"),
