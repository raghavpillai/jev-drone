...
from jev_drone.planning.search_memory import SearchMemory, INSTRUCTIONS
...
def main():
    ...
    for name in list(memory.viewpoints):
        if name.endswith('_2.2'):
            view=memory.viewpoints.pop(name);view['position'][2]=2.5
            memory.viewpoints[name[:-3]+'2.5']=view
    cases=[]
    controls=set()
    for log,call in zip(result['calls'],raw):
        s=call['state'];memory.observe(s['sensors'],log['time'])
        role=log['role'];kind=s.get('selected_task_kind')
        ...
        if role=='planner_mode' and log['time']<640:
            label=('outside' if s['sensors']['room']=='library' else
                   'known_target' if s.get('target_visible') and not s.get('inspection_ready') else
                   'ready' if s.get('inspection_ready') else None)
            if label and label not in controls:
                controls.add(label);cases.append((label,call,deepcopy(memory)))
    protocol={'source':str(path),'variants':['baseline','instructions','survey_facts'],
        'repeats':2,'cases':[c[0] for c in cases],
    ...
    gateway=JevGateway('openrouter',load_credential('openrouter',Path('.env')),journal=a.out/'calls.jsonl')
    rows=[]
    try:
        for label,call,mem in cases:
            for repeat in range(2):
                variants=protocol['variants'] if repeat==0 else list(reversed(protocol['variants']))
                    ...
                    if variant=='survey_facts':
                        ...
                        for name,option in state.get('options',{}).items():
                            if name.startswith('view_'):
                                ...
                                option['look_position']=[(lo[0]+hi[0])/2,(lo[1]+hi[1])/2,option['position'][2]]
                                option['purpose']='Reach this position THEN look into the room.'
                    response=gateway.choose(state,q['instructions'],q['criteria'])
                    choice=response.get('choice');option=state.get('options',{}).get(choice,{})
                    row=dict(case=label,repeat=repeat,variant=variant,choice=choice,
                        selected_position=option.get('position'),error=response.get('error'))
