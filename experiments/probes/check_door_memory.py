...
from jev_drone.planning.door_memory import INSTRUCTIONS, DoorMemory
...
def recorded_cloud(path, hfov):
    with np.load(path) as data:
        ...
        if "right" in data:
            ...
            if np.ptp(data["times"]) > 1e-8 or set(names) != {
                "east",
                "west",
                "north",
                "south",
        ...
        for index, name in enumerate(names):
            frame = Frame(
                ...
                right[index],
                up[index],
                data["depth"].shape[2] / (2 * math.tan(hfov / 2)),
                name,
            )
            cloud = points(frame)
            ...
            for repeat in range(3):
                for variant in ("baseline", "observed_door_surface"):
                    ...
                    row = dict(
                        ...
                        repeat=repeat,
                        variant=variant,
                        observed_obstructions=memory.obstructions,
