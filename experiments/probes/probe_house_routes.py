...
def main():
    ...
    try:
        for destination in ("workshop", "entry"):
            for room, (lo, hi) in HOUSE.rooms.items():
                ...
                for repeat in range(2):
                    ...
                    experiment.layout, experiment.config, experiment.gateway = (
                        HOUSE,
                        Config(policy="v4"),
                        gateway,
                    )
                    objective = {"marker": "red", "room": destination}
                    experiment.world = SimpleNamespace(time=0.0, realtime=True, mission=[objective])
                    experiment.sensors = SimpleNamespace(current=[], observed_gap_at=lambda p: None)
                    experiment.stage, experiment.calls, experiment.memory, experiment.tasks = (
                        0,
                        [],
                        {},
                        [],
                    )
                    experiment.cost, experiment.error_streak = 0.0, 0
                    experiment.visits = Counter()
                    experiment.front_views = []
                    experiment.search_memory, experiment.return_memory, experiment.door_memory = (
                        SearchMemory(HOUSE),
                        ReturnMemory(),
                        DoorMemory(HOUSE),
                    )
                    pose = ((np.array(lo) + hi) / 2).tolist()
                    pose[2] = 1.3
                    observation = dict(
                        room=room,
                        position_estimate=pose,
                        detections=[],
                        speed=0.0,
                        yaw_rate_rps=0.0,
                    )
                    ...
                    state = dict(
                        ...
                        time_remaining=900,
                        sensors=observation,
                        known_room_bounds=HOUSE.rooms,
                        known_doors=HOUSE.doors,
                    ...
                    choice, log = experiment.request(
                        "planner", state, PLANNER, {k: json.dumps(v) for k, v in options.items()}
                    )
                    ...
                    row = dict(
                        ...
                        choice=choice,
                        next_room=next_room,
                        reduces_hops=next_room is not None
                        and distance(next_room, destination) < distance(room, destination),
                        error=log.get("error"),
        ...
        (args.out / "protocol.json").write_text(
            json.dumps(
                {
                    "kind": "synthetic room reasoning, not a flight",
                    "states": 14,
                    "repetitions": 2,
                    "expected_requests": 56,
                    "score": "Does the selected next room reduce unblocked architectural graph distance? Distances never enter model input.",
                },
                indent=2,
            )
        )
