...
from jev_drone.planning.return_intent import describe, INSTRUCTIONS
...
def main():
    ...
    parser.add_argument('--scope',action='store_true',help='Exclude object search/approach categories from dock objectives.')
    args=parser.parse_args();args.out.mkdir(parents=True,exist_ok=False);cases=[]
    for cohort,trial,times in SOURCES:
        ...
        modes=[i for i,c in enumerate(result['calls']) if c['role']=='planner_mode' and c['state'].get('stage')==1]
        for time in times:
            index=min(modes,key=lambda i:abs(result['calls'][i]['time']-time))
            assert result['calls'][index+1]['role']=='planner'
            cases.append((cohort+'-'+str(round(result['calls'][index]['time'])),journal[index],journal[index+1]))
    protocol={'sources':SOURCES,'cases':[label for label,_,_ in cases], 'repeats':2,
        'variants':['original','room_return'], 'exclude_marker_tasks_during_dock':args.scope,
        ...
        for label,mode_call,task_call in cases:
            for repeat in range(2):
                for variant in (protocol['variants'] if repeat==0 else protocol['variants'][::-1]):
                    state=deepcopy(mode_call['state']);modeq=deepcopy(mode_call['questions']['selection'])
                    ...
                    criteria={name:taskq['criteria'].get(name,json.dumps(option)) for name,option in state['options'].items()}
                    if variant=='room_return':
                        state,criteria=describe(state,criteria)
                        modeq['instructions']=modeq['instructions'].split('\nFor return_to_launch,')[0]+INSTRUCTIONS
                        modeq['criteria']['retrace']='Returning and local approach is blocked: consider a nearby observed outbound position as a local bypass; do not replay the entire search history.'
                        if args.scope:
                            modeq['criteria']={name:text for name,text in modeq['criteria'].items() if name not in ('search','approach')}
                        ...
                        if old in taskq['instructions']:
                            start=taskq['instructions'].index(old);finish=taskq['instructions'].index(end,start)+len(end)
                            taskq['instructions']=taskq['instructions'][:start]+taskq['instructions'][finish:]
                        taskq['instructions']+=INSTRUCTIONS
                    mode=gateway.choose(state,modeq['instructions'],modeq['criteria'])
                    kind=mode.get('choice');task=None;choice=None
                    if kind:
                        state['selected_task_kind']=kind
                        task_instructions=taskq['instructions'].split('\nYou selected task kind ')[0]+'\nYou selected task kind '+kind+'. Select the concrete task from current evidence.'
                        task=gateway.choose(state,task_instructions,{n:c for n,c in criteria.items() if eligible(n,kind)})
                        choice=task.get('choice')
                    option=state['options'].get(choice,{})
                    row={'case':label,'repeat':repeat,'variant':variant,'current_room':state['sensors']['room'],
                         'launch_room':state['ACTIVE_OBJECTIVE']['room'],'kind':kind,'choice':choice,
