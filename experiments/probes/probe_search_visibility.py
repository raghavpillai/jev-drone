...
from jev_drone.planning.visibility_memory import INSTRUCTIONS, VisibilityMemory
...
def main():
    ...
    files = iter(sorted((path / "result-frames").glob("*.npz")))
        ...
        cases.append(
            {
                ...
                "time": stamp,
                "call": call,
                "room_visibility": memory.summary(call["state"]["sensors"]["room"]),
            }
        )
        ...
        json.dumps(
            {
                ...
                "scope": "First five recorded search decisions, with sparse saved front-depth memory at request time. Choice differences only, no claimed flight improvement. No geometry or target truth enters memory; identical option universe in all arms.",
            },
            indent=2,
        )
        + "\n"
            ...
            for repeat in range(2):
                ...
                for variant in variants:
                    ...
                    if variant == "visibility":
                        state["room_visibility"] = case["room_visibility"]
