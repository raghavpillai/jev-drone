...
from jev_drone.planning.room_transition import describe, INSTRUCTIONS
...
def main():
    ...
    for call in calls:
        state=call['state']
        if 'selected_task_kind' not in state:continue
        previous=next((t for t in reversed(state.get('recent_tasks',[])) if t['name'].startswith('cross_') and t['outcome']=='arrived'),None)
    ...
    try:
        for index,call in enumerate(cases[:4]):
            for repeat in range(3):
                for variant in (('baseline','completed_transition') if repeat%2==0 else ('completed_transition','baseline')):
                    state=deepcopy(call['state']);q=deepcopy(call['questions']['selection'])
                    annotated,_=describe(state,q['criteria'],HOUSE)
                    reverse={name for name,option in annotated['options'].items() if option.get('reverses_completed_crossing')}
                    if variant=='completed_transition':
                        state,q['criteria']=describe(state,q['criteria'],HOUSE);q['instructions']+=INSTRUCTIONS
                    r=gateway.choose(state,q['instructions'],q['criteria'])
                    row=dict(index=index,repeat=repeat,variant=variant,room=state['sensors']['room'],choice=r.get('choice'),
                             backtracks=r.get('choice') in reverse,error=r.get('error'))
