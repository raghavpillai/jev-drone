...
from jev_drone.control.progress_facts import enrich, INSTRUCTIONS
...
def main():
    ...
    blocked = [c for c in source if c['state'].get('task', {}).get('name') == 'return_launch'
    ...
    try:
        for index, call in enumerate(selected):
            ...
            for variant in ('baseline', 'progress_facts'):
                ...
                row = {'index': index, 'variant': variant, 'choice': result.get('choice'),
