        ...
        json.dumps(
            {
                ...
                "not physical stopping or mission completion. Native physics must verify overshoot improvement.",
            },
            indent=2,
        )
        + "\n"
    ...
    try:
        for case in cases:
            for repeat in range(2):
                for variant in (
                    ["original", "fine_pan"] if repeat == 0 else ["fine_pan", "original"]
                ):
                    ...
                    if variant == "fine_pan":
                        ...
                        for side, rate in [("left", 0.08), ("right", -0.08)]:
                            ...
                            effect["patch"] = changed_channels(
                                state["current_controls"]["body_controls"],
                                effect["resulting_controls"],
                            )
                            state["control_effects"][name] = effect
                            question["criteria"][name] = (
                                "Apply "
                                + json.dumps(effect["patch"])
                                + "; retain omitted channels. "
                                "Fine pan for final alignment, 0.08 rad/s. Turn effect: "
                                + effect["turn_effect"]
                                + ". Movement violations: "
