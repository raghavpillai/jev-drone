...
from experiments.representations.door_centering import describe, INSTRUCTIONS, REFERENCE_INSTRUCTIONS
...
def main():
    ...
    cases, negatives = [], {}
    sources = [('px4-house-v36-validation', 'house_hidden-5311'),
    ...
    for cohort, trial in sources:
        ...
        for index, line in enumerate(path.read_text().splitlines()):
            ...
            if 'control_effects' not in state:
                continue
            updated, _ = describe(state, call['questions']['selection']['criteria'], local_reference=True)
            phase = state.get('doorway_phase', {}).get('phase')
            task = state['task']['name']
            useful = [name for name, effect in updated['control_effects'].items()
            ...
            if useful and task not in selected and len(selected) < 2:
                cases.append({'label': label, 'source': str(path), 'index': index,
                              'positive': True, 'call': call})
                selected.add(task)
            if phase in ('FACE_OPENING', 'CROSS_OPENING') and phase not in negatives:
                negatives[phase] = {'label': label, 'source': str(path), 'index': index,
                                    'positive': False, 'call': call}
    cases.extend(negatives.values())
    protocol = {'cases': [{k: v for k, v in c.items() if k != 'call'} for c in cases],
                'variants': ['original', 'facts', 'local_reference'], 'repeats': 2,
        ...
        for case in cases:
            for repeat in range(protocol['repeats']):
                ...
                for variant in variants:
                    ...
                    if variant != 'original':
                        state, question['criteria'] = describe(state, question['criteria'], local_reference=variant == 'local_reference')
                    ...
                    row = {'case': case['label'], 'positive': case['positive'], 'variant': variant,
                           ...
                           'centerline_progress': effect.get('door_centerline_effect') == 'toward_goal',
