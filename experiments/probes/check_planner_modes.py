...
from jev_drone.planning.planner_modes import INSTRUCTIONS, MODES, eligible
...
def main():
    ...
    paths = [
        Path("results/px4-autonomy-transit-v12/occluded-4753/result.json"),
    ...
    for path in paths:
        ...
        if "occluded" in path.parent.name:
            ...
            cases.extend(
                (path.parent.name, c) for c in planners if c["choice"] in ("inspect", "dock")
            )
            cases.extend(
                (path.parent.name, c) for c in planners if c["choice"].startswith("transit_")
            )
    gateway = JevGateway(
        ...
        for trial, call in cases:
            ...
            for repeat in range(2):
                ...
                if not mode.get("error"):
                    ...
                    criteria = {
                        k: v for k, v in question["criteria"].items() if eligible(k, selected)
                    }
                    task = gateway.choose(
                        {**state, "selected_task_kind": selected},
                ...
                row = dict(
                    ...
                    time=call["time"],
                    repeat=repeat,
                    recorded=call["choice"],
                    baseline=baseline.get("choice"),
                    mode=mode.get("choice"),
                    task=task.get("choice") if task else None,
                    errors=[r.get("error") for r in [baseline, mode, task or {}] if r.get("error")],
                    cost_usd=sum(
                        r.get("usage", {}).get("cost", 0) or 0 for r in [baseline, mode, task or {}]
                    ),
                )
                rows.append(row)
