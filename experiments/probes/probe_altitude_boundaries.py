...
def main():
    ...
    for trial in ("house_hidden-5311", "house_reverse-5312"):
        ...
        for index, line in enumerate(path.read_text().splitlines()):
            c = json.loads(line)
            s = c["state"]
            if "control_effects" not in s:
                continue
            updated, _ = describe(s, c["questions"]["selection"]["criteria"])
            fact = updated.get("altitude_access", {})
            names = (
                ("up", "up_creep") if s["body_goal_offset_m"]["up"] > 0 else ("down", "down_creep")
            )
            reasons = [
                reason
                for name in names
                for reason in s["control_effects"][name]["movement_violations"]
            ]
            kinds = []
            if (
                fact
                and not fact["requested_vertical_motion_permitted"]
                and s["horizontal_distance"] > 1.0
            ):
                kinds.append("vertical_blocked_far")
            if any("must_brake_before_reversing_up" == r for r in reasons):
                kinds.append("reverse_velocity_brake")
            if fact and len(fact["permitted_vertical_controls"]) == 1:
                kinds.append("creep_only_vertical")
            if s["at_position"]:
                kinds.append("position_reached")
            if s.get("doorway_phase", {}).get("phase") == "CROSS_OPENING":
                kinds.append("door_crossing")
            if s.get("active_detour"):
                kinds.append("active_bypass")
    ...
    if args.uniform:
        ...
        for prior in ("altitude", "boundaries"):
            for c in json.loads(
                Path(f"results/px4-house-v39-{prior}-probe/cases.json").read_text()
            ):
        ...
        for trial in ("house_hidden-5311", "house_reverse-5312"):
            ...
            for quantile in range(20):
                ...
                cases.append(
                    dict(
                        label=f"{trial}-{quantile}",
                        kind="uniform_unused_state",
                        source=str(path),
                        index=index,
                        call=c,
                    )
        ...
        json.dumps(
            {
                ...
                "variants": ["original", "altitude"],
                "repeats": repeats,
                "scope": "Development replay audit of v39. No changes to running policy. Uniform mode samples 20 unused control states across each v36 flight after excluding earlier probe cases; boundary mode uses stratified early cases. Neither is an independent flight reliability estimate.",
            },
            indent=2,
        )
        + "\n"
