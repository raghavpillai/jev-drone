...
def describe(state, criteria):
    ...
    eligible = (not any(state["current_controls"]["body_controls"].values())
                and observation["speed"] <= .1 and abs(observation["yaw_rate_rps"]) <= .1
                and not state["at_position"]
                and not progress.get("target_in_inspection_range") and not progress.get("dock_in_range")
                and any(not e["movement_violations"] and "toward_goal" in e["travel_effect"].values()
                        for e in effects.values() if "travel_effect" in e))
    state["neutral_and_settled_away_from_goal"] = eligible
    if eligible:
        for name, effect in effects.items():
            if not any(effect["resulting_controls"].values()) and not name.startswith("detour_") and name != "brake_and_replan":
                effect["position_progress"] = "already_stopped_no_position_progress"
                criteria[name] += " ALREADY STOPPED: this makes no position progress. A permitted translation toward the unfinished goal exists; do not repeat an already satisfied release."
...
def main():
    ...
    selected = []
    for trial, kind, count in (("sequence-4933", "stationary_loop", 3),
                               ("controller-4910", "braking", 2), ("controller-4910", "at_position", 1)):
        ...
        for index, (call, raw) in enumerate(zip(result["calls"], journal)):
            state = raw["state"]
            if call["role"] != "control" or raw.get("error") or call["time"]-last < 5.:
                continue
            if kind == "stationary_loop":
                match = (call["time"] > 300 and call.get("choice") == "stop_pan"
                         and describe(state, raw["questions"]["selection"]["criteria"])[0]["neutral_and_settled_away_from_goal"])
            elif kind == "braking":
                match = state.get("neutral_controls_still_braking", False)
            else:
                match = state["at_position"]
            if match:
                found.append((trial, kind, index, call["time"], raw)); last = call["time"]
            if len(found) == count:
                break
        if len(found) != count:
    ...
    (args.out/"protocol.json").write_text(json.dumps({
        "states": [{"trial": t,"kind": k,"call_index": i,"sim_time": s} for t,k,i,s,_ in selected],
        "variants": ["baseline", "noop_facts", "noop_and_path"], "repeats": 2,
        "noop_instruction": NOOP, "path_instruction": PATH,
        ...
        for trial, kind, index, time, raw in selected:
            for repeat in range(2):
                ...
                for variant in variants:
                    ...
                    state, criteria, instructions = raw["state"], question["criteria"], question["instructions"]
                    if variant != "baseline":
                    ...
                    row = dict(trial=trial,kind=kind,call_index=index,sim_time=time,repeat=repeat,variant=variant,
                               choice=choice,error=response.get("error"),violations=effect.get("movement_violations"),
