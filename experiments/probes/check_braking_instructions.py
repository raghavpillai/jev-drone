...
def main():
    ...
    parser.add_argument("--diverse", action="store_true", help="Sample each movement axis as well as braking failures")
    ...
    for path in sorted(Path("results/px4-joystick-final-validation").glob("*/result.json")):
        ...
        cases.extend((path.parent.name, "braking_failure", c) for c in controls if c["time"] in violations)
        # Include successful movement to check that the instructions do not just stop everything.
        nominal = [c for c in controls if c.get("accepted") and c["choice"] in ("forward_full", "strafe_left_slow", "pan_left", "up")
                   and not c["state"]["control_effects"][c["choice"]]["movement_violations"]]
        if args.diverse:
            nominal = []
            for name in ("forward_full", "strafe_left_slow", "strafe_right_slow", "pan_left", "pan_right", "up", "down"):
                call = next((c for c in controls if c.get("accepted") and c["choice"] == name
                             and not c["state"]["control_effects"][name]["movement_violations"]), None)
                if call:
                    nominal.append(call)
        cases.extend((path.parent.name, "nominal", c) for c in (nominal if args.diverse else nominal[:2]))
        ...
        for trial, kind, call in cases:
            for repeat in range(2):
                for variant in ("baseline", "braking_facts"):
                    ...
                    row = dict(trial=trial, time=call["time"], kind=kind, repeat=repeat, variant=variant,
                               choice=choice, violations=effect.get("movement_violations"),
                               nonzero=any(effect.get("resulting_controls", {}).values()),
                               latency_seconds=result["latency_seconds"], error=result.get("error"),
