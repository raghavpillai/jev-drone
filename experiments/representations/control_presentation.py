...
def describe(state, criteria, *, summary=False):
    ...
    for name, effect in effects.items():
        ...
        if reasons:
            ...
            criteria[name] = (
                "FORBIDDEN in this observation: "
                + ", ".join(reasons)
                + ". This command cannot be used to make progress now."
            )
    if summary:
        state = dict(state)
        state["control_feasibility"] = {
            "permitted_by_clearance": [
                name for name, e in effects.items() if not e.get("movement_violations")
            ],
            "forbidden": forbidden,
