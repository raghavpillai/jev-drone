...
def describe(state, criteria, *, local_reference=False):
    if (state.get('doorway_phase', {}).get('phase') != 'CENTER_IN_OPENING'
            or state.get('active_detour') or state.get('at_position')
    ...
    lateral_right = -math.cos(yaw) if abs(math.cos(required)) > .5 else math.sin(yaw)
    if abs(lateral_right) < .9:
        return state, criteria
    ...
    for name, effect in state['control_effects'].items():
        ...
        progress = ('stopped' if speed == 0 else 'axis_reached_brake' if ready
                    else 'toward_goal' if speed * remaining > 0 else 'away_from_goal')
