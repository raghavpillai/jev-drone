...
ACTIONS: dict[str, Vec] = {
    "forward": (1, 0, 0), "forward_left": (1, 1, 0),
    "left": (0, 1, 0), "back_left": (-1, 1, 0),
    "back": (-1, 0, 0), "back_right": (-1, -1, 0),
    "right": (0, -1, 0), "forward_right": (1, -1, 0),
    "up": (0, 0, 1), "down": (0, 0, -1), "brake": (0, 0, 0),
}
...
def unit(v: Vec) -> Vec:
    n = norm(v)
    return tuple(x/n for x in v) if n else (0., 0., 0.)
...
def scenario(name: str, seed: int) -> Scenario:
    ...
    y = 5. + rng.uniform(-.35, .35)
    start, goal = (1., y, 1.), (10.5, y, 1.)
    layouts = {
        "open": (),
        "pillar": (Box((4.5, 4., 0.), (5.5, 6., 4.)),),
        "wall": (Box((5., 2., 0.), (5.4, 8., 4.)),),
        "overpass": (Box((5., 0., 0.), (6., 10., 2.4)),),
        "u_trap": (
            Box((7., 3., 0.), (7.4, 7., 4.)),
            Box((3., 3., 0.), (7.4, 3.4, 4.)),
            Box((3., 6.6, 0.), (7.4, 7., 4.)),
    ...
    if seed:
        ...
        obstacles = tuple(Box((b.lo[0]+shift, b.lo[1], b.lo[2]),
                              (b.hi[0]+shift, b.hi[1], b.hi[2])) for b in obstacles)
        if seed % 2:
            start = (12.-start[0], start[1], start[2])
            goal = (12.-goal[0], goal[1], goal[2])
            obstacles = tuple(Box((12.-b.hi[0], b.lo[1], b.lo[2]),
                                  (12.-b.lo[0], b.hi[1], b.hi[2])) for b in obstacles)
...
class Simulation:
    ...
    radius: float = .2
    sensor_range: float = 4.
    goal_radius: float = .65
    command_lease: float = .8
    position: Vec = field(init=False)
    velocity: Vec = (0., 0., 0.)
    time: float = 0.
    action: str = "brake"
    command_speed: float = field(init=False)
    command_expires: float = 0.
    status: str = "running"
    settled: float = 0.
    path_length: float = 0.
    brake_time: float = 0.
    ...
    def advance(self, duration: float):
        ...
        while remaining > 1e-9 and self.status == "running":
            ...
            displacement = tuple((a+b)*.5*dt for a, b in zip(self.velocity, new_velocity))
            ...
            fraction = max(0., min(1., free/distance)) if collision else 1.
            ...
            if norm(subtract(self.world.goal, self.position)) <= self.goal_radius and norm(self.velocity) <= .15:
                self.settled += dt
                if self.settled >= .4:
                    self.status = "success"
            else:
                self.settled = 0.
            if not self.trace or self.time-self.trace[-1][0] >= .099 or self.status != "running":
                self.record()
    ...
    def observation(self, history: list, target: Vec | None = None) -> dict:
        ...
        labels = []
        for value, positive, negative in zip(delta, ["forward", "left", "up"], ["back", "right", "down"]):
            if abs(value) > .3:
        ...
        for name, direction in ACTIONS.items():
            ...
            ranges[name] = {
                ...
                "proximity": "very_close" if distance < .65 else "near" if distance < 1.3 else "open",
            }
        return {
            ...
            "goal": {"direction": "+".join(labels) or "here", "offset_metres_xyz": [round(x, 2) for x in delta],
                     "distance_metres": round(norm(delta), 2), "inside_arrival_radius": norm(delta) <= self.goal_radius},
    ...
    def summary(self) -> dict:
        return {
            "world": asdict(self.world), "status": self.status,
            "simulation_seconds": round(self.time, 3), "path_metres": round(self.path_length, 3),
            "final_distance_metres": round(norm(subtract(self.world.goal, self.position)), 3),
            "braking_seconds": round(self.brake_time, 3), "trace": self.trace,
            "physics": {"speed_mps": self.speed, "acceleration_mps2": self.acceleration,
                        "vehicle_half_extent_m": self.radius, "goal_radius_m": self.goal_radius,
