...
def audit(path):
    ...
    for call in accepted:
        ...
        if "control_options" in call:
            ...
            if choice.startswith(("slow_", "cruise_")):
                ...
                expected = np.array(DIRECTIONS[direction]) * (
                    0.25 if pace == "slow" else result["world"]["config"]["speed"]
                )
            values = dict(zip(("x", "y", "z"), expected.tolist()))
            patch = (
                {k: v for k, v in values.items() if v != old[k]}
            ...
            if state["control_effects"][call["choice"]] != {
                "patch": patch,
            ...
            if state["control_interface"] == "patch":
                ...
                if (
                    not receipt["applied"]
                    or receipt["expected_revision"] != state["current_controls"]["revision"]
                    or receipt["before"] != state["current_controls"]
        ...
        if choice.startswith(("slow_", "cruise_")):
            ...
            reasons = []
            if depth is None or depth < (0.4 if speed == "slow" else 1.3):
    ...
    if commands and "after" in commands[0]:
        events = [
            *commands,
            *[e for e in result.get("command_expirations", []) if e["wall"] >= commands[0]["wall"]],
        ]
        ...
        for event in sorted(events, key=lambda e: (e["wall"], e["before"]["revision"])):
            ...
            if "patch" in event:
                ...
                if (
                    ...
                    and event["expected_revision"] != previous["revision"]
                ):
                ...
                values = dict.fromkeys(("x", "y", "z"), 0.0)
    ...
    for setpoint in setpoints:
        ...
        if not preceding:
            ...
            adjacent = [
                c
                for c in commands
                if 0 < c["wall"] - setpoint["wall"] < 0.005
            ...
            if setpoint["wall"] - command["wall"] > 0.61:
    ...
    ages = [
        c["state"]["sensors"]["frame_age_seconds"] + c["latency_seconds"]
        for c in accepted
        if c["state"]["sensors"].get("frame_age_seconds") is not None
    ]
    intervals = np.diff([s["wall"] for s in setpoints])
    changes = sum(
        not np.allclose(c["velocity"], p["velocity"]) for p, c in zip(commands, commands[1:])
    )
    confirmations = sum(
        c["control_options"][c["choice"]]["resulting_velocity_mps"]
        == c["state"]["current_controls"]["velocity_mps"]
        for c in accepted
        if "control_options" in c
    )
    channel_assignments = sum(
        len(c["control_options"][c["choice"]]["patch"]) for c in accepted if "control_options" in c
    )
    return {
        "id": result["id"],
        ...
        "stages": result["stage"],
        "mission_stages": len(result["world"]["mission"]),
        ...
        "sensor_to_command_p95_ms": float(np.quantile(ages, 0.95) * 1000) if ages else None,
        ...
        "models": models,
        "command_vector_changes": changes,
        "unchanged_control_confirmations": confirmations,
        "published_vector_changes": sum(
            not np.allclose(a["velocity"], b["velocity"]) for a, b in zip(setpoints, setpoints[1:])
        ),
        "keep_choices": sum(c["choice"] == "keep_controls" for c in accepted),
        "assigned_velocity_channels": channel_assignments,
        "revision_rejections": sum(
            c.get("rejection_reason") == "control_state_changed_during_request" for c in controls
        ),
        "moving_lease_expirations": sum(
            any(e.get("before", {}).get("velocity_mps", {}).values())
            for e in result.get("command_expirations", [])
        ),
        "mean_local_input_tokens": float(
            np.mean(
                [
...
def main():
    ...
    table = [
        "| Trial | Outcome | Stages | Strict pass | Contacts | Rule violations | Seconds | Jev controls/s |",
        "|---|---|---:|---|---:|---:|---:|---:|",
    ]
    for row in rows:
        table.append(
            f"| {row['case']} | {row['status']} | {row['stages']}/{row['mission_stages']} | {'yes' if row['strict_pass'] else 'no'} | {row['contacts']} | {row['rule_violations']} | {row['wall_seconds']:.1f} | {row['accepted_controls_per_wall_second']:.2f} |"
        )
    ...
    print(
        json.dumps(
            {
                "trials": len(rows),
                "completions": sum(r["status"] == "success" for r in rows),
                "strict_passes": sum(r["strict_pass"] for r in rows),
                "cost_usd": sum(r["cost_usd"] for r in rows),
                "audit_problems": {
                    r["id"]: r["audit_problems"] for r in rows if r["audit_problems"]
                },
            },
            indent=2,
        )
