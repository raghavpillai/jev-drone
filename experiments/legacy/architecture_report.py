...
def metrics(path,e):
    ...
    roles={role:[c["latency_seconds"] for c in e["calls"] if c["role"]==role]
           for role in ["planner","navigator","recognition","control"]}
    return {"run":path.parent.name,"stage":"confirm" if "confirm" in path.parent.name else "screen",
        "architecture":e["architecture"],"planner":e["planner"],"case":e["case"],"seed":e["seed"],
        "status":e["status"],"seconds":e["simulation_seconds"],"path_m":e["path_metres"],
        "first_sighting_seconds":e["target_first_seen_seconds"],"coverage":e["coverage"],
        "calls":len(e["calls"]),"role_counts":e["role_counts"],"cost_usd":e["cost_usd"],
        "accepted_movement_hz":accepted/max(e["simulation_seconds"],.001),
        "movement_active_hz":accepted/max(sum(max(.2,c["latency_seconds"]) for c in controls),.001),
        "latency_median_ms":{r:1000*statistics.median(xs) if xs else None for r,xs in roles.items()},
        "latency_p95_ms":{r:1000*sorted(xs)[min(len(xs)-1,int(len(xs)*.95))] if xs else None for r,xs in roles.items()},
        "request_seconds_by_role":{r:sum(xs) for r,xs in roles.items()},
        "errors":sum(bool(c.get("error")) for c in e["calls"]),
        "stale_control_calls":sum(c["latency_seconds"]>.8 for c in controls),
...
def aggregate(rows):
    ...
    for (stage,architecture,planner),group in sorted(grouped.items()):
        ...
        result.append({"stage":stage,"architecture":architecture,"planner":planner,
            "present_successes":len(successes),"present_trials":len(present),
            ...
            "success_median_seconds":statistics.median(r["seconds"] for r in successes) if successes else None,
            "calls":sum(r["calls"] for r in group),"cost_usd":sum(r["cost_usd"] for r in group),
...
def plot(rows,stage):
    ...
    colors={"success":"#c9e9dc","correct_absence":"#c9e9dc","timeout":"#f4e4b7","collision":"#f2c9c9"}
    ...
    for y,(architecture,planner) in enumerate(conditions):
        for x,case in enumerate(cases):
            ...
            ax.text(x+.5,y+.5,label,ha="center",va="center",fontsize=10)
    ax.set(xlim=(0,4),ylim=(len(conditions),0),xticks=[i+.5 for i in range(4)],
           xticklabels=["Nearby target","Target in third room","Occluded target","Target absent"],
           yticks=[i+.5 for i in range(len(conditions))],
           yticklabels=[f"{a.title()} handoff\n{p.title()} planner" for a,p in conditions])
    ax.xaxis.tick_top()
    ax.tick_params(length=0)
    for spine in ax.spines.values():
        spine.set_visible(False)
    ax.set_title(f"{stage.title()} trials: every movement decision uses Jev\nMatched scenes · actual API delay included · 210 s deadline",pad=34,fontsize=14)
...
def main():
    ...
    Path("report/architecture-results.json").write_text(json.dumps({"episodes":rows,"summary":summary},indent=2))
