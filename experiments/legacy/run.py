...
from experiments.legacy.policy import (
    CRITERIA,
    INSTRUCTIONS,
    INTENT_CRITERIA,
    INTENT_INSTRUCTIONS,
...
def episode(args, name, seed, controller):
    ...
    while (
        ...
        and len(decisions) < args.max_calls
    ):
        ...
        if controller:
            ...
            decision = {
                "action": baseline(observation, args.policy, rng),
                "latency_seconds": args.baseline_delay,
            }
        latency = decision["latency_seconds"]
        delay = latency if args.timing == "delayed" else 0.0
        sim.advance(min(delay, max(0.0, args.seconds - sim.time)))
        received = sim.time
        accepted = (
            sim.status == "running"
            and not decision.get("error")
            and delay <= args.max_age
        ...
        decisions.append(
            {
                "sent_at": round(sent, 4),
                "received_at": round(received, 4),
                "accepted": accepted,
        ...
        cost += decision.get("usage", {}).get("cost", 0.0) or 0.0
        ...
        padding = max(0.0, 0.2 - delay) if args.timing == "delayed" else 0.2
        sim.advance(min(padding, max(0.0, args.seconds - sim.time)))
    ...
    result.update(
        {
            ...
            "stale_decisions": sum(d["latency_seconds"] > args.max_age for d in decisions)
    ...
    result["variant"] = {
        "dense_depth": getattr(args, "dense_depth", False),
        "long_memory": getattr(args, "long_memory", False),
    }
...
def main():
    ...
    parser.add_argument(
        "--scenarios",
        nargs="+",
        choices=["open", "pillar", "wall", "overpass", "u_trap"],
        default=["open", "pillar", "wall", "overpass", "u_trap"],
    )
    ...
    parser.add_argument("--baseline-delay", type=float, default=0.3)
    parser.add_argument("--episode-budget", type=float, default=0.025)
    ...
    if args.speed != 1.0:
        parser.error(
            "This prompt defines 1 m/s actions; change the prompt and experiment together before changing speed."
        )
    if args.out.exists():
    ...
    metadata = {
        ...
        "prompt": INSTRUCTIONS,
        "criteria": CRITERIA,
        "intent_prompt": INTENT_INSTRUCTIONS,
        "intent_criteria": INTENT_CRITERIA,
    }
    (args.out / "metadata.json").write_text(json.dumps(metadata, indent=2))
    controller = (
        Jev(load_key(args.key_file), args.dense_depth, args.long_memory, args.simple_controls)
        if args.policy == "jev"
        else None
    )
    total_cost = 0.0
                    ...
                    json.dumps(
                        {
                            ...
                            "distance": result["final_distance_metres"],
                            "calls": result["calls"],
                            "cost_usd": round(result["cost_usd"], 6),
                            "errors": result["errors"],
                            "stale": result["stale_decisions"],
                        }
                    ),
                    flush=True,
                )
