...
def main():
    ...
    parser.add_argument(
        "--plot-runs",
        nargs="+",
        help="Optional run directories to include in the figure; summaries always include all runs",
    )
    ...
    for name, episodes in runs.items():
        times = (
            [d["latency_seconds"] * 1000 for e in episodes for d in e["decisions"]]
        ...
        rows.append(
            {
                ...
                "other": sum(
                    e["status"] not in ["success", "collision", "timeout"] for e in episodes
                ),
                "calls": sum(e["calls"] for e in episodes),
                "median_ms": round(statistics.median(times), 1) if times else "",
                "p95_ms": round(percentile(times, 0.95), 1) if times else "",
    ...
    table = header + "\n".join(
        f"| {r['run']} | {r['success']}/{r['episodes']} | {r['collision']} | {r['timeout']} | {r['other']} | {r['calls']} | {r['median_ms']} / {r['p95_ms']} ms | ${r['cost_usd']:.6f} |"
        for r in rows
    )
    ...
    for name, episodes in runs.items():
        ...
        for seed in sorted({e["world"]["seed"] for e in episodes}):
            plot_rows.append(
                (
                    f"{name} / seed {seed}",
                    {e["world"]["name"]: e for e in episodes if e["world"]["seed"] == seed},
                )
            )
    if not plot_rows:
        return
    ...
    for row, (label, episodes) in enumerate(plot_rows):
        for column, name in enumerate(names):
            ...
            for box in world["obstacles"]:
                lo, hi = box["lo"], box["hi"]
                ax.add_patch(
                    Rectangle((lo[a], lo[b]), hi[a] - lo[a], hi[b] - lo[b], color="#d8dce0")
                )
            trace = e["trace"]
            ...
            ax.scatter(
                world["goal"][a], world["goal"][b], marker="*", s=140, color="#1970b5", zorder=5
            )
            ...
            ax.set_aspect("equal")
            ax.set_title(
                f"{name.replace('_', ' ')} · {e['status']}\n{e['simulation_seconds']:.1f}s / {e['final_distance_metres']:.2f}m remaining",
                loc="left",
                fontsize=10,
            )
    ...
    fig.suptitle(
        "Jev navigation prototype — actual recorded trajectories\nStart ●   Goal ★   End ×   Gray: obstacles   Overpass shown from the side",
        fontsize=16,
    )
