...
def matching_targets(home, item, room):
    ...
    return {
        ...
        if o.kind == item and (room == "anywhere" or room_at(o.box.lo) == room)
    }
...
class ActiveSearch:
    ...
    def observe(self):
        ...
        raw = visible_objects(
            self.scene.home, self.sim.position, view_direction=LOOK[self.pilot.look]
        )
        rng = random.Random(
            self.args.seed * 100003 + int(self.sim.time * 5) * 7 + list(LOOK).index(self.pilot.look)
        )
        self.visible = []
        for obj in raw:
            if rng.random() < 0.15:
    ...
    def options(self):
        ...
        options = {
            f"look_{look}": {
                "kind": "look",
                "xyz": list(self.sim.position),
                "look": look,
                "description": f"Stop and look {look} from this position.",
            }
            for look in LOOK
        }
        ...
        for name in sorted(eligible):
            ...
            if self.args.memory == "coverage":
                entry["potential_unseen_by_look"] = self.attention.novelty(
                    point["xyz"], point["room"]
                )
            options["go_" + name] = entry
        for obj in self.memory.values():
            if obj["room"] not in self.scene.scope or obj["room"] != room:
                continue
            options["inspect_" + obj["id"]] = {
                "kind": "inspect",
                "xyz": list(self.sim.position),
                "object": obj["id"],
                "look": look_toward(obj["visible_surface_xyz"], self.sim.position),
                "description": "Look toward this remembered surface to clarify/confirm it: "
                + obj["appearance"],
            }
            options["approach_" + obj["id"]] = {
                "kind": "approach",
                "xyz": obj["approach_xyz"],
                "object": obj["id"],
                "description": "Move to the observed standoff position, stop and look at: "
                + obj["appearance"],
            }
            options["report_found_" + obj["id"]] = {
                "kind": "report",
                "description": "Report this object as the requested item: " + obj["appearance"],
            }
        options["report_search_incomplete"] = {
            "kind": "report",
            "description": "Unable to finish searching; do not claim absence or success.",
        }
        return options
    ...
    def state(self, options):
        ...
        state = {
            ...
            "room_bounds": ROOMS,
            "doorways": self.scene.portals,
            "time": round(self.sim.time, 2),
            "remaining_seconds": round(self.args.seconds - self.sim.time, 1),
            "position_xyz": [round(v, 2) for v in self.sim.position],
            "speed_mps": round(norm(self.sim.velocity), 2),
            "current_room": current,
            "camera_look": self.pilot.look,
            "room_entries": list(self.entries),
            "looks_taken_near_this_position": sorted(
                self.attention.place_looks[tuple(round(v) for v in self.sim.position)]
            ),
            "observed_objects": [
                {
                    **o,
                    "visible_now": o["id"] in visible_ids,
                    "last_seen_ago_s": round(self.sim.time - o["last_seen_at"], 1),
                }
                for o in self.memory.values()
            ],
            "recent_task_results": self.tasks[-12:],
            "task_counts": dict(Counter(t["choice"] for t in self.tasks)),
        ...
        if self.args.memory == "coverage":
            ...
            state["recent_views"] = [
                {
                    "look": v["look"],
                    "new_visible_samples": v["new_visible_samples"],
                    "position": v["position"],
                }
                for v in self.attention.views[-6:]
            ]
            state["potential_unseen_from_here"] = self.attention.novelty(self.sim.position, current)
        return state

    def advance(self, seconds):
        self.sim.advance(min(seconds, max(0.0, self.args.seconds - self.sim.time)))
    ...
    def handle_api_result(self, result):
        ...
        if self.sim.status == "running" and (
            not transient or self.consecutive_api_errors >= self.api_error_limit
        ):
    ...
    def plan(self):
        ...
        if selected["kind"] == "report":
            fresh = visible_objects(
                self.scene.home, self.sim.position, view_direction=LOOK[self.pilot.look]
            )
            perceived = {
                o["id"] for o in self.visible if "unclear from this view" not in o["appearance"]
            }
            if choice == "report_search_incomplete":
                ...
                evaluator_home = replace(
                    self.scene.home,
                    target_id=reported_id if reported_id in self.target_ids else None,
                )
        ...
        self.task = {
            **selected,
            "choice": choice,
            "started_at": self.sim.time,
            "seen_before": len(self.attention.seen),
            "objects_before": set(self.memory),
        }
    ...
    def task_outcome(self):
        ...
        arrived = norm(subtract(task["xyz"], self.sim.position)) <= 0.65 and stopped
        elapsed = self.sim.time - task["started_at"]
        if task["kind"] in ("look", "inspect"):
            if self.pilot.look == task["look"] and stopped and elapsed >= 0.25:
    ...
    def controller_context(self):
        ...
        if self.task["kind"] == "approach":
            requested = look_toward(
                self.memory[object_id]["visible_surface_xyz"], self.sim.position
            )
        return {
            "task": self.task["kind"],
            "requested_look": requested,
    ...
    def move(self):
        ...
        accepted = (
            ...
            and self.sim.time < self.args.seconds
            and latency <= self.sim.command_lease
            and not result.get("error")
        )
        if self.sim.status == "running":
            self.sim.command(
                result["action"] if accepted else "brake",
                speed=result.get("speed") if accepted else None,
            )
        ...
        self.calls.append(
            {
                ...
                "sent_at": sent,
                "accepted": accepted,
                "task": self.task["choice"],
                **result,
            }
        )
        self.handle_api_result(result)
        self.advance(max(0.0, 0.2 - latency))
    ...
    def run(self):
        ...
        return {
            **self.sim.summary(),
            ...
            "mission": self.scene.mission,
            "room_entries": self.entries,
            "tasks": self.tasks,
            "objects_observed": self.memory,
            "coverage": {r: self.attention.summary(r) for r in ROOMS},
            "views": self.attention.views,
            ...
            "cost_usd": sum(c.get("usage", {}).get("cost", 0) or 0 for c in self.calls),
            "error": self.error,
        }
