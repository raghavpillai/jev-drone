...
def label(direction):
    return "_".join(
        p if v > 0 else n
        for v, p, n in zip(direction, ("forward", "left", "up"), ("back", "right", "down"))
        if v
    )
...
def world(case, seed):
    ...
    if seed:
        ...
        if case == "doorway":
            obstacles = (Box((5.85, 0, 0), (6.15, 4.5, 4)), Box((5.85, 6.5, 0), (6.15, 10, 4)))
        elif case == "overpass":
        ...
        def mirror(p):
            return size - p[0], p[1], p[2]

        base = replace(
            base,
            start=mirror(base.start),
            goal=mirror(base.goal),
            obstacles=tuple(
                Box((size - b.hi[0], b.lo[1], b.lo[2]), (size - b.lo[0], b.hi[1], b.hi[2]))
                for b in obstacles
            ),
        )
...
def observe(sim, history, intent, degraded, seed, target=None):
    ...
    cutoff = max(0.0, sim.time - (0.3 if degraded else 0.0))
    ...
    sensor = Simulation(sim.world, sensor_range=3.0)
    ...
    for direction in DIRECTIONS:
        ...
        if degraded:
            noise, missing = rng.gauss(0.0, 0.15), rng.random() < 0.15
            value = None if missing else min(3.0, max(0.0, round((value + noise) * 4) / 4))
        readings[label(direction)] = None if value is None else round(value, 2)
    common["sensor"] = {
        "range_cap_m": 3.0,
        "age_seconds": round(sim.time - capture_time, 3),
        "capture_position": [round(p, 2) for p in capture_position],
        "missing": "unknown, not free",
        "size_adjusted_clearance": True,
    }
...
def encode(common, readings, variant):
    ...
    if variant == "brief":
        ...
        def describe(name):
            return f"{name}: {bucket(readings[name])}" + (
                f", clearance {readings[name]} m" if readings[name] is not None else ""
            )

        return (
            "Current task, motion, observation age and memory:\n"
            + json.dumps(common)
    ...
    for z, name in ((1, "upward"), (0, "horizontal"), (-1, "downward")):
        layers[name] = [
            ["SELF" if (x, y, z) == (0, 0, 0) else readings[label((x, y, z))] for y in (1, 0, -1)]
            for x in (1, 0, -1)
        ]
    return {
        **common,
        "directional_depth_grid": layers,
...
def main():
    ...
    parser.add_argument(
        "--conditions", nargs="+", choices=("clean", "degraded"), default=["clean", "degraded"]
    )
    ...
    with ThreadPoolExecutor(max_workers=3) as pool:
        futures = {
            pool.submit(run, key, v, c, d == "degraded", args.seed, args.seconds): (v, c, d)
            for v, c, d in jobs
        }
