...
def probes():
    ...
    for room, bounds in ROOMS.items():
        ...
        for i, j, k in itertools.product(range(3), range(3), range(2)):
            ...
            for n, (dx, dy) in enumerate(itertools.product((-0.35, 0.35), (-0.55, 0.55))):
                name = f"{room}:{i}{j}{k}:{n}"
                result[name] = {
                    "room": room,
                    "region": region,
                    "height": "low" if k == 0 else "high",
                    "xyz": (x0 + 1 + i * 2 + dx, 10 / 6 + j * 10 / 3 + dy, 0.7 if k == 0 else 2.5),
                }
    return result
...
def in_view(point, position, look, range_m=3.5):
    ...
    distance = norm(delta)
    return 1e-6 < distance <= range_m and sum(a * b for a, b in zip(unit(delta), LOOK[look])) >= 0.5
...
def visible_probes(world, position, look):
    ...
    for name, probe in PROBES.items():
        point = probe["xyz"]
        if not in_view(point, position, look):
            continue
        delta = subtract(point, position)
        if any(
...
class SearchAttention:
    ...
    def record(self, observed, position, look, now, room, objects=()):
        ...
        event = {
            ...
            "look": look,
            "new_visible_samples": len(fresh),
            "visible_samples": len(observed),
    ...
    def summary(self, room):
        ...
        return {
            "seen_samples": seen,
            "total_samples": total,
            "fraction": round(seen / total, 3),
            "regions": dict(regions),
            "meaning": "Each height has four representative samples per region. Seen means actually visible; 0 may be occluded or not looked at. This is not proof the region is empty.",
        }

    def potential(self, position, look, room):
        # Geometry-free upper bound: no obstacle access, no predicted visibility oracle.
        return sum(
            name not in self.seen and p["room"] == room and in_view(p["xyz"], position, look)
            for name, p in PROBES.items()
        )
