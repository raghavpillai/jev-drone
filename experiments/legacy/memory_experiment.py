...
def run(provider,key,mode,case,seed,seconds,limited,degraded):
        ...
        while sim.status=="running" and sim.time<seconds-1e-9 and len(calls)<350:
            if nav.target is not None:
                ...
                if recovery:
                    ...
                    sim.advance(min(recovery["latency_seconds"],max(0.,seconds-sim.time)))
    ...
    sim.record()
    return {**sim.summary(),"memory_mode":mode,"case":case,"seed":seed,"provider":provider,
            "limited_view":limited,"degraded_depth":degraded,"calls":calls,"recovery_events":nav.events,
...
def main():
    ...
    parser.add_argument("--modes",nargs="+",choices=["history","map","recovery"],default=["history","map","recovery"])
    parser.add_argument("--cases",nargs="+",choices=["doorway","descent","overpass","u_trap"],default=["overpass","u_trap"])
    ...
    with ThreadPoolExecutor(max_workers=3) as pool:
        futures=[pool.submit(run,args.provider,key,m,c,s,args.seconds,args.limited,args.degraded) for m,c,s in jobs]
        for future in as_completed(futures):
            d=future.result()
            (args.out/f"{d['memory_mode']}-{d['case']}-{d['seed']}.json").write_text(json.dumps(d,indent=2))
            print(json.dumps({k:d[k] for k in ["memory_mode","case","seed","status","simulation_seconds","cost_usd"]}),flush=True)
