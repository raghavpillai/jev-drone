...
def main():
    ...
    samples=[]
    selections=[("architecture-screen-task-jev-occluded",0),
    ...
    for run,index in selections:
        ...
        p=plans[index]
        samples.append({"source":run,"index":index,"state":p["state"],"expected":p["choice"]})
    ...
    try:
        for sample in samples:
            ...
            if args.mode=="jev":
                ...
                result=deepseek_ask(client.client,sample["state"],TASK_INSTRUCTIONS,criteria,max_tokens=4096,reasoning_effort=args.mode)
            results.append({**sample,**result,"agrees_with_recorded_choice":result.get("choice")==sample["expected"]})
            print(json.dumps({"mode":args.mode,"choice":result.get("choice"),"expected":sample["expected"],
                              "latency":result["latency_seconds"],"error":result.get("error")}),flush=True)
    finally:
        client.close()
        (args.out/"probe.json").write_text(json.dumps({"mode":args.mode,"results":results},indent=2))
