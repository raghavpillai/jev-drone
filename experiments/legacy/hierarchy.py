...
from experiments.legacy.house import DESTINATIONS, PORTALS, ROOMS, WAYPOINTS, world, locate_room
...
def ask(client, state, instructions, criteria):
    ...
    response = client.post(ENDPOINT, json={"model": MODEL, "state": state, "questions": {
    ...
    return {"choice": answer["choice"], "answer": answer, "latency_seconds": time.perf_counter()-started,
            "usage": data.get("usage", {}), "model": data.get("model"), "state": state}


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--mission", required=True)
    parser.add_argument("--expected", choices=list(DESTINATIONS), required=True, help="Evaluator ground truth; never passed as the parsed model answer")
    parser.add_argument("--start-room", choices=list(ROOMS), default="living_room")
    parser.add_argument("--flat", action="store_true", help="Bypass global waypoint selection; still resolve the mission with Jev")
    ...
    best_distance, last_progress = float("inf"), 0.
    error = None
    try:
        resolution = ask(local.client, {"mission":args.mission,"destinations":DESTINATIONS},
        ...
        while sim.status == "running" and sim.time < args.seconds-1e-9 and len(decisions)<180 and len(plans)<20:
            arrived = target is not None and target_name != "destination" and norm(subtract(target,sim.position)) < .65 and norm(sim.velocity)<.15
            stalled = target is not None and sim.time-last_progress>8.
            if target is None or arrived or stalled:
                ...
                if args.flat:
                    ...
                    sim.command("brake")
                    state = {"mission":args.mission,"destination":DESTINATIONS[destination],
                             "current_room":locate_room(sim.position),
                             "position_xyz":sim.position,"rooms":ROOMS,"portals":PORTALS,
                             "obstacles": [asdict(b) for b in sim.world.obstacles],
                             "waypoints":candidates,"reached_waypoints":visited,
                             "previous_target":target_name,"local_report":"stalled" if stalled else "arrived" if arrived else "starting"}
                    plan = ask(local.client,state,
                ...
                intent = "direct"
                print(json.dumps({"next_waypoint":target_name,"xyz":target,"sim_seconds":round(sim.time,2)}),flush=True)
            ...
            accepted = sim.status=="running" and sim.time<args.seconds and latency<=.8 and not decision.get("error")
            if sim.status=="running":
                sim.command(decision["action"] if accepted else "brake")
            if accepted:
                intent=decision["intent"]
            decisions.append({"sent_at":sent,"received_at":sim.time,"accepted":accepted,"target":target_name,"observation":observation,**decision})
            ...
            if cost>=.05 and sim.status=="running":
        ...
        local.close()
        result=sim.summary()
        result.update({"mission":args.mission,"expected":args.expected,"plans":plans,"decisions":decisions,
                       "policy":"jev-flat" if args.flat else "jev-hierarchical","timing":"delayed",
                       "calls":len(plans)+len(decisions),"errors":sum(bool(d.get("error")) for d in decisions)+bool(error),
                       "stale_decisions":sum(d["latency_seconds"]>.8 for d in decisions),
                       "cost_usd":sum(d.get("usage",{}).get("cost",0) or 0 for d in decisions+plans),"error":error})
        (args.out/"episode.json").write_text(json.dumps(result,indent=2))
        print(json.dumps({k:result[k] for k in ["status","simulation_seconds","calls","cost_usd","error"]}),flush=True)
