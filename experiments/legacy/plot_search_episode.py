...
def main():
    ...
    plan=fig.add_subplot(grid[0,1]);alt=fig.add_subplot(grid[1,:])
    ...
    for obj in data['ground_truth']['objects']:
        ...
        space.add_collection3d(Poly3DCollection(faces(lo,hi),facecolor=color,edgecolor='#888c83',alpha=.32,linewidth=.4))
        plan.add_patch(Rectangle(lo[:2],hi[0]-lo[0],hi[1]-lo[1],facecolor=color,alpha=.5))
        if target:plan.text(lo[0],hi[1]+.15,'target',fontsize=8,color='#93461f')
    points=trace[:,1:3]
    segments=np.stack([points[:-1],points[1:]],axis=1)
    line=LineCollection(segments,cmap='viridis',norm=plt.Normalize(0,4),linewidth=1.8)
    line.set_array(trace[:-1,3]);plan.add_collection(line)
    ...
    space.set_box_aspect((18,10,6));space.view_init(25,-60)
    ...
    for wall in data['world']['obstacles'][:4]:
        ...
        space.add_collection3d(Poly3DCollection(faces(lo,hi),facecolor='#737d70',alpha=.06,linewidth=.2))
    for x,name in ((3,'living room'),(9,'bedroom'),(15,'study')):plan.text(x,9.7,name,ha='center',fontsize=8)
    plan.scatter(*points[0],color='#226d5f',s=35,zorder=4)
    plan.scatter(*points[-1],color='#b93f3c',marker='x',s=50,zorder=4)
    fig.colorbar(line,ax=plan,label='Height / m',shrink=.7)
    alt.plot(trace[:,0],trace[:,3],color='#23776b')
    for task in data['tasks']:
        if task['outcome']=='viewpoint_inspected':
            alt.scatter(task['time'],task['position'][2],marker='o',color='#7a5086',s=25,zorder=3)
    alt.set(xlim=(0,data['simulation_seconds']),ylim=(0,4),xlabel='Simulation time / seconds',ylabel='Height / m',
            title='Purple dots: completed stop-and-inspect viewpoints')
    fig.suptitle(f"{data['item']} in {data['room'].replace('_',' ')} · {data['status'].replace('_',' ')} · {data['simulation_seconds']:.1f}s\n"
                 f"{data['trial_id']} | All decisions: Jev through OpenRouter | Boxes shown only for evaluation",fontsize=13)
