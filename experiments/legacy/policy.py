...
CRITERIA = {
    name: f"Move {name.replace('_', '-')} at 1 m/s if that direction is OPEN and helps reach the destination or detour around an obstacle. Do not choose if BLOCKED."
    for name in ACTIONS
    if name != "brake"
}
CRITERIA["brake"] = (
    "Slow to a stop and hover: use when inside the arrival area, or when no movement is appropriate."
)
...
def sensor_text(observation: dict) -> str:
    ...
    for direction, reading in observation["ranges"].items():
        ...
        lines.append(
            f"{direction}: {'BLOCKED' if distance < 1.3 else 'OPEN'}, clearance {distance} metres."
        )
    lines += [
        "Ranges cap at 4 metres. Space beyond that and between rays is unknown.",
...
def load_key(path: Path | None = None) -> str:
    ...
    if not key and path:
        match = re.search(
            r"(?m)^\s*(?:export\s+)?OPENROUTER_API_KEY\s*=\s*[\"']?([^\s\"'#]+)", path.read_text()
        )
...
class Jev:
    def __init__(self, key: str, dense_depth=False, long_memory=False, simple_controls=False):
        ...
        self.criteria = {
            ...
            if not simple_controls or "_" not in name
        }
        self.client = httpx.Client(
            headers={"Authorization": f"Bearer {key}"}, timeout=3.0, follow_redirects=False
        )
    ...
    def decide(self, observation: dict) -> dict:
        ...
        guidance = ""
        if self.dense_depth:
            state += "\nAdditional depth rays (direction: metres of vehicle clearance): " + str(
                observation["depth_rays"]
            )
        ...
        if self.long_memory:
            state += (
                "\nEarlier visited positions, goal distances, and movements (chronological): "
                + str(observation["breadcrumbs"])
            )
        ...
        try:
            ...
            if set(probabilities) != set(self.criteria) or any(
                not isinstance(p, (float, int)) or not 0 <= p <= 1 for p in probabilities.values()
            ):
...
def baseline(observation: dict, name: str, rng: random.Random) -> str:
    ...
    return max(
        options, key=lambda a: sum(x * y for x, y in zip(unit(ACTIONS[a]), delta)), default="brake"
    )
