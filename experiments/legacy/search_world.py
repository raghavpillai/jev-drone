...
ROOMS = {name: {"bounds_xyz": [[i*6, 0, 0], [(i+1)*6, 10, 4]]}
         for i, name in enumerate(["living_room", "bedroom", "study"])}
PORTALS = [{"rooms": [a, b], "opening_xyz": [[x-.15, 4, 0], [x+.15, 7, 4]],
...
def make_home(seed, target_room="bedroom", absent=False):
    ...
    walls = tuple(Box((x-.15, y0, 0), (x+.15, y1, 4))
                  for x in (6, 12) for y0, y1 in [(0, 4), (7, 10)])
    furniture = []
    # Beds/tables are low 3D solids with rectangular floor footprints, not full-height walls.
    for i, name in enumerate(ROOMS):
        x = i*6
        shift = rng.uniform(-.35, .35)
        kind, appearance = ("bed", "A low padded rectangular platform with a mattress, pillows and folded blanket.") if name == "bedroom" else (
            "table", "A low broad horizontal tabletop, with four short legs and a smooth empty top.")
        furniture.append((kind, Box((x+2+shift, 1.2, 0), (x+4.2+shift, 3.5, .75)), appearance))
        furniture.append(("cabinet", Box((x+.3, 8.8, 0), (x+1.4, 9.6, 2.5)),
                          "A tall wooden storage unit with solid closed doors; no visible shelves or books."))
        furniture.append(("plant", Box((x+4.8, 7.7, 0), (x+5.4, 8.3, 1.7)),
                          "A pot supporting green leaves and thin upright stems."))
    if not absent:
        x = list(ROOMS).index(target_room)*6
        center = x+rng.uniform(2.6, 4.1)
        furniture.append(("bookcase", Box((center-.7, 9.1, 0), (center+.7, 9.7, 2.7)),
    ...
    objects = tuple(Object(f"object_{i+1}", kind, box, appearance)
    ...
    world = Scenario("unknown_furniture", seed, (2., 5.5, 1.4), (-100., -100., -100.),
                     walls+tuple(o.box for o in objects), (18., 10., 4.))
    ...
    for obj in home.objects:
        ...
        for point in itertools.product(*axes):
            ...
            if not 1e-6 < distance <= max_range:
                continue
            if view_direction is not None and sum(a*b for a,b in zip(unit(delta), unit(view_direction))) < .5:
                continue
            # Includes the object's own geometry: rear faces cannot be seen through it.
            if any((hit := ray_box(position, delta, box)) is not None and hit < 1-1e-6
                   for box in home.world.obstacles):
                continue
            visible.append((distance, point))
        if visible:
            distance, point = min(visible)
            observations.append({"id": obj.id, "appearance": obj.appearance,
                                 "visible_surface_xyz": list(point), "range_metres": round(distance, 2),
                                 "room": room_at(point)})
    return observations
...
def update_memory(memory, observations, position, now):
    for observation in observations:
        ...
        memory[observation["id"]] = {**observation, "first_seen_at": old.get("first_seen_at", now),
                                      "last_seen_at": now,
...
def search_waypoints():
    ...
    for room, data in ROOMS.items():
        ...
        for side, dx, y in [("south_west", 1.6, 2.5), ("south_east", 4.4, 2.5),
                            ("north_west", 1.6, 7.5), ("north_east", 4.4, 7.5)]:
            points[f"search_{room}_{side}"] = {"xyz": [x+dx, y, 1.4], "room": room,
                                                "description": f"Look around the {side} part of {room}."}
        points[f"search_{room}_high"] = {"xyz": [x+3, 6, 2.8], "room": room,
    ...
    for i, portal in enumerate(PORTALS):
        for room, point in portal["approaches"].items():
            points[f"door_{i}_{room}"] = {"xyz": point, "room": room,
...
def evaluate_report(choice, home, sim, visible):
    ...
    if distance_to_box(sim.position, obj.box) > 2.15 or norm(sim.velocity) > .15:
