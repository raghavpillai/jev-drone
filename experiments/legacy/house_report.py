...
def main():
    ...
    for ax, name in zip(axes, names):
        ...
        ax.plot(
            ...
            color="#187d67",
            linewidth=2,
            label="Executed local movements",
        )
        ...
        ax.grid(alpha=0.12)
        ax.set_title(
            e["mission"] + f"\n{e['status']} · {e['simulation_seconds']:.1f}s",
            loc="left",
            fontsize=12,
        )
        summary.append(
            {
                ...
                "global_waypoints": sequence,
                "global_calls": len(e["plans"]),
                "local_calls": len(e["decisions"]),
                "accepted_local_hz": sum(d["accepted"] for d in e["decisions"])
                / e["simulation_seconds"],
                "cost_usd": e["cost_usd"],
            }
        )
    fig.suptitle(
        "Jev global + local navigation\nNumbers: selected global waypoints · Green: executed local trajectory · Gray: solid objects",
        fontsize=15,
    )
