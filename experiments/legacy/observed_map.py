...
class ObservedMap:
    ...
    def record(self, position, readings, now, cap=3.):
        def add(point, kind):
            key = self.cell(point)
        ...
        for direction in DIRECTIONS:
            ...
            for step in range(1, max(1, int((value-.25)/self.resolution)+1)):
        ...
        if not self.landmarks or norm(subtract(position, self.landmarks[-1]["xyz"])) >= 1.:
            self.landmarks.append({"id": f"visited_{len(self.landmarks)}", "xyz": [round(p,2) for p in position], "seen_at": round(now,2)})
    ...
    def describe(self, position, now):
        ...
        for key, item in self.cells.items():
            point = [(k+.5)*self.resolution for k in key]
            distance = norm(subtract(point, position))
            if distance <= 4.:
                nearby.append((distance, {"relative_xyz": [round(p-q,2) for p,q in zip(point,position)],
                                         "evidence": self.evidence(point), "age_s": round(now-item["last_seen"],2)}))
        nearby.sort(key=lambda pair: pair[0])
        return {"meaning": "Sparse ray samples, not certified free cells. Unlisted space is UNKNOWN. Evidence can be stale or conflicting; no route computed.",
                "cell_width_m": self.resolution, "recorded_cells": len(self.cells),
...
def recovery_options(position, memory):
    points = {"resume": None}
    criteria = {"resume": "Keep pursuing the mission waypoint; the local controller can still make progress."}
    for direction in DIRECTIONS:
        ...
        points[name] = point
        criteria[name] = f"Temporary exploration waypoint {point}; map evidence at endpoint: {memory.evidence(point)}. This does not guarantee a clear path."
    for item in memory.landmarks[-12:]:
        if norm(subtract(item["xyz"], position)) > 1.:
            points[item["id"]] = item["xyz"]
            criteria[item["id"]] = f"Return toward previously visited position {item['xyz']}, observed at {item['seen_at']} s. The straight path may be obstructed."
