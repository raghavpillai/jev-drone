...
def course(name):
    if name == "descent":
        return Scenario(name, 0, (1.6,6.,2.8), (1.6,2.5,1.4),
                        (Box((1.95,1.2,0), (4.2,3.5,.75)),))
    if name == "overpass":
        return Scenario(name, 0, (2.,5.,1.4), (9.,5.,1.4),
                        (Box((5.,0,0), (6.,10.,2.4)),))
    return Scenario(name, 0, (2.,5.5,1.4), (8.5,5.5,1.4),
                    (Box((5.85,0,0), (6.15,4.,4.)), Box((5.85,7.,0), (6.15,10.,4.))))
...
def run(args, controller):
    ...
    while sim.status == "running" and sim.time < args.seconds-1e-9 and len(calls)<160:
        ...
        try:
            if args.variant == "original":
                ...
                decision = choose_control(controller.client, sim, sim.world.goal, history,
                                          {"task": "reach_waypoint_and_stop"}, slow=args.variant=="slow")
        except Exception as exc:
            decision = {"action": "brake", "speed": 0., "latency_seconds": time.perf_counter()-started,
        ...
        if sim.status == "running":
            sim.command(decision["action"] if accepted else "brake", decision["speed"] if accepted else 0.)
        if accepted:
            intent = decision.get("intent", intent)
        history.append({"position": [round(x,2) for x in sim.position], "action":decision["action"],
    ...
    if sim.status == "running":
        sim.status = "timeout"
    sim.record()
    return {**sim.summary(), "variant":args.variant,"decisions":calls,
            "calls":len(calls),"cost_usd":sum(d.get("usage",{}).get("cost",0) or 0 for d in calls)}
...
def main():
    ...
    print(json.dumps({k:result[k] for k in ["variant","status","simulation_seconds","calls","cost_usd"]}),flush=True)
