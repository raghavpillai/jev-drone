...
def main():
    ...
    trace=data['trace'];times=[p[0] for p in trace]
    views=data['views'];view_times=[v['time'] for v in views]
    commands=[]
    for call in data['calls']:
        if call['role']=='control':
            commands.append((call['sent_at']+call['latency_seconds'],
                call.get('selected_action',call['action']) if call.get('accepted') else 'brake / stale response'))
        else:
            commands.append((call['sent_at'],'brake / deliberation'))
    commands.sort();received=[t for t,_ in commands]
    fig,(plan,alt)=plt.subplots(2,1,figsize=(9,7),gridspec_kw={'height_ratios':[4,1]})
    fig.subplots_adjust(top=.81,bottom=.16,hspace=.45)
    fig.patch.set_facecolor('#f5f2ea')
    furniture=[]
    for obj in data['ground_truth']['objects']:
        lo,hi=obj['box']['lo'],obj['box']['hi']
        patch=Rectangle(lo[:2],hi[0]-lo[0],hi[1]-lo[1],facecolor='#adb4ab',alpha=.12)
        plan.add_patch(patch);furniture.append((obj,patch))
    for wall in data['world']['obstacles'][:4]:
        lo,hi=wall['lo'],wall['hi']
        plan.add_patch(Rectangle(lo[:2],hi[0]-lo[0],hi[1]-lo[1],facecolor='#737d70',alpha=.5))
    for x,name in ((3,'living room'),(9,'bedroom'),(15,'study')):plan.text(x,9.7,name,ha='center',fontsize=9)
    plan.set(xlim=(0,18),ylim=(0,10),aspect='equal',xlabel='x / m',ylabel='y / m')
    path,=plan.plot([],[],color='#247b6e',linewidth=1.5)
    drone=Rectangle((0,0),.4,.4,facecolor='#164e47',edgecolor='white',zorder=5);plan.add_patch(drone)
    ...
    title=fig.suptitle('',fontsize=12,x=.1,ha='left')
    caption=fig.text(.1,.035,'',fontsize=9)
    directions={'forward':(1,0),'back':(-1,0),'left':(0,1),'right':(0,-1),'up':(0,0),'down':(0,0)}
    # Derive persistent memory from actual logged sightings, not target labels.
    seen=set();memory=[]
    for view in views:
        seen.update(view['objects']);memory.append(seen.copy())

    def update(t):
        index=max(0,bisect.bisect_right(times,t)-1)
        p=trace[index];prefix=trace[:index+1]
        view_index=max(0,bisect.bisect_right(view_times,t)-1)
        view=views[view_index];seen=memory[view_index]
        path.set_data([r[1] for r in prefix],[r[2] for r in prefix])
        height.set_data([r[0] for r in prefix],[r[3] for r in prefix])
        drone.set_xy((p[1]-.2,p[2]-.2))
        dx,dy=directions[view['look']]
        aim.set_data([p[1],p[1]+dx],[p[2],p[2]+dy])
        for obj,patch in furniture:
            patch.set_alpha(.55 if obj['id'] in seen else .12)
            patch.set_facecolor('#d9843b' if obj['id'] in seen and obj['id'] in data['eligible_target_ids'] else '#adb4ab')
        ...
        title.set_text(f"Jev room search · {data['item']} in {data['room'].replace('_',' ')}\n"
                       f"{status} | {t:.1f}s | {args.speed:g}× playback | height {p[3]:.2f}m\n"
                       f"Action: {action} | Camera: {view['look']} | Objects remembered: {len(seen)}")
        caption.set_text('Recorded live OpenRouter decisions. Purple line: camera direction in the floor plane.\n'
    ...
    frames=[i*args.speed/10 for i in range(int(times[-1]*10/args.speed)+1)]+[times[-1]]*12
