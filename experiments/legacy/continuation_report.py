...
def paths():
    return sorted(
        p
        for folder in Path("results").glob("search-v*")
...
def verify(data):
    ...
    def box(b):
        return Box(tuple(b["lo"]), tuple(b["hi"]))
    ...
    world = Scenario(
        ...
        tuple(raw["start"]),
        tuple(raw["goal"]),
        tuple(box(b) for b in raw["obstacles"]),
        tuple(raw["size"]),
    )
    objects = tuple(
        Object(o["id"], o["kind"], box(o["box"]), o["appearance"])
        for o in data["ground_truth"]["objects"]
    )
    report = data["calls"][-1]
    choice = report["choice"]
    oid = choice.removeprefix("report_found_")
    assert oid in data["eligible_target_ids"]
    state = report["state"]
    evidence = next(
        o for o in state.get("objects", state.get("observed_objects", [])) if o["id"] == oid
    )
    assert evidence["visible_now"] and "unclear from this view" not in evidence["appearance"]
    home = Home(world, objects, oid)
    sim = Simulation(world)
    sim.position = tuple(data["trace"][-1][1:4])
    sim.velocity = tuple(data["trace"][-1][4:7])
...
def stationary_task_streak(tasks):
    ...
    for task in tasks:
        ...
        if position is None:
            continue
        if anchor is None or norm(subtract(position, anchor)) >= 0.15:
            anchor = position
            count = 1
        else:
            count += 1
...
def control_violations(calls):
    ...
    for call in calls:
        ...
        reading = re.search(
            rf"(?m)^{action}: (OPEN|BLOCKED|UNKNOWN)(?:, clearance ([\d.]+) m)?", state
        )
        slow = call.get("speed", 1.0) <= 0.3
        enough_depth = (
            (reading and reading[2] is not None and float(reading[2]) >= 0.45)
            if slow
            else (reading and reading[1] == "OPEN")
        )
        if not enough_depth:
            counts["translation_without_sufficient_depth"] += 1
        if common["goal"]["inside_arrival_radius"]:
            counts["translation_after_arrival"] += 1
...
def summarize(path):
    ...
    accepted = (
        ...
        if target_present
            ...
            data["status"] == "search_incomplete"
            and data["room"] in coverage
            and coverage[data["room"]] >= 0.85
        )
    )
    ...
    target_sightings = [
        v["time"] for v in data["views"] if set(v["objects"]) & set(data["eligible_target_ids"])
    ]
    entry_looks = [
        {
            "room": entry["room"],
            "time": entry["time"],
            "first_six_seconds_looks": sorted(
                {
                    v["look"]
                    for v in data["views"]
                    if v["room"] == entry["room"]
                    and entry["time"] <= v["time"] <= entry["time"] + 6
                }
            ),
        }
    ...
    return {
        ...
        **{
        ...
        "report_verification": verify(data),
        "stationary_task_streak": stationary,
        "completion_and_loop_check_pass": accepted and stationary <= 10,
        ...
        "room_entry_looks": entry_looks,
        "first_target_instance_sighting_s": min(target_sightings) if target_sightings else None,
        "coverage": coverage,
        "maximum_altitude_m": round(max(row[3] for row in data["trace"]), 2),
        "repeated_tasks": {k: v for k, v in counts.items() if v > 1},
        "stalled_tasks": sum(t["outcome"].startswith("stalled") for t in data["tasks"]),
        "calls": len(calls),
        ...
        "unpriced_api_errors": sum(
            bool(c.get("error")) and "cost" not in c.get("usage", {}) for c in calls
        ),
        "stale_controls": sum(
            c["latency_seconds"] > data["physics"]["command_lease_s"] for c in control
        ),
        "median_control_latency_ms": round(statistics.median(latency) * 1000, 1)
        if latency
        else None,
        "p95_control_latency_ms": round(
            latency[min(len(latency) - 1, int(len(latency) * 0.95))] * 1000, 1
        )
        if latency
        else None,
        "inference_capacity_hz": round(len(control) / sum(latency), 2) if latency else None,
        "control_hz": round(len(control) / sum(max(0.2, t) for t in latency), 2)
        if latency
        else None,
        "mission_control_hz": round(len(control) / data["simulation_seconds"], 2)
        if latency
        else None,
        "accepted_mission_control_hz": round(
            sum(bool(c.get("accepted")) for c in control) / data["simulation_seconds"], 2
        )
        if latency
        else None,
    }
...
def main():
    ...
    for phase in sorted({r["phase"] for r in results}):
        ...
        phases[phase] = {
            "episodes": len(rows),
            "target_successes": sum(r["status"] == "success" for r in present),
            "target_trials": len(present),
            "absence_search_passes": sum(r["acceptance_pass"] for r in absent),
            "absence_trials": len(absent),
            ...
            "episodes_with_control_violations": sum(
                bool(r["control_rule_violations"]) for r in rows
            ),
    ...
    columns = [
        ...
        "path_metres",
        "acceptance_pass",
        "stationary_task_streak",
        "completion_and_loop_check_pass",
        "strict_pass",
        "calls",
        "cost_usd",
        "control_hz",
