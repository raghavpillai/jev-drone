...
class AllJevExperiment(Experiment):
    ...
    def visible(self):
        ...
        visible = visible_objects(self.scene.home,self.sim.position,
        ...
        for obj in visible:
            if self.args.uncertain and rng.random() < .15:
                continue
            self.sightings[obj["id"]] += 1
            obj = dict(obj)
            if self.args.uncertain and self.sightings[obj["id"]] <= 2 and "wooden" in obj["appearance"]:
    ...
    def observe(self):
        ...
        for name,point in self.scene.points.items():
            if norm(subtract(point["xyz"],self.sim.position)) <= .65:
                ...
                if not self.args.limited or set(("forward","back","left","right")) <= self.view_angles[name]:
        ...
        if signature != self.recognition_signature and signature:
            ...
            state = {"mission":self.scene.mission,"objects":[{"id":k,"appearance":v["appearance"],"room":v["room"]} for k,v in self.memory.items()]}
            match = self.request("recognition",state,
                ...
                {"none":"No supported match",**{k:v["appearance"] for k,v in self.memory.items()}})
    ...
    def state(self):
        state = planning_state(self.scene,self.sim,self.memory,self.visible(),self.checked,self.recognized,self.report)
        state["observation_contract"] = {"furniture_initially_unknown":True,"limited_view":self.args.limited,
            "current_look":self.nav.look,"object_memory_has_last_seen_times":True,
            "coverage": "Search viewpoint requires four horizontal camera looks" if self.args.limited else "Visited configured viewpoints"}
    ...
    def choose_waypoint(self):
        ...
        if self.args.limited and self.target_name and self.target_name.startswith("search_") and self.target_name not in self.checked and norm(subtract(self.target,self.sim.position)) <= .65:
            return
    ...
    def move(self):
        ...
        if recovery:
            ...
            self.sim.advance(min(recovery["latency_seconds"],max(0.,self.args.seconds-self.sim.time)))
        if not self.active():
            return
        missing = []
        if self.target_name and self.target_name.startswith("search_") and norm(subtract(self.target,self.sim.position)) <= .65:
            missing = sorted(set(("forward","back","left","right"))-self.view_angles[self.target_name])
        sent = self.sim.time
        decision = self.nav.decide(self.sim,self.target,{"task":self.task,"missing_views_here":missing})
        ...
        self.calls.append({"role":"control","sent_at":sent,"accepted":accepted,"waypoint":self.target_name,**decision})
    ...
    def run(self):
        result = super().run()
        result.update(memory_mode=self.args.memory,limited_view=self.args.limited,uncertain_semantics=self.args.uncertain,
                      degraded_depth=self.args.degraded,provider=self.local.provider,recovery_events=self.nav.events,
                      observed_map_cells=len(self.nav.map.cells),camera_views={k:sorted(v) for k,v in self.view_angles.items()},
