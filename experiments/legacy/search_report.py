...
def metrics(path, e):
    ...
    return {
        ...
        "target_seen": e["target_seen"],
        "target_identified": e["target_identified"],
        "first_seen_seconds": e["target_first_seen_seconds"],
        "rooms_entered": sorted({room_at(p[1:4]) for p in e["trace"]}),
        "search_views_completed": {
            room: sum(v.startswith(f"search_{room}_") for v in e["visited_waypoints"])
            for room in ROOMS
        },
        ...
        "planner_median_ms": statistics.median(p["latency_seconds"] for p in plans) * 1000
        if plans
        else None,
        "calls": e["calls"],
        "global_calls": len(e["plans"]),
...
def plot_episode(ax, e, title):
    ...
    for obj in objects:
        ...
        ax.add_collection3d(
            Poly3DCollection(
                box_faces(obj["box"]["lo"], obj["box"]["hi"]),
                facecolor=color,
                edgecolor="#536475",
                alpha=0.4,
                linewidth=0.5,
    ...
    trace = e["trace"]
    ax.plot(
        [p[1] for p in trace], [p[2] for p in trace], [p[3] for p in trace], color="#127d67", lw=2
    )
    ...
    if e["target_first_seen_seconds"] is not None:
        ...
        ax.text(first[1], first[2], first[3] + 0.45, "First target sighting", fontsize=8)
    for i, room in enumerate(ROOMS):
        ax.text(i * 6 + 1, -0.8, 0, room.replace("_", " "), fontsize=9)
    ax.set(
        xlim=(0, 18), ylim=(0, 10), zlim=(0, 4), xlabel="x (m)", ylabel="y (m)", zlabel="height (m)"
    )
...
def main():
    ...
    runs = args.runs or [
        Path("results/search-calibration-jev"),
        Path("results/search-eval-jev-study"),
    ]
    fig = plt.figure(figsize=(15, 6), layout="constrained")
    for i, run in enumerate(runs[:2], 1):
        e = json.loads((run / "episode.json").read_text())
        plot_episode(
            fig.add_subplot(1, 2, i, projection="3d"),
            e,
            "Jev: known target room" if "calibration" in run.name else "Jev: unknown target room",
        )
    fig.suptitle(
        "Search with unknown furniture: actual executed 3D paths\nEvaluator view: gold = target; all furniture was hidden from the model until sensed",
        fontsize=14,
    )
