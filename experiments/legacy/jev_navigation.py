...
from experiments.legacy.representation_experiment import (
    DIRECTIONS,
    GUIDANCE,
    encode,
    label,
...
class Navigation:
    def __init__(self, gateway, mode="history", limited=False, degraded=False, seed=0):
        ...
        self.last_recovery = -10.0

    def observation(self, sim, target):
        common, readings = observe(
            sim, self.history, self.intent, self.degraded, self.seed, target=target
        )
        capture_time = sim.time - common["sensor"]["age_seconds"]
        capture_look = next(
            (look for when, look in reversed(self.look_history) if when <= capture_time + 1e-3),
            "forward",
        )
        if self.limited:
            for direction in DIRECTIONS:
                if sum(a * b for a, b in zip(unit(direction), LOOK[capture_look])) < 0.5:
                    readings[label(direction)] = None
        if self.mode != "history":
            self.map.record(
                common["sensor"]["capture_position"],
                readings,
                sim.time - common["sensor"]["age_seconds"],
            )
        common["camera"] = {
            "look_direction": self.look,
            "captured_look_direction": capture_look,
            "coverage": "120 degree cone" if self.limited else "surround",
    ...
    def recovery(self, sim, target, report):
        if (
            self.mode != "recovery"
            or sim.time - self.last_progress < 6
            or sim.time - self.last_recovery < 6
        ):
            return None
        common, readings = self.observation(sim, target)
        points, criteria = recovery_options(sim.position, self.map)
        state = {
            "mission_waypoint": target,
            "local_report": report,
            "current": encode(common, readings, "brief"),
            "observed_map": self.map.describe(sim.position, sim.time),
            "previous_recovery_attempts": self.events[-8:],
        }
        ...
        if not result.get("error"):
            ...
            self.events.append(
                {"at": sim.time, "choice": result["choice"], "xyz": self.recovery_target}
            )
            self.intent = "direct"
        return result
    ...
    def decide(self, sim, target, extra=None):
        if self.target != tuple(target):
            ...
            self.intent = "direct"
        if self.recovery_target and norm(subtract(self.recovery_target, sim.position)) <= 0.65:
            self.recovery_target = None
    ...
    def questions(self):
        ...
        if self.limited:
            questions["look"] = {
                "type": "choice",
                "instructions": "Where should the camera look for the NEXT observation? Missing directions are unknown. Look toward intended travel or unexplored space. If stopped at a search viewpoint, inspect missing_views_here before leaving. Camera aim is independent of movement; it does not rotate the flight axes.",
    ...
    def accept(self, sim, decision, accepted, target):
        ...
        self.history.append(
            {
                "position": [round(p, 2) for p in sim.position],
