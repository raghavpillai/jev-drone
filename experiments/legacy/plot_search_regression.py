...
PAIRS = [
    (
        "Before: stale approach and conflicting completion rules",
        Path("results/search-v6-heldout/heldout-occluded-bookcase-712.json"),
    ),
    (
        "After: current approach and shared completion facts",
        Path("results/search-v8-screen/v8-bookcase-712.json"),
    ),
]
...
def main():
    ...
    for col, (label, path) in enumerate(PAIRS):
        ...
        for obj in d["ground_truth"]["objects"]:
            ...
            ax.add_patch(
                Rectangle(
                    ...
                    xx - x,
                    yy - y,
                    facecolor="#df9354" if target else "#d7d5ca",
                    edgecolor="#a09f94",
                )
            )
            if obj["kind"] == "screen":
                ax.text(
                    (x + xx) / 2, (y + yy) / 2, "partition", ha="center", va="center", fontsize=8
                )
            if target:
                ax.text(
                    (x + xx) / 2, yy + 0.18, "bookcase", ha="center", fontsize=8, color="#8b4d22"
                )
        ...
        lines.set_array(room_trace[:-1, 3])
        ...
        ax.set_title(
            f"{label}\n{d['status'].replace('_', ' ')} · {d['simulation_seconds']:.1f}s",
            fontsize=11,
            pad=15,
        )
        ...
        low.set(
            xlabel="Simulation time / seconds", ylabel="Height / metres", ylim=(0, 4), xlim=(0, 150)
        )
        low.legend(frameon=False, fontsize=8, loc="upper right")
        for a in (ax, low):
            for spine in a.spines.values():
                spine.set_color("#b4b1a7")
            a.tick_params(labelsize=8)
    fig.suptitle(
        "The drone saw the bookcase, but could not finish the approach",
        x=0.06,
        ha="left",
        fontsize=16,
        y=0.99,
    ...
    fig.text(
        0.06,
        0.025,
        "Same room layout, seed 712. Path color indicates flight height (dark: low, yellow: high). Geometry shown here is evaluator-only.",
        fontsize=9,
        color="#55584f",
    )
