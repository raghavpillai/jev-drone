...
from experiments.legacy.active_search import PLANNING, ActiveSearch, mission_scene
...
class DirectPilot(Navigation):
    ...
    def observation(self, sim, target):
        ...
        if tolerance is not None:
            common["goal"]["altitude_within_tolerance"] = (
                abs(target[2] - sim.position[2]) <= tolerance
            )
        ...
        motion = "stopped" if speed <= 0.15 else (dominant if along / speed >= 0.98 else "diagonal")
        goal_delta = subtract(target, sim.position)
        remaining_along = sum(a * b for a, b in zip(goal_delta, LOOK[dominant]))
        common["flight_status"] = {
            "stopped": speed <= 0.15,
            "speed_mps": round(speed, 2),
            "slow_speed_ready": speed <= 0.35,
            "current_motion_direction": motion,
            "position_reached": common["goal"]["inside_arrival_radius"],
            "current_axis_still_needs_travel": remaining_along > 0.35
            if motion not in ("stopped", "diagonal")
            else None,
            "remaining_along_current_motion_m": round(remaining_along, 2)
            if motion not in ("stopped", "diagonal")
            else None,
    ...
    def questions(self):
        questions = {
            "movement": {
                "type": "choice",
                "instructions": PILOT + MOTION_RULES,
                "criteria": {
                    **{
                        name: f"Move {name} toward an unresolved goal coordinate or for a deliberate short obstacle detour. Requires position_reached false and {name} depth OPEN. If still moving, requires that you are already moving {name} AND current_axis_still_needs_travel is true; otherwise brake/look first."
                        for name in LOOK
                    },
                    **{
                        "look_"
                        + name: f"Brake and aim camera {name}; prepare to move along a remaining goal axis or inspect this direction on the next request."
                        for name in LOOK
                    },
        ...
        if self.allow_slow:
            ...
            questions["movement"]["criteria"].update(
                {
                    "slow_"
                    + name: f"Move slowly {name} at 0.3m/s for a small correction or short clearance. Requires measured {name} clearance >=0.45m, slow_speed_ready true, waypoint not reached, and stopped or already moving {name}. UNKNOWN or <0.45m forbids this."
                    for name in LOOK
                }
            )
        ...
        for name in LOOK:
            for action in (name, "slow_" + name):
                if action in questions["movement"]["criteria"]:
                    questions["movement"]["criteria"][action] += (
                        f" The camera MUST currently look {name}; if it looks elsewhere, choose look_{name} first."
                    )
        return questions
    ...
    def decode(self, result):
        ...
        result.update(
            ...
            action="brake" if selected.startswith("look_") else selected.removeprefix("slow_"),
            speed=0.3 if selected.startswith("slow_") else 1.0,
            look=selected.removeprefix("look_") if selected.startswith("look_") else self.look,
            intent="direct",
        )
        return result
    ...
    def accept(self, sim, decision, accepted, target):
        ...
        self.history[-1]["selected_action"] = (
            decision.get("selected_action", "brake") if accepted else "brake"
        )
...
def varied_scene(args):
    ...
    for obj in scene.home.objects:
        ...
        if variation in ("east_wall", "west_wall") and obj.kind in ("bookcase", "screen"):
            if variation == "east_wall":
                lo[:2], hi[:2] = (
                    ([offset + 5.1, 8.5], [offset + 5.7, 9.9])
                    if obj.kind == "bookcase"
                    else ([offset + 4.1, 6.9], [offset + 4.5, 9.9])
                )
            else:
                lo[:2], hi[:2] = (
                    ([offset + 0.3, 0.3], [offset + 0.9, 1.7])
                    if obj.kind == "bookcase"
                    else ([offset + 1.4, 0.1], [offset + 1.6, 3.1])
                )
        if obj.kind == "screen":
    ...
    world = replace(
        scene.home.world, obstacles=scene.home.world.obstacles[:4] + tuple(o.box for o in objects)
    )
...
"""
    ...
    def options(self):
        ...
        for name, option in list(options.items()):
            ...
            if not name.endswith("_high"):
                high = {
                    **option,
                    "xyz": [option["xyz"][0], option["xyz"][1], 2.8],
                    "description": option["description"]
                    + " Use a high view at 2.8m to look over low occluders.",
                }
                ...
                options[name + "_south"] = {
                    **option,
                    "look": "right",
                    "description": f"Look across the south half of {option['room']} from 2.8m; stop and inspect right after arrival.",
                }
    ...
    def at_task_position(self):
        return norm(subtract(self.task["xyz"], self.sim.position)) <= 0.65 and abs(
            self.task["xyz"][2] - self.sim.position[2]
        ) <= self.task.get("altitude_tolerance", float("inf"))

    def task_outcome(self):
        task = self.task
        if task["kind"] == "go" and task.get("look"):
            arrived = self.at_task_position() and norm(self.sim.velocity) <= 0.15
            if (
                arrived
                and self.pilot.look == task["look"]
                and self.sim.time - task["started_at"] >= 0.25
            ):
    ...
    def run(self):
        result = super().run()
        result.update(
            experiment="purposeful-v4",
            variation=getattr(self.args, "variation", "original"),
            pilot=getattr(self.args, "pilot", "direct"),
            completed_view_tasks=dict(self.view_completions),
        )
        return result
