...
def main():
    ...
    fig.subplots_adjust(top=.79, bottom=.16, wspace=.3)
    artists = []
    for ax, vertical in [(top, 1), (side, 2)]:
        for box in world["obstacles"]:
            lo, hi = box["lo"], box["hi"]
            ax.add_patch(Rectangle((lo[0], lo[vertical]), hi[0]-lo[0], hi[vertical]-lo[vertical], color="#d8dce0"))
        ax.scatter(world["start"][0], world["start"][vertical], s=35, color="#24384d", label="Start")
        ax.scatter(world["goal"][0], world["goal"][vertical], marker="*", s=160, color="#1970b5", label="Goal")
        ax.set(xlim=(0, 12), ylim=(0, world["size"][vertical]), xlabel="x / metres",
        ...
        drone = Rectangle((0, 0), .4, .4, facecolor=color, edgecolor="white", zorder=5)
        ax.add_patch(drone)
        artists.append((path, drone, vertical))
    title = fig.suptitle("", fontsize=14, x=.08, ha="left")
    caption = fig.text(.08, .055, "", fontsize=10)
    frame_times = [i*args.speed/15 for i in range(int(times[-1]*15/args.speed)+1)] + [times[-1]]*15
    ...
    def update(t):
        ...
        for path, drone, axis in artists:
            ...
            drone.set_xy((point[1]-.2, point[axis+1]-.2))
        ...
        if decision:
            action = decision["action"] if t-decision["received_at"] < episode["physics"]["command_lease_s"] else "brake (expired command)"
        status = episode["status"] if t >= times[-1] else "running"
        title.set_text(f"Jev live-call replay / {world['name']} / seed {world['seed']}\n{status.upper()} · simulation {min(t, times[-1]):.1f}s · playback {args.speed:g}×")
