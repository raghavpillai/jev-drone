...
def files():
    return sorted(Path("results").glob("active-search-v2-*/episode.json")) + sorted(
        Path("results/active-search-v2-suite").glob("*-*-*.json")
    )
...
def summarize(path):
    ...
    return {
        ...
        **{
        ...
        "control_latency_median_ms": round(statistics.median(latencies) * 1000, 1)
        if latencies
        else None,
        "control_latency_p95_ms": round(
            latencies[min(len(latencies) - 1, int(len(latencies) * 0.95))] * 1000, 1
        )
        if latencies
        else None,
        "stale_control_responses": sum(c["latency_seconds"] > 0.8 for c in control),
        ...
        "zero_gain_repeat_views": sum(
            v["new_visible_samples"] == 0 and v["repeat_view"] for v in data["views"]
        ),
...
def plot(results):
    ...
    for row, seed in enumerate(seeds):
        for col, mode in enumerate(("checklist", "coverage")):
            ax = axes[row, col]
            match = next(
                (r for r in results if r["seed"] == seed and r["memory_mode"] == mode), None
            )
            if match is None:
            ...
            for box in d["world"]["obstacles"]:
                ...
                ax.add_patch(
                    Rectangle(
                        (x, y), xx - x, yy - y, facecolor=color, edgecolor="#b5b7ae", alpha=0.9
                    )
                )
            target = targets.get(d["ground_truth"]["target_id"])
            if target:
                x, y, _ = target["box"]["lo"]
                xx, yy, _ = target["box"]["hi"]
                ax.add_patch(
                    Rectangle(
                        (x, y),
                        xx - x,
            ...
            for i, task in enumerate(d["tasks"]):
                if task["choice"].startswith(("look_", "inspect_")):
                    x, y, _ = task["position"]
                    ax.scatter(x, y, s=75, facecolors="none", edgecolors="#d3993c", zorder=5)
            ...
            condition = {"absent": " · target absent", "occluded": " · partition"}.get(
                d["case"], ""
            )
            ax.set_title(
                f"{label} · {d['item']} in {d['room'].replace('_', ' ')}{condition}\n{d['status'].replace('_', ' ')} · {d['simulation_seconds']:.1f}s · {d['path_metres']:.1f}m",
                loc="left",
                fontsize=10,
                pad=17,
            )
            ...
            for spine in ax.spines.values():
                spine.set_color("#b5b7ae")
    fig.suptitle(
        "Jev searches unfamiliar rooms: matched memory comparisons",
        fontsize=17,
        x=0.07,
        ha="left",
        y=0.995,
    ...
    fig.text(
        0.07,
        0.009,
        "Green: flight path. Gold rings: explicit look/inspection tasks. Orange: evaluator-only target. Red ×: final pose. Top view; vertical motion is simulated.",
        fontsize=9,
        color="#555c56",
    )
    fig.tight_layout(rect=[0, 0.03, 1, 0.975], h_pad=2)
