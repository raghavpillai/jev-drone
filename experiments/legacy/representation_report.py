...
def main():
    ...
    for folder in sorted(Path("results").glob("representation-*")):
        for path in sorted(folder.glob("*.json")):
            ...
            if "status" not in d:
                continue
            calls = d["decisions"]
            row = {k: d[k] for k in ("variant", "case", "seed", "degraded", "status", "calls", "cost_usd", "simulation_seconds")}
            row.update(source=str(path), accepted_hz=sum(c["accepted"] for c in calls)/d["simulation_seconds"],
                       latency_median_ms=1000*statistics.median(c["latency_seconds"] for c in calls),
                       errors=sum(bool(c.get("error")) for c in calls),
                       stale=sum(c["latency_seconds"] > .8 for c in calls),
    ...
    for variant, items in groups.items():
        summary[variant] = {"episodes": len(items), "success": sum(r["status"] == "success" for r in items),
    ...
    (output/"representation-results.json").write_text(json.dumps({"summary": summary, "episodes": rows}, indent=2))
    ...
    for variant in variants:
        ...
        for seed, degraded in conditions:
            ...
            labels.append(f"{wins}/{len(items)} completed\n{crashes} collisions" if items else "Not tested")
            backgrounds.append("#eeeeee" if not items else colors["success"] if wins == len(items) else colors["timeout"] if wins else colors["collision"])
        cells.append(labels)
        shades.append(backgrounds)
    ax.axis("off")
    table = ax.table(cellText=cells, cellColours=shades, rowLabels=variants,
                     colLabels=["Original · clean", "Original · degraded", "Changed · clean", "Changed · degraded"], loc="center")
    table.auto_set_font_size(False)
    table.set_fontsize(11)
    table.scale(1, 2.9)
    ax.set_title("Jev local control: same observations, different representations", fontsize=15, pad=24)
    fig.text(.5, .025, "Four courses per cell · 40 s limit · actual API delay · degraded = missing + noisy + delayed ranges", ha="center", fontsize=10)
    fig.tight_layout(rect=(.07, .07, 1, 1))
