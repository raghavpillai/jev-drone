...
from experiments.legacy.policy import Jev, MODEL, load_key
from experiments.legacy.search import deepseek_ask, depth_observation
from experiments.legacy.search_world import evaluate_report, ground_truth, room_at, update_memory, visible_objects
from experiments.legacy.sim import Simulation, norm, subtract
from experiments.legacy.task_planning import NAV_INSTRUCTIONS, TASK_INSTRUCTIONS, WAYPOINT_INSTRUCTIONS, all_checked, planning_state, room_coverage, task_options, waypoint_options
...
class Experiment:
    def __init__(self, args, local):
        ...
        self.best, self.last_progress = float("inf"), 0.
    ...
    def request(self, role, state, instructions, criteria):
        ...
        try:
            if role == "planner" and self.args.planner == "deepseek":
                result = deepseek_ask(self.local.client,state,instructions,criteria,max_tokens=4096)
            else:
                result = ask(self.local.client,state,instructions,criteria)
        except Exception as exc:
            result = {"error":type(exc).__name__,"latency_seconds":time.perf_counter()-started,"state":state}
        self.calls.append({"role":role,"sent_at":sent,**result})
        self.sim.advance(min(result["latency_seconds"],max(0.,self.args.seconds-self.sim.time)))
        if result.get("error"):
    ...
    def observe(self):
        ...
        changed = False
        if new_ids:
            state = {"mission":self.scene.mission,
                     "objects":[{"id":k,"appearance":v["appearance"],"room":v["room"]} for k,v in self.memory.items()]}
            match = self.request("recognition",state,
                ...
                {"none":"No observed match",**{k:v["appearance"] for k,v in self.memory.items()}})
    ...
    def state(self):
        return planning_state(self.scene,self.sim,self.memory,self.visible(),
                              self.checked,self.recognized,self.report)
    ...
    def choose_task(self):
        ...
        print(json.dumps({"case":self.args.case,"architecture":self.args.architecture,"planner":self.args.planner,
                          "time":round(self.sim.time,2),"task":self.task_name}),flush=True)

    def choose_waypoint(self):
        state = self.state()
        points,candidates,criteria = waypoint_options(self.scene,self.sim,self.memory,self.checked,self.attempts,self.task)
        state.update({"assigned_task":self.task,"candidate_waypoints":candidates,
    ...
    def move(self):
        ...
        self.sim.advance(min(latency,max(0.,self.args.seconds-self.sim.time)))
        accepted = self.active() and latency <= .8 and not decision.get("error")
        if self.sim.status == "running":
            self.sim.command(decision["action"] if accepted else "brake")
        if accepted:
            self.intent = decision["intent"]
        self.calls.append({"role":"control","sent_at":sent,"received_at":self.sim.time,
                           "accepted":accepted,"waypoint":self.target_name,"state":observation,**decision})
        self.history.append({"position":observation["position_metres_xyz"],
            "goal_direction":observation["goal"]["direction"],"goal_distance":observation["goal"]["distance_metres"],
            "action":decision["action"] if accepted else "brake"})
        self.sim.advance(min(max(0.,.2-latency),max(0.,self.args.seconds-self.sim.time)))
    ...
    def run(self):
        try:
            while self.active() and len(self.calls) < 1100:
                ...
                if not self.active():
                    break
                arrived = self.target is not None and norm(subtract(self.target,self.sim.position)) <= .65 and norm(self.sim.velocity) <= .15
                stalled = self.target is not None and self.sim.time-self.last_progress > 8.
                self.report = {"status":"target_discovered" if discovery else "arrived" if arrived else "stalled" if stalled else "traveling",
                               "waypoint":self.target_name,"recognized_target":self.recognized}
                if self.args.architecture == "task":
                    ...
                    if self.task:
                        if self.task["kind"] == "search_room":
                            ...
                            complete = arrived
                    if complete:
                        self.report["status"] = "views_checked" if self.task["kind"] == "search_room" else "approach_complete"
                        self.task_events.append({"time":self.sim.time,"task":self.task,**self.report})
                    if self.task is None or discovery or complete:
                        self.choose_task()
                    if self.active() and self.task is not None and (self.target is None or arrived or stalled):
                ...
                if self.active() and self.target is not None:
                    self.move()
            if self.sim.status == "running":
                self.sim.status = "timeout" if self.sim.time >= self.args.seconds-1e-9 else "call_limit"
        except TimeoutError:
            if self.sim.status == "running":
        ...
        result.update({"architecture":self.args.architecture,"planner":self.args.planner,"case":self.args.case,
            "seed":self.args.seed,"mission":self.scene.mission,"ground_truth":ground_truth(self.scene.home),
            "portals":self.scene.portals,"candidate_points":self.scene.points,"calls":self.calls,
            "role_counts":dict(Counter(c["role"] for c in self.calls)),"task_events":self.task_events,
            "observed_objects":self.memory,"checked_views":sorted(self.checked),
            "coverage":room_coverage(self.scene,self.checked),"cost_usd":self.cost(),"error":self.error,
            "movement_model_request":MODEL,"movement_models_returned":sorted({c.get("model","") for c in self.calls if c["role"]=="control" and c.get("model")}),
            "target_first_seen_seconds":self.memory.get(self.scene.home.target_id,{}).get("first_seen_at")})
        return result
...
def main():
    ...
    (args.out/"metadata.json").write_text(json.dumps({k:str(v) if isinstance(v,Path) else v for k,v in vars(args).items() if k!="key_file"},indent=2))
    ...
    print(json.dumps({k:result[k] for k in ["architecture","planner","case","seed","status","simulation_seconds","role_counts","cost_usd","error"]}),flush=True)
