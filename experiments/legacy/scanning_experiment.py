...
class ScanningNavigation(InspectionNavigation):
    def questions(self):
        ...
        movement["criteria"] = {
            **{
                name: f"Move {name} at 1 m/s. Only choose if the named {name} clearance in the current observation is OPEN, never UNKNOWN/BLOCKED. Camera stays aimed as before."
                for name in LOOK
            },
            **{
                "look_"
                + name: f"Brake to stop and aim the camera {name}. Observe that unknown direction or inspect the target/search viewpoint; do not translate."
                for name in LOOK
            },
    ...
    def decode(self, result):
        ...
        result["look"] = (
            selected.removeprefix("look_") if selected.startswith("look_") else self.look
        )
