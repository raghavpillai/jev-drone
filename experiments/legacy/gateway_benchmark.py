...
def requests():
    ...
    questions={
        "movement":{"type":"choice","instructions":INSTRUCTIONS+GUIDANCE,"criteria":{k:v for k,v in CRITERIA.items() if "_" not in k}},
...
def summarize(calls):
    ...
    for provider in sorted({c["provider"] for c in calls}):
        ...
        summary[provider]={"measured_requests":len(measured),"valid":len(valid),
            "errors":len(measured)-len(valid),"over_800ms":sum(c["latency_seconds"]>.8 for c in measured),
            "cold_ms":1000*cold["latency_seconds"] if cold else None,
            "median_ms":1000*statistics.median(latency) if latency else None,
            "p95_ms":1000*latency[min(len(latency)-1,int(.95*len(latency)))] if latency else None,
            "serial_requests_per_second":len(measured)/sum(c["latency_seconds"] for c in measured) if measured else None,
    ...
    return {"providers":summary,"paired_valid":len(complete),
            "movement_agreement":sum(len(set(p.values()))==1 for p in complete)/len(complete) if complete else None}


def main():
    parser=argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--providers",nargs="+",choices=["openrouter","vercel"],default=["openrouter","vercel"])
    ...
    try:
        for i in range(args.rounds+2):
            ...
            for provider in order:
                ...
                (args.out/"benchmark.json").write_text(json.dumps({"unavailable":unavailable,"calls":calls,**summarize(calls)},indent=2))
    finally:
        for gateway in gateways.values(): gateway.close()
