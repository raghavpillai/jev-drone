...
APPROACH_STANDOFF = CONFIRMATION_DISTANCE - .65 - .1
...
class FocusedSearch(PurposefulSearch):
    api_error_limit = 3
    planning_instructions=PurposefulSearch.planning_instructions.replace('2.15',str(CONFIRMATION_DISTANCE))+"""
...
"""
    ...
    def options(self):
        ...
        for name,option in list(options.items()):
            if name.startswith('go_search_') and option['xyz'][2]>2:
                ...
                option['altitude_tolerance']=.25
                option['description']=option['description'].replace('2.8','3.0')+' Reach within 0.25m of the requested altitude before inspecting.'
            ...
            if option['kind']=='approach':
                ...
                if getattr(self.args,'high_approach',True):
                    ...
                    options[high_name]={**option,'xyz':[*option['xyz'][:2],3.0],'altitude_tolerance':.25,
                        'description':'Approach this observed object from above at 3.0m. Useful when the direct standoff is behind a lower occluder. '+self.memory[option['object']]['appearance'],
                        'failed_attempts':sum(t['outcome'].startswith('stalled') for t in attempts),
    ...
    def confirmation_facts(self,obj):
        ...
        return {'distance_to_observed_surface':round(distance,2),'visible_now':visible,
                'within_confirmation_distance':distance<=CONFIRMATION_DISTANCE,'stopped':stopped,
                'physical_confirmation_ready':visible and distance<=CONFIRMATION_DISTANCE and stopped}
    ...
    def stopping_state(self,options):
        return {'mission':self.scene.mission,'current_room':room_at(self.sim.position),
            'seconds_remaining':round(self.args.seconds-self.sim.time,1),'speed_mps':round(norm(self.sim.velocity),2),
            'final_budget_review':self.args.seconds-self.sim.time<=8,
            'confirmation_distance_m':CONFIRMATION_DISTANCE,
            'objects':[{'id':o['id'],'appearance':o['appearance'],'room':o['room'],
                        **self.confirmation_facts(o)}
                       for o in self.memory.values() if o['room'] in self.scene.scope],
            'remaining_view_goals':[name for name,o in options.items() if name.startswith('go_search_') and not o['completed_views'] and not o['failed_attempts']],
            'view_outcomes':[{k:o[k] for k in ('description','completed_views','failed_attempts')} for name,o in options.items() if name.startswith('go_search_')],
    ...
    def select_task(self,options):
        ...
        for name in terminal:
            if name.startswith('report_found_'):
                ...
                terminal[name]+=f' Physical confirmation ready: {ready}. ONLY finish if True AND this is the requested object. If False, continue to approach or inspect first.'
        decision=self.request_choice('mission',self.stopping_state(options),STOPPING+CONFIRMATION_RULES,
                                     {'continue_search':'Continue: choose a useful navigation, viewing, inspection or approach task.',**terminal})
        if decision!='continue_search' or self.sim.status!='running' or self.sim.time>=self.args.seconds:
            return decision
        tasks={name:o for name,o in options.items() if o['kind']!='report'}
        return self.request_choice('planner',self.state(tasks),self.planning_instructions,
