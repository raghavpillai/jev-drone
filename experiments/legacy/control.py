...
MOVES = {
    f"{direction}_{pace}": (direction, speed)
    for direction in ("forward", "back", "left", "right", "up", "down")
    for pace, speed in (("cruise", 1.0), ("slow", 0.4))
}
MOVES["brake"] = ("brake", 0.0)
...
def control_state(sim, target, history, task):
    ...
    predicted = tuple(p + v * (0.35 + stop_time_half) for p, v in zip(sim.position, velocity))
    ...
    return {
        ...
        "aligned_axes": [axis for axis, value in zip("xyz", delta) if abs(value) <= 0.25],
        "clearance_metres": ranges,
        "recent": history[-4:],
    }
...
def choose_control(client, sim, target, history, task, slow=True):
    ...
    criteria = {
        name: f"Set {direction} velocity to {speed} m/s; other axes slow toward zero."
        for name, (direction, speed) in moves.items()
    }
    criteria["brake"] = (
        "Decelerate all movement to a hover. Use for arrival, braking before overshoot, or no clear movement."
    )
