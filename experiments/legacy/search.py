...
def planner_state(
    mission, sim, memory, visible, visited, attempts, target_name, report, recognized
):
    points = search_waypoints()
    for object_id, item in memory.items():
        points[f"approach_{object_id}"] = {
            "xyz": item["approach_xyz"],
            "room": item["room"],
    ...
    candidates = {
        name: {
            **p,
            "distance_metres": round(norm(subtract(p["xyz"], sim.position)), 2),
            "completed_visits": visited.count(name),
    ...
    state = {
        ...
        "visible_now": visible,
        "observed_objects": list(memory.values()),
        "recognized_target": recognized,
        "candidate_waypoints": candidates,
        "completed_waypoints": visited,
        "previous_target": target_name,
        "local_report": report,
    }
    criteria = {name: p["description"] for name, p in points.items()}
    for object_id in memory:
        criteria[f"report_found_{object_id}"] = (
            f"Finish: {object_id} matches the mission, is currently visible within 2.1 m, and the drone is stopped."
        )
    criteria["report_not_found"] = (
        "Finish: relevant rooms were thoroughly searched and the requested object was not found."
    )
...
def depth_observation(sim, history, target, intent):
    ...
    for x, y, z in itertools.product([-1, 0, 1], repeat=3):
        ...
        parts = [
            ...
            for v, positive, negative in zip(
                (x, y, z), ("forward", "left", "up"), ("back", "right", "down")
            )
            if v
        ]
...
def episode(args, local):
    ...
    best, last_progress, last_plan = float("inf"), 0.0, 0.0
    error = None
    ...
    def global_call(kind, state, instructions, criteria):
        ...
        sim.advance(min(decision["latency_seconds"], max(0.0, args.seconds - sim.time)))
        ...
        while (
            ...
            if new_ids:
                recognized = global_call(
                    ...
                    {
                        "none": "No observed object matches the request.",
                        **{k: v["appearance"] for k, v in memory.items()},
                    },
                )
            ...
            stalled = target is not None and sim.time - last_progress > 7.0
            if target is None or arrived or stalled or new_ids or sim.time - last_plan > 10.0:
                ...
                report = (
                    ...
                    if stalled
                    else "new objects observed"
                    if new_ids
                    else "progress update"
                )
                visible = visible_objects(home, sim.position)
                state, criteria, points = planner_state(
                    args.mission,
                    sim,
                    memory,
                    visible,
                ...
                if choice.startswith("report_"):
                    sim.status = evaluate_report(
                        choice, home, sim, visible_objects(home, sim.position)
                    )
                    break
                ...
                best, last_progress, last_plan = (
                    norm(subtract(target, sim.position)),
                    sim.time,
                    sim.time,
                )
            if sim.status != "running" or sim.time >= args.seconds - 1e-9:
    ...
    result.update(
        {
            "mission": args.mission,
            "planner": args.planner,
            "ground_truth": ground_truth(home),
            "plans": plans,
            "decisions": decisions,
            "observed_objects": memory,
            "visited_waypoints": visits,
            "recognized_target": recognized,
            "target_seen": home.target_id in memory,
            "target_identified": home.target_id is not None and recognized == home.target_id,
            "calls": len(plans) + len(decisions),
            "cost_usd": cost(),
            "error": error,
            "local_accepted_hz": sum(d["accepted"] for d in decisions) / max(sim.time, 0.001),
...
def main():
    ...
    parser.add_argument(
        ...
        choices=list(ROOMS),
        default="bedroom",
        help="Evaluator-only hidden placement",
    )
    parser.add_argument("--absent", action="store_true", help="Evaluator-only target absence")
    parser.add_argument(
        "--mission",
        default="Find a bookcase somewhere in this home. Approach it, stop nearby, and report it found.",
    )
