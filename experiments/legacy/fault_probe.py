...
class FaultGateway(JevGateway):
    ...
    def evaluate(self, state, questions):
        ...
        if inject:
            ...
            time.sleep(3.0)
            result = {
                "provider": "injected_transport_fault",
...
def main():
    ...
    prepare_output(
        args.out,
        {"job": job, "test": "two consecutive synthetic 3s transport timeouts; first while moving"},
    )
    gateway = FaultGateway(
        "openrouter", load_credential("openrouter", args.key_file), journal=args.out / "calls.jsonl"
    )
    try:
