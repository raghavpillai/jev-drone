...
def run_one(key, out, job):
    args = SimpleNamespace(**({"seconds": 150.0, "budget": 0.2} | job))
    gateway = JevGateway("openrouter", key, journal=out / (job["id"] + ".calls.jsonl"))
    try:
        experiment_class = {
            "purposeful": PurposefulSearch,
            "focused": FocusedSearch,
            "baseline": ActiveSearch,
        }[job.get("engine", "purposeful")]
    ...
    return {
        ...
        **{
            k: result[k]
            for k in ("status", "simulation_seconds", "role_counts", "cost_usd", "error")
        },
...
def main():
    ...
    prepare_output(
        args.out, {"jobs": jobs, "seconds": 150.0, "budget": 0.2, "workers": args.workers}
    )
