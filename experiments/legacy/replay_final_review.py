...
def main():
    ...
    prepare_output(
        args.out,
        {"source": str(args.episode), "kind": "checkpoint regression", "repeats": args.repeats},
    )
    ...
    for repeat in range(args.repeats):
        ...
        try:
            config = SimpleNamespace(**({"seconds": 150.0, "budget": 0.2} | job))
            experiment = FocusedSearch(config, gateway)
            sim = experiment.sim
            t = review["sent_at"]
            row = max((r for r in data["trace"] if r[0] <= t), key=lambda r: r[0])
            sim.time = row[0]
            sim.position = tuple(row[1:4])
            sim.velocity = tuple(row[4:7])
            previous = next(
                c
                for c in reversed(data["calls"][:-1])
                if c["role"] == "control" and c.get("accepted")
            )
            sim.command(previous["action"], speed=previous.get("speed"))
            sim.command_expires = (
                previous["sent_at"] + previous["latency_seconds"] + sim.command_lease
            )
            sim.advance(t - sim.time)
            sim.trace = []
            sim.record()
            experiment.memory = data["objects_observed"].copy()
            experiment.sightings = Counter(
                {
                    oid: 2 if "unclear from this view" in o["appearance"] else 3
                    for oid, o in experiment.memory.items()
                }
            )
            ...
            experiment.view_completions = Counter(
                t["choice"] for t in experiment.tasks if t["outcome"] == "viewpoint_inspected"
            )
            for view in data["views"]:
                if view["time"] <= t:
                    experiment.attention.place_looks[tuple(round(v) for v in view["position"])].add(
                        view["look"]
                    )
            look = data["views"][-1]["look"]
            experiment.pilot.look = look
            experiment.pilot.look_history = [(0.0, look)]
            experiment.deadline_reviewed = True
            result = experiment.run()
        finally:
            gateway.close()
        result.update(
            trial_id=f"final-review-checkpoint-{repeat}",
            checkpoint_time=t,
        ...
        print(
            json.dumps(
                {k: result[k] for k in ("trial_id", "status", "continuation_seconds", "cost_usd")}
            ),
            flush=True,
        )
