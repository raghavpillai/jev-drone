...
def main():
    ...
    components = (
        ("jev_drone",)
        if (source / "jev_drone").is_dir()
        else ("px4sim", "realism", "jev_gateway.py")
    )
    module = "jev_drone.sim.run" if components == ("jev_drone",) else "px4sim.run"
    for name in components:
        if not (source / name).exists():
            raise ValueError("Missing reference source " + name)
    credentials = (
        ["-e", "OPENROUTER_API_KEY"]
        if os.environ.get("OPENROUTER_API_KEY")
        else [
            "-v",
            f"{Path(os.environ.get('JEV_KEY_FILE', root / '.env')).resolve()}:/run/secrets/openrouter.env:ro",
        ]
    )
    ...
    def run(job):
        ...
        command += [
            ...
            "--budget",
            str(protocol["budget_usd_per_trial"]),
        ]
