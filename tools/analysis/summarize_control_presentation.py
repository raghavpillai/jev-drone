...
def main():
    ...
    for name in PROBES:
        ...
        for variant in dict.fromkeys(r["variant"] for r in rows):
            ...
            variants[variant] = {
                "requests": len(chosen),
                "errors": sum(bool(r["error"]) for r in chosen),
                "unsafe_choices": sum(bool(r["violations"]) for r in chosen),
                "choices": dict(Counter(r["choice"] for r in chosen)),
                "latency_median_ms": statistics.median(c["latency_seconds"] for c in requests)
                * 1000,
                "input_chars_median": statistics.median(
                    len(json.dumps({"state": c["state"], "questions": c["questions"]}))
                    for c in requests
                ),
            ...
            for kind in dict.fromkeys(r["kind"] for r in chosen):
                ...
                by_kind[kind] = {
                    ...
                    "unsafe_choices": sum(bool(r["violations"]) for r in selected),
    ...
    data = {
        "scope": "Selected archived states; repeat counts are frozen in each protocol. Correlated development probes, not independent flights or safety certification.",
        "total_requests": sum(v["requests"] for s in summaries for v in s["variants"].values()),
        "total_cost_usd": sum(v["cost_usd"] for s in summaries for v in s["variants"].values()),
        "probes": summaries,
    }
    (output / "probe-analysis.json").write_text(json.dumps(data, indent=2) + "\n")
    lines = [
        "# Jev control-presentation probes",
        "",
        data["scope"],
        "",
    ...
    for s in summaries:
        for name, v in s["variants"].items():
            lines.append(
                f"| [{s['probe']}](../../{s['directory']}/results.json) | {name} | {v['requests']} | {v['unsafe_choices']} | {v['errors']} | {v['latency_median_ms']:.0f} |"
            )
    lines += [
        "",
        f"{data['total_requests']} requests; reported API cost ${data['total_cost_usd']:.6f}.",
        "",
    ...
    print(
        json.dumps(
            {"requests": data["total_requests"], "cost_usd": data["total_cost_usd"]}, indent=2
        )
    )
