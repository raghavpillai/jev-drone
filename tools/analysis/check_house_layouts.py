...
def check(case):
    ...
    step, radius, altitude = 0.25, 0.45, 1.3
    ...
    for box in scene.geometry:
        gaps = np.minimum(
            gaps,
            np.linalg.norm(np.maximum(np.abs(points - box["center"]) - box["half"], 0), axis=1),
        )
    ...
    distances, queue = {start: 0.0}, deque([start])
    while queue:
        cell = queue.popleft()
    ...
    for objective in scene.mission[:-1]:
        ...
        for cell, distance in distances.items():
            ...
            if (
                scene.layout.room_at(point) == objective["room"]
        ...
        inspections.append(
            {
                ...
                "room": objective["room"],
                "reachable_inspection_positions": len(candidates),
                "grid_distance_from_launch_m": min(candidates),
            }
        )
    hidden = None
    ...
    return {
        "case": case,
        "seed": 5111,
        "collision_parts": len(scene.geometry),
        "reachable_grid_cells": len(distances),
        "grid_spacing_m": step,
        "body_radius_m": radius,
        "altitude_m": altitude,
        "inspection_positions": inspections,
        "hidden_from_library_entry": hidden,
    }
...
def main():
    ...
    output.write_text(
        json.dumps(
            {
                "kind": "Evaluator truth only; no model calls or native flights",
