...
def main():
    ...
    for mode in ("full", "patch"):
        ...
        controls = [
            c
            for d in raw
            if d["world"]["config"]["control_interface"] == mode
            for c in d["calls"]
            if c["role"] == "control"
        ]
        ...
        aggregate[mode] = {
            "trials": len(selected),
            "completions": sum(r["status"] == "success" for r in selected),
            ...
            "accepted_control_hz": accepted / wall,
            "accepted_controls": accepted,
            "local_requests": len(controls),
            ...
            "channels_per_accepted_control": sum(r["assigned_velocity_channels"] for r in selected)
            / accepted,
            "published_vector_changes": sum(r["published_vector_changes"] for r in selected),
            "published_vector_changes_per_second": sum(
                r["published_vector_changes"] for r in selected
            )
            / wall,
            "api_p50_ms": float(np.median(latency) * 1000),
            "api_p95_ms": float(np.quantile(latency, 0.95) * 1000),
    ...
    for row in rows:
        ...
        patch = next(
            r
            for r in rows
            if (r["case"], r["seed"], r["control_interface"]) == (row["case"], row["seed"], "patch")
        )
        pairs.append(
            {
                "case": row["case"],
                "seed": row["seed"],
                "full": {
                    k: row[k] for k in ("status", "strict_pass", "wall_seconds", "rule_violations")
                },
                "patch": {
                    k: patch[k]
                    for k in ("status", "strict_pass", "wall_seconds", "rule_violations")
                },
                "patch_minus_full_seconds": (
                    patch["wall_seconds"] - row["wall_seconds"]
                    if patch["status"] == row["status"] == "success"
                    else None
                ),
            }
        )
    ...
    (args.out / "comparison.json").write_text(
        json.dumps({"protocols": protocols, "aggregate": aggregate, "pairs": pairs}, indent=2)
    )
    table = [
        "| Scene | Seed | Full commands | Partial updates | Difference |",
        "|---|---:|---|---|---:|",
    ]
    for pair in pairs:
        ...
        def cell(mode):
            r = pair[mode]
            return (
                f"{r['status']}, {r['wall_seconds']:.1f}s, {r['rule_violations']} rule violations"
            )

        difference = (
            f"{pair['patch_minus_full_seconds']:+.1f}s"
            if pair["patch_minus_full_seconds"] is not None
            else "not comparable"
        )
        table.append(
            f"| {pair['case']} | {pair['seed']} | {cell('full')} | {cell('patch')} | {difference} |"
        )
