...
def summarize(path):
    ...
    row.update(
        ...
        valid_reports=[
            {"choice": r.get("choice", r.get("task")), "stage": r.get("stage"), "time": r["time"]}
            for r in result["reports"]
            if r["valid"]
        ],
        task_outcomes=dict(Counter(t["outcome"] for t in result["tasks"])),
        final_estimated_position=calls[-1]["state"]["sensors"]["position_estimate"]
        if calls
        else None,
    )
    manifest = json.loads(path.with_name("manifest.json").read_text())
    row["snapshot_hash"] = hashlib.sha256(
        json.dumps(manifest["sha256"], sort_keys=True).encode()
    ).hexdigest()
    return row
...
def main():
    ...
    for cohort in sorted({r["cohort"] for r in rows}):
        ...
        cohorts[cohort] = {
            "trials": len(batch),
            "strict_passes": sum(r["strict_pass"] for r in batch),
        ...
        failed_attempts.append(
            {
                ...
                "case": config["case"],
                "seed": config["seed"],
                "error": path.read_text().strip().splitlines()[-1],
                "request_attempts": len(calls),
                "changed_source_files": changed,
                "reported_cost_usd": sum(float(c.get("usage", {}).get("cost", 0)) for c in calls),
            }
        )
    summary = {
        "native_trials": len(rows),
        "native_request_attempts": sum(r["calls"] for r in rows),
        "native_valid_responses": sum(r["valid_model_responses"] for r in rows),
        "native_reported_cost_usd": sum(r["cost_usd"] for r in rows),
        "offline": offline,
        "cohorts": cohorts,
        "failed_attempts_without_result": failed_attempts,
        "notes": [
            "Development seeds, courses, and fresh validation are separate populations.",
    ...
    lines = [
        "# Native recovery experiment ledger",
        "",
    ...
    for cohort in cohorts:
        ...
        for r in (r for r in rows if r["cohort"] == cohort):
            ...
            lines.append(
                f"| [{label}]({link}) | {r['status']} | {r['stages']}/{r['mission_stages']} | "
    ...
    if failed_attempts:
        lines += [
            "## Exceptions without a completed flight result",
            "",
        ...
        for failure in failed_attempts:
            lines.append(
                f"- [{failure['directory']}](../../{failure['directory']}/failure.txt): "
                f"{failure['error']} ({failure['request_attempts']} model request attempts)."
            )
        lines.append("")
    total_cost = summary["native_reported_cost_usd"] + sum(
        r["reported_cost_usd"] for r in offline + failed_attempts
    )
    lines += [
        f"Native: {len(rows)} trials, {summary['native_request_attempts']} request attempts; "
        ...
        json.dumps(
            {
                ...
                "cost_usd": total_cost,
                "cohorts": {
                    k: {n: v[n] for n in ("trials", "strict_passes", "audit_problem_trials")}
