...
def measure(path):
    ...
    for index, command in enumerate(commands):
        ...
        if lag > .1 or speed < .15:
            continue
        end_time = next((c["time"] for c in commands[index+1:]
                         if any(c["after"]["body_controls"][a] for a in axes)), result["sim_seconds"])
        stop_index = next((i for i in range(start_index+1, len(trace)-1)
                           if trace[i+1]["time"] <= end_time
                           and np.linalg.norm(trace[i]["velocity"]) <= .05
                           and np.linalg.norm(trace[i+1]["velocity"]) <= .05), None)
        if stop_index is None:
            rows.append({"trial": str(path.parent), "time": command["time"],
                         "initial_speed_mps": speed, "censored_by_next_motion_or_end": True})
            continue
        ...
        rows.append({"trial": str(path.parent), "time": command["time"],
                     "initial_speed_mps": speed, "censored_by_next_motion_or_end": False,
                     ...
                     "effective_deceleration_mps2": speed**2/(2*observed_distance) if observed_distance > 0 else None})
    return rows


def main():
    names = ["px4-recovery-depth-v15/controller-4910", "px4-recovery-cycle-v18/controller-4910",
             "px4-recovery-precision-v21/controller-4910", "px4-recovery-validation-v23/clutter-4931"]
    ...
    summary = {
        "zero_translation_transitions": len(rows), "settled_samples": len(settled),
        "censored_samples": len(rows)-len(settled),
        "speed_range_mps": [min(r["initial_speed_mps"] for r in settled), max(r["initial_speed_mps"] for r in settled)],
        "maximum_observed_displacement_m": max(r["max_along_track_displacement_m"] for r in settled),
        "samples_exceeding_constant_0_5_braking_term": sum(r["max_along_track_displacement_m"] > r["constant_0_5_acceleration_distance_m"] for r in settled),
    ...
    Path("report/px4-recovery/braking.json").write_text(json.dumps({"summary": summary, "samples": rows}, indent=2)+"\n")
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt
    figure, axis = plt.subplots(figsize=(8, 5.5))
    speed = np.linspace(.14, .29, 100)
    axis.plot(speed, speed**2, label="Constant 0.5 m/s² term", color="#b6573b")
    axis.plot(speed, speed**2+.7*speed, label="Term + 0.7 s settling allowance", color="#246b8e")
    axis.scatter([r["initial_speed_mps"] for r in settled],
                 [r["max_along_track_displacement_m"] for r in settled],
                 label="27 measured native stops", color="#222222", s=30, alpha=.7)
    axis.set(xlabel="Speed immediately before zero-translation command (m/s)",
    ...
    figure.text(.1, .015, "Reaction and depth margins omitted. Static slow-flight samples; one censored stop excluded.", fontsize=9)
