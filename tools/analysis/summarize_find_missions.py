...
def summarize(path):
    ...
    row.update(
        ...
        first_seen=seen,
        first_seen_in_target_room=seen_in_room,
        inspected=inspected,
        ...
        stationary_seconds=float(
            sum(dt for t, dt in zip(trace, durations) if np.linalg.norm(t["velocity"]) < 0.05)
        ),
        selected_replans=sum(
            bool(c.get("accepted")) and c.get("choice") == "brake_and_replan"
...
def rank(row):
    return (
        not row["strict_pass"],
        row["inspected"] is None,
        row["contacts"],
        row["rule_violations"],
        row["false_reports"],
        row["inspected"] if row["inspected"] is not None else float("inf"),
        row["path_metres"],
    )
...
def main():
    ...
    paths = [
        ROOT / f"{trial['case']}-{trial['seed']}" / "result.json" for trial in protocol["trials"]
    ]
    ...
    payload = {
        "selection_rule": protocol["description"],
        "trials": rows,
    ...
    lines = [
        "# Find-only demonstration missions",
        "",
        payload["scope"],
        "",
    ...
    for row in rows:
        ...
        lines.append(
            f"| {row['id']} | {'Strict pass' if row['strict_pass'] else row['status']} | {found} | "
            f"{row['contacts']} / {row['rule_violations']} | {row['path_metres']:.1f} |"
        )
    lines += [
        "",
        f"Selected: **{selected['id']}**. Both outcomes and independent audits are preserved in comparison.json.",
        "",
