...
def analyze(result, journal):
    ...
    for task in result["tasks"]:
        ...
        rows.append(
            {
                "task": task["name"],
                "mission_stage": task.get("mission_stage", 0),
                "start": task["time"],
                "end": end,
                "duration": end - task["time"],
                "outcome": task["outcome"],
                "controls": len(samples),
                "pan_direction_changes": sum(a != b for a, b in zip(pans, pans[1:])),
                "phase_samples": dict(
                    Counter(s.get("doorway_phase", {}).get("phase", "other") for _, s in samples)
                ),
            }
        )
    return {
        "scope": "Recorded task durations and request samples. Pan sign changes include legitimate "
