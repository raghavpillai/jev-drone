...
def analyze(path, out):
    ...
    room = objective["room"]
    lo, hi = map(np.array, result["world"]["rooms"][room])
    ...
    grid = np.array(
        [
            ...
            for y in np.arange(lo[1] + 0.25, hi[1], 0.25)
            for x in np.arange(lo[0] + 0.25, hi[0], 0.25)
        ]
    )
    solid = np.zeros(len(grid), dtype=bool)
    for box in result["world"]["geometry"]:
        solid |= (np.abs(grid - box["center"]) <= np.array(box["half"]) + 0.04).all(axis=1)
    samples = grid[~solid]
    first = np.full(len(samples), np.nan)
    frames = 0
    headings = []
    timeline = []
    for file in sorted(path.with_name("result-frames").glob("*.npz")):
        nominal = float(file.stem)
        if not start - 0.65 <= nominal <= end:
            continue
        with np.load(file) as data:
            ...
            stamp = float(data["times"][i])
            if (
                not start - 0.65 <= stamp <= end
                or not ((origin[:2] >= lo[:2]) & (origin[:2] <= hi[:2])).all()
            ):
                continue
            ...
            fresh = visible & ~np.isfinite(first)
            first[fresh] = stamp
            frames += 1
            timeline.append(
                {
                    "time": stamp,
                    "new_samples": int(fresh.sum()),
                    "seen_samples": int(np.isfinite(first).sum()),
                }
            )
            headings.append((stamp, origin.copy(), data["forward"][i].copy()))
    metrics = []
    fig, axes = plt.subplots(1, 3, figsize=(15, 5.6), sharex=True, sharey=True)
    for ax, z in zip(axes, (0.6, 1.6, 2.4)):
        mask = samples[:, 2] == z
        known = mask & np.isfinite(first)
        unknown = mask & ~np.isfinite(first)
        for box in result["world"]["geometry"]:
            center, half = np.array(box["center"]), np.array(box["half"])
            if abs(center[2] - z) > half[2] or box["name"] in ("floor", "ceiling"):
                continue
            ax.add_patch(
                Rectangle(
                    center[:2] - half[:2], 2 * half[0], 2 * half[1], facecolor="#bfc2c5", alpha=0.6
                )
            )
        ...
        points = ax.scatter(
            *samples[known, :2].T,
            ...
            cmap="viridis",
            vmin=0,
            vmax=max(end - start, 1),
        )
        nearby = [h for h in headings if abs(h[1][2] - z) < 0.6]
        if nearby:
        ...
        for threshold in (80, 90, 95, 99):
            ...
            milestones[f"seconds_to_{threshold}pct"] = (
                max(0.0, float(observed_times[required - 1] - start))
    ...
    fig.colorbar(
        ...
        shrink=0.6,
        label="Seconds after room entry when first seen",
        pad=0.02,
    )
    fig.suptitle(
        f"{path.parent.parent.name} / {result['id']}\n{room} search phase · {end - start:.1f} s · {frames} saved front frames"
    )
    fig.text(
        0.03,
        0.04,
        "Gray squares remain unobserved in saved front depth; orange arrows show camera poses. This is not object-recognition coverage or proof of absence.",
        fontsize=9,
    )
    ...
    summary = {
        ...
        "start": start,
        "end": end,
        "saved_front_frames": frames,
        "planes": metrics,
        "timeline": timeline,
        "method": "0.25 m grid; solid cells excluded using evaluator geometry. A point is visible only inside a saved FRONT frame with >=3 finite pixels in its 3x3 patch and 20th percentile depth >= axial range + .035 m. Sparse saved frames are a visibility sample, not recognition evidence or proof of absence.",
    }
