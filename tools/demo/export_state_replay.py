...
def export(source, output):
        ...
        decisions.append(
            {
                ...
                "request_time": call["time"],
                "role": call["role"],
                "choice": choice,
                "error": call.get("error"),
                "accepted": call.get("accepted"),
                "rejection": call.get("rejection_reason"),
                "latency_ms": raw["latency_seconds"] * 1000,
                "probabilities": selection.get("probabilities", {}),
                "room": sensors["room"],
                "position": sensors["position_estimate"],
                "objective": progress.get("objective"),
                "task_kind": state.get("selected_task_kind"),
                "task": state.get("task"),
                "option": option,
                "visible": progress.get("target_visible", False),
                "ready": progress.get("inspection_ready", False)
                or progress.get("dock_ready", False),
                "target_distance": progress.get("target_distance"),
                "clearance": sensors["body_clearance_m"],
                "depth_status": state.get("body_depth_status", {}),
                "goal": state.get("goal"),
                "offset": state.get("body_goal_offset_m"),
                "heading_error": state.get("goal_bearing_error_degrees"),
                "path_status": state.get("goal_path_status"),
                "altitude_access": state.get("altitude_access"),
                "detour": state.get("active_detour"),
                "viewpoints_seen": sum(
                    bool(v.get("inward_view_observed")) for v in memory.values()
                ),
                "viewpoints_total": len(memory),
                "heading_sectors": survey.get("surveyed_heading_sectors", []),
                "recent_tasks": [
                    {k: task.get(k) for k in ("name", "outcome")}
                    for task in state.get("recent_tasks", [])[-2:]
                ],
    ...
    controls = [
        {
            ...
            "values": c["after"]["body_controls"],
            "choice": c["choice"],
            "patch": c["patch"],
            "source": "Jev update" if c["expected_revision"] is not None else "Neutral reset",
        }
    ...
    controls += [
        {
            ...
            "choice": c["reason"],
            "patch": None,
            "source": "Lease expired" if c["reason"] == "lease_expired" else "Telemetry stale",
        }
        for c in result["command_expirations"]
    ]
    controls.sort(key=lambda c: (c["time"], c["revision"]))
    tag = source.parent.name + "--" + source.name
    payload = {
        "episode": "data/" + tag + ".json",
        "decisions": decisions,
        "controls": controls,
        "provenance": {
            "source": str(source),
            "model": next(c["model"] for c in result["calls"] if c.get("model")),
            "request_journal_sha256": hashlib.sha256(
                (source / "calls.jsonl").read_bytes()
            ).hexdigest(),
    ...
    print(
        json.dumps(
            {"output": str(output), "decisions": len(decisions), "control_events": len(controls)}
        )
    )
