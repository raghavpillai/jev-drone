...
def render(output, url, speed):
    ...
    with sync_playwright() as playwright:
        ...
        assert page.locator('.mission').evaluate('(element) => element.parentElement.className') == 'state-rail'
        source = json.loads((root / 'demo/data/state-replay.json').read_text())
        episode = json.loads((root / 'demo' / source['episode']).read_text())
        objective_stage = next(i for i, objective in enumerate(episode['world']['mission']) if objective.get('marker'))
        found = any(report['valid'] and report['stage'] == objective_stage for report in episode['reports'])
        ...
        command = [ffmpeg, '-hide_banner', '-loglevel', 'warning', '-y', '-f', 'image2pipe',
                   '-framerate', str(fps), '-vcodec', 'mjpeg', '-i', 'pipe:0', '-an',
                   '-c:v', 'libx264', '-threads', '4', '-preset', 'fast', '-crf', '18',
                   '-pix_fmt', 'yuv420p', '-fps_mode', 'passthrough', '-movflags', '+faststart', str(output)]
        with output.with_suffix('.encode.log').open('w') as log:
            ...
            try:
                for index in range(frames):
                    rendered = page.evaluate('([i,fps,speed])=>stateReplay.renderFrame(i,fps,speed)', [index, fps, speed])
                    assert math.isclose(rendered['time'], min(end, index * speed / fps), abs_tol=1e-9)
                    screenshot = cdp.send('Page.captureScreenshot', {'format': 'jpeg', 'quality': 97,
                    ...
                    if index % 150 == 0 or index == frames - 1:
                        print(json.dumps({'frame': index + 1, 'frames': frames, 'sim_seconds': round(rendered['time'], 2),
                                          'render_wall_seconds': round(time.monotonic() - started)}), flush=True)
                        snapshots.append(rendered)
                    if index in (0, flight_frames // 2, flight_frames - 1):
                        label = 'start' if index == 0 else ('found' if found else 'unfinished') if index == flight_frames - 1 else 'search'
        ...
        assert page.locator('#mission-phase').inner_text() == ('Item found' if found else 'Search unfinished')
        browser.close()
    ...
    subprocess.run([ffmpeg, '-hide_banner', '-loglevel', 'error', '-y', '-i', str(output),
                    '-an', '-f', 'framemd5', str(checksum_file)], check=True)
    checksum_text = checksum_file.read_text()
    assert '#tb 0: 1/30' in checksum_text
    decoded = [line.split(',') for line in checksum_text.splitlines() if not line.startswith('#') and line.strip()]
    ...
    manifest = {
        'file': str(output), 'source_episode': source['episode'], 'request_provenance': source['provenance'],
        ...
        'resolution': metadata['size'], 'fps': fps, 'frames': frames, 'flight_frames': flight_frames,
        'duration_seconds': frames / fps, 'playback_rate': speed, 'mission_end_sim_seconds': end,
        'repeated_decoded_frames': repeated, 'unique_capture_frames': len(set(frame_hashes)),
        'full_decode_verified': True, 'browser_errors': errors, 'size_bytes': output.stat().st_size,
        'render_wall_seconds': time.monotonic() - started, 'snapshots': snapshots,
        'renderer_sha256': {name: hashlib.sha256((root/'demo'/name).read_bytes()).hexdigest()
                            for name in ('src/state.js', 'state.html', 'src/state.css', 'src/scene.js', 'src/furnishings.js', 'src/target-effects.js')},
    }
    output.with_suffix('.json').write_text(json.dumps(manifest, indent=2) + '\n')
    print(json.dumps({k: v for k, v in manifest.items() if k not in ('snapshots', 'renderer_sha256', 'request_provenance')}, indent=2), flush=True)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--out', type=Path, default=Path('report/px4-find-video/jev-best-mission-12x.mp4'))
