...
class DoorMemory:
    ...
    def observe(self, cloud, time):
        ...
        for name, center in self.layout.doors.items():
            ...
            across = 1-axis
            mask = ((np.abs(cloud[:, axis]-center[axis]) <= .2)
                    & (np.abs(cloud[:, across]-center[across]) < .5)
                    & (cloud[:, 2] >= .8) & (cloud[:, 2] < 2.1))
            samples = cloud[mask]
            if not len(samples):
                continue
            columns = np.floor((samples[:, across]-center[across]+.5)/.2).astype(int)
            rows = np.floor((samples[:, 2]-.8)/.26).astype(int)
            fraction = len(np.unique(columns+5*rows))/25.
            if fraction >= .4:
                self.obstructions[name] = {"occupied_opening_fraction": round(fraction, 2),
                    "observed_at": time, "source": "rendered depth surface in central opening"}

    def describe(self, state, criteria):
        state, criteria = dict(state), dict(criteria)
        state["observed_door_obstructions"] = {name: dict(value) for name, value in self.obstructions.items()}
        for door, evidence in self.obstructions.items():
            for name in ("approach_"+door, "cross_"+door):
                if name in criteria:
                    criteria[name] += (" OBSERVED OBSTRUCTION across this door's central opening (depth coverage "
                        +str(evidence["occupied_opening_fraction"])+"). Prefer another room connection instead of retrying this route.")
