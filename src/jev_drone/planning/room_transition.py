...
def describe(state, criteria, layout):
    previous = next((t for t in reversed(state.get('recent_tasks', []))
        if t['name'].startswith('cross_') and t.get('outcome')=='arrived'
        and t.get('mission_stage',0)==state['stage']), None)
    ...
    state['last_completed_room_transition'] = {'door':door,'from_room':origin,'to_room':room,
        'mission_stage':state['stage'],'outcome':'already completed'}
    ...
    for name, option in state['options'].items():
        if option.get('leads_to_room') == origin:
            ...
            if name in criteria:
                criteria[name] += ' This goes BACK to '+origin+' across the door you already crossed into '+room+'. It is not a continuation of that completed crossing. Use only for deliberate backtracking.'
    return state, criteria
