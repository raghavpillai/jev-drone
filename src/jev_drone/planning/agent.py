...
class Experiment:
    ...
    def __init__(self, config, gateway, frame_dir=None, *, world=None, sensors=None):
        ...
        if frame_dir:
            frame_dir.mkdir()
        self.last_saved_frame = -10.0
        self.started_wall = time.perf_counter()
        self.sensors.advance(config.sensor_delay + 0.01)
    ...
    def observe(self):
        ...
        if self.last_observation:
            ...
            for direction, clearance in observation["clearance_m"].items():
                ...
                if clearance is not None and previous is not None and elapsed > 0.05:
                    closing[direction] = round(max(0.0, (previous - clearance) / elapsed), 2)
    ...
    def facts(self, observation):
        ...
        return {
            ...
            "inspection_ready": bool(
                target
                and distance <= 1.5
                and observation["speed"] <= 0.15
                and observation["room"] == objective.get("room")
            ),
            "target_visible": target is not None,
            "target_distance": round(distance, 2) if distance is not None else None,
            "dock_ready": bool(
                "dock" in objective
    ...
    def options(self, observation):
        ...
        for name, center in self.layout.doors.items():
            ...
            options["approach_" + name] = {
                "position": near.tolist(),
                "purpose": "approach doorway from current room",
            }
            if np.linalg.norm(pose - near) < 1.4:
                options["cross_" + name] = {
                    "position": far.tolist(),
                    "purpose": "cross doorway into " + (second if room == first else first),
                }
        objective = self.world.mission[self.stage]
            ...
            for name, view in self.layout.search_viewpoints().items():
                if view["room"] == room:
                    options[name] = {
                        ...
                        "purpose": "search from a different viewpoint",
                    }
        ...
        observed = (
            ...
            else {d["marker"]: d for d in observation["detections"]}
        )
        if objective.get("marker") in observed:
            target = np.array(observed[objective["marker"]]["position"])
            vector = target - pose
            direction = vector / max(0.001, np.linalg.norm(vector))
            goal = target - direction * 0.95
            options["approach_marker"] = {
                "position": goal.tolist(),
                "look": cardinal(vector),
                "purpose": "approach last observed target; verify visually",
            }
        if "dock" in objective and room == self.layout.room_at(objective["dock"]):
            options["return_launch"] = {
                "position": objective["dock"],
                "purpose": "return to launch position",
            }
        if "marker" in objective:
            options["inspect"] = {
                "purpose": "report current requested marker inspected; requires inspection_ready=true"
            }
        else:
        ...
        for option in options.values():
            if "position" in option:
                ...
                option.update(distance_m=round(distance, 2), already_at_position=distance <= 0.42)
        return options
    ...
    def request(self, role, state, instructions, criteria):
        ...
        if result.get("error"):
            ...
            failed = (
                time.monotonic() - self.error_started > 15
                if getattr(self.config, "front_controls", False)
    ...
    def plan(self, observation):
        ...
        state = {
            ...
            "stage": self.stage,
            **facts,
            "ACTIVE_OBJECTIVE": facts["objective"]
            if "marker" in facts["objective"]
            else {
                "return_to_launch": facts["objective"]["dock"],
                "room": self.layout.room_at(facts["objective"]["dock"]),
            },
            "time_remaining": round(self.config.seconds - self.world.time, 1),
            "sensors": observation,
            "known_room_bounds": self.layout.rooms,
            "known_doors": self.layout.doors,
            "known_room_connections": self.layout.neighbors(),
            "options": options,
        }
        if self.config.memory:
            state.update(
                observed_markers=self.memory,
                recent_tasks=self.tasks[-12:],
                visits=dict(self.visits),
            )
            if self.config.policy == "v4":
                failures = Counter(
                    t["name"] for t in self.tasks if t["outcome"] in ("stalled", "replan_requested")
                )
                state["failed_navigation_attempts"] = dict(failures)
                state["routing_note"] = (
                    "A stalled cross_DOOR task means this doorway may be obstructed. Choose another connected room/door to bypass it. Repeating the same failed crossing is not progress."
                )
        choice, log = self.request(
            "planner",
            state,
            PLANNER,
        ...
        if choice in ("inspect", "dock"):
            ...
            if choice == "inspect":
                ...
                valid = (
                    ...
                    and np.linalg.norm(self.world.position - target["position"]) <= 1.75
                )
            else:
                valid = (
                    valid
                    and "dock" in objective
                    and np.linalg.norm(self.world.position - self.world.start) <= 0.55
                )
            valid = bool(valid and np.linalg.norm(self.world.velocity) <= 0.15)
            self.reports.append(
                {
                    "choice": choice,
                    "stage": self.stage,
        ...
        self.tasks.append(
            {
                **self.task,
                "time": self.world.time,
                "start": observation["position_estimate"],
                "outcome": "active",
            }
        )
    ...
    def distance(self, fresh):
        if self.task and "position" in self.task:
            return float(
                np.linalg.norm(np.array(fresh["position_estimate"]) - self.task["position"])
            )
        return 0.0
    ...
    def task_finished(self, observation):
        ...
        if distance < self.best_distance - 0.08:
            self.best_distance = distance
            self.last_progress = self.world.time
        at_position = distance <= 0.42
        aim = self.task.get("look")
        aimed = (
            not aim
            or np.dot(self.world.camera_direction, DIRECTIONS[aim]) > 0.99
            or self.config.rig == "surround"
        )
        outcome = None
        if at_position and observation["speed"] <= 0.15 and aimed:
            outcome = "arrived"
    ...
    def control(self, observation):
        ...
        if self.detour is not None:
            reached = (
                np.linalg.norm(np.array(observation["position_estimate"]) - self.detour["position"])
                <= 0.20
            )
            if (reached and observation["speed"] <= 0.15) or self.world.time - self.detour[
                "time"
            ] > 10:
                self.detour = None
        ...
        at_position = np.linalg.norm(delta) <= (0.20 if self.detour else 0.42)
        state = {
            "sensors": observation,
            "goal": goal,
            "goal_offset": np.round(delta, 2).tolist(),
            "at_position": bool(at_position),
            "look_when_at_position": self.task.get("look") if at_position else None,
            "cruise_speed_mps": self.config.speed,
            "task": self.task["name"],
            "recent_actions": [
                {"choice": c.get("choice"), "position": c["state"]["sensors"]["position_estimate"]}
                for c in self.calls[-6:]
                if c["role"] == "control"
            ],
        }
        velocity = np.array(observation["velocity_estimate"])
        motion_direction = cardinal(velocity) if observation["speed"] > 0.15 else None
        along_motion = (
            float(np.dot(delta, DIRECTIONS[motion_direction])) if motion_direction else None
        )
        state.update(
            stopped=observation["speed"] <= 0.15,
            motion_direction=motion_direction,
            remaining_along_motion=round(along_motion, 2) if along_motion is not None else None,
            motion_axis_needs_travel=bool(
                along_motion is not None and along_motion > (0.10 if self.detour else 0.20)
            ),
            axis_needs={
                name: bool(np.dot(delta, direction) > 0.20)
        ...
        for name in DIRECTIONS:
            criteria["look_" + name] = (
                "Brake and gradually aim camera " + name + " to acquire depth before motion."
            )
            criteria["slow_" + name] = (
                "Move "
                + name
                + " at 0.25m/s; require measured clearance >=0.4m and no axis change while moving."
            )
            criteria["cruise_" + name] = (
                "Move "
                + name
                + " at cruise speed; require measured clearance >=1.3m and no axis change while moving."
            )
        instructions = PILOT
        if tactical:
...
"""
            if self.detour is None:
                for name in DIRECTIONS:
                    criteria["detour_" + name] = (
                        "Brake and choose a temporary 1.3m goal "
                        + name
                        + " to go around an obstruction; movement is decided on later calls."
                    )
                    if self.config.policy == "v4":
                        status = state["depth_status"][name]
                        criteria["detour_" + name] += (
                            f" Observed direction is {status}. This is a viable bypass candidate."
                            if status == "OPEN"
                            else f" Observed direction is {status}: DO NOT SELECT this detour. It cannot bypass a known obstruction in that same direction. Look elsewhere for OPEN space first."
                        )
                if self.config.policy == "v4":
                    ...
                    if risk:
                        for prefix in ("cruise_", "slow_", "detour_"):
                            if prefix + name in criteria:
                                criteria[prefix + name] += (
                                    " PREDICTED MOVING-OBSTACLE RISK: do not select; choose brake or an observed safe alternative."
                                )
    ...
    def select_detour(self, choice, observation):
        ...
        self.detour = {
            "position": (
                np.array(observation["position_estimate"]) + 1.3 * np.array(direction)
            ).tolist(),
            "time": self.world.time,
            "selected_by": "jev",
            "choice": choice,
    ...
    def audit(self, choice, observation, log):
        ...
        reasons = []
        if clearance is None or clearance < (0.4 if speed == "slow" else 1.3):
            reasons.append("unknown_or_insufficient_observed_clearance")
        velocity = np.array(observation["velocity_estimate"])
        if (
            np.linalg.norm(velocity) > 0.15
            and np.dot(velocity / np.linalg.norm(velocity), DIRECTIONS[direction]) < 0.95
        ):
    ...
    def run(self):
        try:
            while (
                ...
                and self.world.time < self.config.seconds
                and self.cost < self.config.budget
            ):
                if not getattr(self.world, "telemetry_ready", True):
                    self.sensors.advance(0.1)
                    continue
            ...
            return {
                ...
                "stale_responses": self.stale,
                "telemetry_outages": getattr(self.world, "telemetry_outages", []),
                "trace": self.world.trace,
                "contacts": self.world.contacts,
                "min_clearance": self.world.min_clearance,
                "near_miss_seconds": self.world.near_miss_seconds,
                "blackout_frames": self.sensors.blackout_frames,
                "observed_markers": self.memory,
            }
        finally:
            self.world.close()
