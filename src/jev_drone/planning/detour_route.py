...
def candidate(pose, goal, direction, distance, preserve_offset, layout=SMALL):
    ...
    second[2] = first[2]
    if preserve_offset and np.linalg.norm(second-first) > .4:
        # These are the known building bounds, not hidden furniture geometry.
        lo, hi = layout.bounds
        if np.all(second >= lo+[.45, .45, .6]) and np.all(second <= hi-[.45, .45, .5]):
...
def advance(detour, time):
    ...
    progress = {key: value for key, value in detour.items()
                if key not in ("best_distance", "best_heading_error")}
    return {**progress, "position": remaining[0], "remaining_route": remaining[1:],
            "time": time, "leg": detour.get("leg", 0)+1}
...
def observed_obstacles(route, pose, frames):
    ...
    for target in route:
        ...
        hit = (observed_collision_distance(frames, origin, vector/distance, .45)
               if distance > .001 else float("inf"))
        blocked = hit < distance
        legs.append({"length_m": round(distance, 2), "observed_blocked": blocked,
