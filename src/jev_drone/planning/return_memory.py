...
from jev_drone.planning.return_intent import INSTRUCTIONS
...
class ReturnMemory:
    ...
    def observe(self, observation, returning):
        ...
        turned = abs((heading-previous["yaw_degrees"]+180) % 360-180) if previous else 0.
        if previous and moved < .8 and not (moved >= .2 and turned >= 35):
            return
        # Keep a bounded history. Dropped samples never certify a connecting path.
        if len(self.points) >= 256:
            self.points = self.points[::2]
        self.points.append({"position": pose.tolist(), "room": observation["room"],
                            "yaw_degrees": heading})

    def options(self, observation, tasks):
        if not self.returning:
            return {}
        cursor = min((t["return_route_index"] for t in tasks
        ...
        for index in range(cursor-1, -1, -1):
            ...
            if distance > 3.5 or (np.linalg.norm(delta[:2]) <= .35 and abs(delta[2]) <= .15):
                continue
            candidates[f"retrace_{index}"] = {"position": point["position"],
                "room": point["room"], "return_route_index": index,
    ...
    def describe(self):
        return {"returning": self.returning, "recorded_positions": len(self.points),
