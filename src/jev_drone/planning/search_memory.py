...
def heading_sectors(yaw):
    ...
    return {
        heading for heading in range(0, 360, 45) if abs((heading - yaw + 180) % 360 - 180) <= 45
    }
...
class SearchMemory:
    ...
    def nearby_station(self, room, position):
        matches = [
            ...
            if station["room"] == room
            and np.linalg.norm(np.array(station["position"][:2]) - position[:2]) <= 0.7
            and abs(station["position"][2] - position[2]) <= 0.35
        ]
        return min(
            matches, key=lambda s: np.linalg.norm(np.array(s["position"]) - position), default=None
        )

    def observe(self, observation, time):
        if (
            not observation.get("front_frame_available")
            or observation.get(
                "front_valid_depth_fraction", observation.get("valid_depth_fraction", 0.0)
            )
            <= 0.4
            or (observation.get("frame_age_seconds") or 0.0) > 0.65
        ):
            return
        position = np.array(observation["position_estimate"])
        sector = int(math.floor((observation["yaw_degrees"] + 22.5) / 45)) % 8
        settled = (
            observation.get("speed", 0.0) <= 0.1
            and abs(observation.get("yaw_rate_rps", 0.0)) <= 0.1
        )
        ...
        for view in self.viewpoints.values():
            ...
            if np.linalg.norm(delta[:2]) <= 0.7 and abs(delta[2]) <= 0.35:
                ...
                if settled:
                    ...
                    view["inward"] |= (
                        abs((inward - observation["yaw_degrees"] + 180) % 360 - 180) <= 45
                    )

    def summary(self, room):
        return {
            name: {
                "observed_near_viewpoint": bool(view["headings"]),
            ...
            for name, view in self.viewpoints.items()
            if view["room"] == room
        }
    ...
    def describe(self, state, criteria):
        ...
        state["unobserved_viewpoints"] = [
            name for name, view in summary.items() if not view["observed_near_viewpoint"]
        ]
        state["viewpoints_without_inward_survey"] = [
            name for name, view in summary.items() if not view["inward_view_observed"]
        ]
        objective = state.get("objective", {})
        if "marker" in objective:
            known = objective["marker"] in state.get("observed_markers", {}) or state.get(
                "target_visible", False
            )
            phase = (
                "REPORT_READY"
                if state.get("inspection_ready")
                else "GO_TO_REQUESTED_ROOM"
                if state["sensors"]["room"] != objective["room"]
                else "APPROACH_OBSERVED_TARGET"
                if known
                else "SEARCH_REQUESTED_ROOM"
            )
            state["search_phase"] = {
                "situation": phase,
                "requested_room": objective["room"],
                "current_room": state["sensors"]["room"],
                "target_previously_observed": bool(known),
            }
        pose = state["sensors"].get("position_estimate")
        station = (
            self.nearby_station(state["sensors"]["room"], np.array(pose))
            if pose is not None
            else None
        )
        surveyed = station["surveyed"] if station else set()
        state["local_survey"] = {
            "anchor": list(station["position"]) if station else None,
        ...
        state["options"] = {
            name: {**option, **summary.get(name, {})} for name, option in state["options"].items()
        }
        for name, view in summary.items():
            if name in criteria:
                criteria[name] += (
                    " Camera has NOT yet observed from near this viewpoint."
                    if not view["observed_near_viewpoint"]
                    else " Previously observed near this viewpoint; headings="
                    + str(view["observed_heading_degrees"])
                    + "."
                )
                criteria[name] += (
                    " Inward survey observed="
                    + str(view["inward_view_observed"])
                    + ". Settled front heading sectors="
                    + str(view["surveyed_heading_sectors"])
                    + "."
                )
        for name, heading in {
            "look_east": 0.0,
            ...
            if name in criteria:
                criteria[name] += (
                    " New front heading sectors from here="
                    + str(fresh)
                    + ". Does not reveal space behind an occluder."
                )
        criteria["give_up"] += (
            " Do not select while useful unobserved viewpoints remain with adequate time and sensing; a few unsuccessful views do not establish that navigation is impossible."
        )
