...
DIRECTIONS = {"east": (1, 0, 0), "west": (-1, 0, 0), "north": (0, 1, 0),
              "south": (0, -1, 0), "up": (0, 0, 1), "down": (0, 0, -1)}
ROOMS, DOORS = SMALL.rooms, SMALL.doors
COLORS = {"red": (.95, .06, .06, 1), "blue": (.06, .12, .95, 1),
          "yellow": (.95, .9, .04, 1)}
...
class Config:
    ...
    memory: bool = True
    sensor_hz: float = 8.
    sensor_delay: float = .10
    depth_noise: float = .015
    dropout: float = .06
    odometry_drift: float = .0015
    wind: float = .12
    speed: float = .6
    seconds: float = 150.
    budget: float = .18
...
class World:
    ...
    def __init__(self, config):
        ...
        self.camera_direction = np.array([1., 0., 0.])
        ...
        self._build()
        shape = p.createCollisionShape(p.GEOM_SPHERE, radius=self.radius, physicsClientId=self.client)
        self.drone = p.createMultiBody(baseMass=1., baseCollisionShapeIndex=shape,
                                      basePosition=self.start, physicsClientId=self.client)
        p.changeDynamics(self.drone, -1, linearDamping=0., angularDamping=.95, physicsClientId=self.client)
    ...
    def box(self, name, center, half, color=(.58, .57, .55, 1), dynamic=False):
        ...
        self.geometry.append({"name": name, "center": list(center), "half": list(half),
    ...
    def _wall_with_door(self, axis, coordinate, lo, hi, doors, width):
        ...
        for a, b in zip(edges[::2], edges[1::2]):
            if axis == 0:
                self.box("wall", (coordinate, (a+b)/2, height/2), (.075, (b-a)/2, height/2), (.8, .81, .84, 1))
            else:
                self.box("wall", ((a+b)/2, coordinate, height/2), ((b-a)/2, .075, height/2), (.8, .81, .84, 1))
        for door in doors:
            z, h = (height+door_height)/2, (height-door_height)/2
            center = (coordinate, door, z) if axis == 0 else (door, coordinate, z)
            half = (.075, width/2, h) if axis == 0 else (width/2, .075, h)
    ...
    def _build(self):
        ...
        for name, center, half in [
            ("floor", (6, 4, -.1), (6, 4, .1)), ("ceiling", (6, 4, 4.1), (6, 4, .1)),
            ("west_wall", (-.1, 4, 2), (.1, 4, 2)), ("east_wall", (12.1, 4, 2), (.1, 4, 2)),
            ("south_wall", (6, -.1, 2), (6, .1, 2)), ("north_wall", (6, 8.1, 2), (6, .1, 2))]:
            self.box(name, center, half, (.78, .8, .82, 1))
        self._wall_with_door(0, 6, 0, 8, [2, 6], .95 if case == "narrow" else 1.65)
        self._wall_with_door(1, 4, 6, 12, [9], 1.65)
        jitter = self.rng.uniform(-.25, .25, 2)
        # Furniture is assembled from surfaces and legs, not floor-solid boxes.
        self.box("table_top", (3.5+jitter[0], 4.2, 1.05), (.85, .7, .07), (.45, .27, .12, 1))
        for x in (2.8+jitter[0], 4.2+jitter[0]):
            for y in (3.65, 4.75):
                self.box("table_leg", (x, y, .5), (.055, .055, .5), (.3, .2, .1, 1))
        self.box("bed", (9.5, 1.1, .4), (1.0, .7, .4), (.55, .36, .34, 1))
        self.box("chair_seat", (7.4, 6.8, .6), (.35, .35, .055), (.4, .3, .2, 1))
        self.box("chair_back", (7.4, 7.12, 1), (.35, .045, .4), (.4, .3, .2, 1))
        for x in (7.12, 7.68):
            for y in (6.52, 7.08):
                self.box("chair_leg", (x, y, .28), (.04, .04, .28))
        for z in (.2, 1.2, 2.2):
            self.box("shelf", (11.55, 6.1, z), (.35, 1.1, .06), (.4, .25, .12, 1))
        for y in (5.0, 7.2):
            self.box("shelf_side", (11.55, y, 1.2), (.35, .06, 1.2), (.4, .25, .12, 1))
        self.box("thin_pole", (4.65, 2.8, 1.25), (.045, .045, 1.25))
        if case in ("overhang", "vertical"):
            self.box("hanging_beam", (3.6, 2, 1.65), (.2, 1.0, .28))
            self.box("low_screen", (4.6, 2, .55), (.14, 1.25, .55))
        if case in ("occluded", "moved_target"):
            self.box("search_screen", (9.6, 2.55, 1.15), (.12, .85, 1.15))
        if case == "blocked_door":
            self.box("unexpected_closed_door", (6, 2, 1.35), (.08, .825, 1.35), (.4, .28, .16, 1))
        if case in ("crossing", "crowded"):
            for index, x in enumerate([3.7] if case == "crossing" else [3.7, 7.7, 9.8]):
                body = self.box("crossing_actor", (x, 4, .9), (.22, .22, .9), (.18, .6, .28, 1), True)
                self.dynamic.append({"body": body, "x": x, "phase": float(self.rng.uniform(0, 2*math.pi)), "index": index})
        target_positions = {
        ...
        for name, point in target_positions.items():
            body = self.box(name+"_marker", point, (.20, .20, .20), COLORS[name], case == "moved_target" and name == "red")
            self.targets[name] = {"position": point, "body": body}
        self.mission = ([{"marker": "red", "room": "bedroom"}, {"marker": "blue", "room": "study"},
                         {"marker": "yellow", "room": "living"}, {"dock": self.start.tolist()}]
                        if case in ("sequence", "crowded") else
                        [{"marker": "red", "room": "bedroom"}, {"dock": self.start.tolist()}])
    ...
    def command(self, choice):
        if choice.startswith("look_"):
            ...
            magnitude = .25 if speed == "slow" else self.config.speed
    ...
    def advance(self, duration):
        ...
        for _ in range(count):
            ...
            acceleration = error/.28
            if self.config.servo == "pi":
                self.velocity_integral = np.clip(self.velocity_integral+error*dt, -.5, .5)
                acceleration += 4.*self.velocity_integral
            norm = np.linalg.norm(acceleration)
            if norm > 1.8:
                acceleration *= 1.8/norm
            wind = self.config.wind*np.array([math.sin(.7*self.time), math.cos(.43*self.time), .2*math.sin(self.time)])
            force = acceleration + np.array([0, 0, 9.81]) + wind - .1*self.velocity
            p.applyExternalForce(self.drone, -1, force, self.position, p.WORLD_FRAME, physicsClientId=self.client)
            self._turn_camera(dt)
            for actor in self.dynamic:
                y = 2 + 1.35*math.sin(.65*self.time+actor["phase"])
                if actor["x"] > 6:
                    y = 5.7 + 1.25*math.sin(.65*self.time+actor["phase"])
                p.resetBasePositionAndOrientation(actor["body"], (actor["x"], y, .9), (0, 0, 0, 1), physicsClientId=self.client)
            if self.config.case == "moved_target" and self.time >= 18:
                target = self.targets["red"]
                target["position"] = [8.2, 3.1, 1.5]
                p.resetBasePositionAndOrientation(target["body"], target["position"], (0, 0, 0, 1), physicsClientId=self.client)
            p.stepSimulation(physicsClientId=self.client)
            self.time += dt
            contact = p.getContactPoints(bodyA=self.drone, physicsClientId=self.client)
            if contact:
                self.status = "collision"
                self.contacts.append({"time": self.time, "bodies": [c[2] for c in contact], "position": self.position.tolist()})
            if not self.trace or self.time-self.trace[-1]["time"] >= .1-1e-8:
                self._record()
    ...
    def _turn_camera(self, dt):
        ...
        if angle < 1e-7:
            return
        fraction = min(1., self.camera_rate*dt/angle)
        ...
        self.camera_direction = math.cos(angle*fraction)*self.camera_direction + math.sin(angle*fraction)*tangent

    def _record(self):
        # Ground-truth distances are evaluator-only; never used by control/sensors.
        clearance = 100.
        for body in self.bodies:
            points = p.getClosestPoints(self.drone, body, .5, physicsClientId=self.client)
            if points:
                clearance = min(clearance, min(point[8] for point in points))
        self.min_clearance = min(self.min_clearance, clearance)
        if clearance < .12:
            self.near_miss_seconds += .1
        self.trace.append({"time": round(self.time, 4), "position": self.position.tolist(),
                           "velocity": self.velocity.tolist(), "camera": self.camera_direction.tolist(),
                           "clearance": clearance,
                           "actors": [list(p.getBasePositionAndOrientation(a["body"], physicsClientId=self.client)[0]) for a in self.dynamic]})

    def metadata(self):
        return {"config": asdict(self.config), "geometry": self.geometry, "mission": self.mission,
                "targets": self.targets, "start": self.start.tolist(), "rooms": self.layout.rooms, "doors": self.layout.doors,
