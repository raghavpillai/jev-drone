...
class Session:
    def __init__(self, config, output, source=Path(".sim/PX4-Autopilot")):
        ...
        self.motion_time = self.context.Value("d", 0., lock=False)
        self.motion_epoch = self.context.Value("d", 0., lock=False)
    ...
    def launch(self):
        ...
        env = dict(os.environ, GZ_SIM_RESOURCE_PATH=str(models), GZ_SIM_SYSTEM_PLUGIN_PATH=str(
            self.source/"build/px4_sitl_default/src/modules/simulation/gz_plugins"))
        self._start(["gz", "sim", "-r", "-s", "--headless-rendering", "--force-version", "8", str(self.output/"world.sdf")],
                    "gazebo", env, self.output)
        ...
        rootfs.mkdir()
        env.update(PX4_SYS_AUTOSTART="4001", PX4_GZ_STANDALONE="1", PX4_GZ_WORLD="jev", PX4_GZ_MODEL_NAME="drone",
                   PX4_PARAM_EKF2_GPS_CTRL="0", PX4_PARAM_EKF2_EV_CTRL="15", PX4_PARAM_EKF2_HGT_REF="3",
                   PX4_PARAM_EKF2_EV_NOISE_MD="1", PX4_PARAM_EKF2_EVP_NOISE="0.03", PX4_PARAM_EKF2_EVV_NOISE="0.05",
                   PX4_PARAM_EKF2_MAG_TYPE="5",
                   PX4_PARAM_COM_ARM_WO_GPS="1", PX4_PARAM_COM_RC_IN_MODE="4", PX4_PARAM_NAV_DLL_ACT="0",
                   PX4_PARAM_COM_OF_LOSS_T="0.5", PX4_PARAM_COM_OBL_RC_ACT="4", PX4_PARAM_COM_DISARM_LAND="2")
        self._start([str(build/"bin/px4"), "-d", str(build/"etc")], "px4", env, rootfs)
        self.flight = Flight(lambda: self.transport.time, body_controls=getattr(self.config, "front_controls", False),
                             lease=self.config.command_lease_seconds)
        self.flight.connect()
        if self.scene["actors"] or self.config.case == "moved_target":
            self.motion_process = self.context.Process(target=animate, args=(self.scene, self.config.case,
                self.motion_time, self.motion_epoch, self.motion_active, self.motion_running, self.motion_messages))
    ...
    def _start(self, command, name, env, cwd):
        ...
        self.processes.append(subprocess.Popen(command, cwd=cwd, env=env, stdout=stream, stderr=subprocess.STDOUT,
                                               start_new_session=True))

    def check(self):
        for process in self.processes:
            if process.poll() is not None:
                raise RuntimeError(f"Simulator process exited with {process.returncode}; inspect logs")
