...
from jev_drone.world.world import World

WIDTH, HEIGHT, HFOV = 160, 120, 2.4
# Cameras are fixed to the vehicle, including its actual pitch and roll.
CAMERAS = {
    "east": ((0.30, 0, 0.10), (0, 0, 0)),
    "west": ((-0.30, 0, 0.10), (0, 0, math.pi)),
    "north": ((0, 0.30, 0.10), (0, 0, math.pi / 2)),
    "south": ((0, -0.30, 0.10), (0, 0, -math.pi / 2)),
    "up": ((0, 0, 0.22), (0, -math.pi / 2, 0)),
    "down": ((0, 0, -0.26), (0, math.pi / 2, 0)),
}
...
def generate(config, source, output):
    ...
    for library, name in (
        ...
        plugin = element(
            world, "plugin", filename=f"gz-sim-{library}-system", name=f"gz::sim::systems::{name}"
        )
        if name == "Sensors":
    ...
    drone.find("pose").text = numbers([*scene.start[:2], 0.24, 0, 0, 0])
    ...
    element(odometry, "gaussian_noise", 0.005)
    ...
    metadata = {
        ...
        "actors": scene.dynamic,
        "rooms": scene.layout.rooms,
        "doors": scene.layout.doors,
        "room_connections": scene.layout.connections,
        "cameras": cameras,
        "width": WIDTH,
        "height": HEIGHT,
        "hfov": hfov,
        "localization": "PX4 EKF2 fusing simulated external odometry; no GPS, no real VIO",
