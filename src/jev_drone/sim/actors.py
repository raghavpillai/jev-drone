    ...
    try:
        while running.value:
            ...
            for actor in scene["actors"]:
                positions[f"crossing_actor_{actor['body']}"] = [actor["x"], 2+1.35*math.sin(.65*elapsed+actor["phase"]), .9]
            if case == "moved_target":
                target = scene["targets"]["red"]
                fraction = min(1., max(0., (elapsed-18)/5))
                positions[f"red_marker_{target['body']}"] = [a*(1-fraction)+b*fraction for a, b in zip(target["position"], [8.2, 3.1, 1.5])]
            request = Pose_V()
            ...
            if not ok or not response.data:
                if not ready and time.monotonic() < startup_deadline:
                    continue
                raise RuntimeError(f"Gazebo moving-entity request failed: transport={ok}, applied={response.data}")
            if not ready:
                messages.put(None)
                ready = True
            time.sleep(.04)
