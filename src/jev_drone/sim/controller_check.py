...
class ControllerCheck(FrontExperiment):
    ...
    course = [
        ("forward", [2.4, 2.0, 1.3], 0.0),
        ("backward", [1.4, 2.0, 1.3], 0.0),
        ("strafe_left", [1.4, 3.0, 1.3], 0.0),
        ("strafe_right", [1.4, 2.0, 1.3], 0.0),
        ("pan_left", [1.4, 2.0, 1.3], 90.0),
        ("strafe_right_after_turn", [2.4, 2.0, 1.3], 90.0),
        ("strafe_left_after_turn", [1.4, 2.0, 1.3], 90.0),
        ("pan_right", [1.4, 2.0, 1.3], 0.0),
        ("up", [1.4, 2.0, 2.3], 0.0),
        ("down", [1.4, 2.0, 1.3], 0.0),
    ]
    ...
    def plan(self, observation):
        if self.tasks:
            task = self.tasks[-1]
            error = abs(
                wrap(self.world.transport.truth_yaw - math.radians(task["heading_degrees"]))
            )
            valid = (
                task["outcome"] == "arrived"
                and np.linalg.norm(self.world.position - task["position"]) <= 0.48
                and error <= math.radians(15)
                and np.linalg.norm(self.world.velocity) <= 0.15
            )
            self.reports.append(
                {
                    "task": task["name"],
                    ...
                    "truth_yaw_degrees": math.degrees(self.world.transport.truth_yaw),
                }
            )
            if not valid:
    ...
    def run(self):
        result = super().run()
        result["evaluation"] = (
            "Jev local controller qualification; supplied test goals, no model mission planner"
        )
