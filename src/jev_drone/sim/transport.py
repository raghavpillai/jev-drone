...
class Transport:
    def __init__(self, shared_time=None):
        ...
        self.images = {
            name: {"rgb": deque(maxlen=12), "depth": deque(maxlen=12)} for name in CAMERAS
        }
        self.truth = None
        self.truth_yaw = 0.0
        self.entity_truth = {}
        ...
        for name in CAMERAS:
            self.node.subscribe(
                Image, "/jev/camera/" + name + "/image", partial(self._image, name, "rgb")
            )
            self.node.subscribe(
                Image, "/jev/camera/" + name + "/depth_image", partial(self._image, name, "depth")
            )
    ...
    def _pose(self, msg):
        for pose in msg.pose:
            if pose.name.startswith(("crossing_actor_", "red_marker_")):
                self.entity_truth[pose.name] = np.array(
    ...
    def _image(self, name, kind, msg):
        if kind == "depth":
            image = (
                np.frombuffer(msg.data, dtype="<f4")
                .reshape(msg.height, msg.step // 4)[:, : msg.width]
                .copy()
            )
        else:
    ...
    def _contact(self, msg):
        if msg.contact:
            self.contacts.append(
                {
                    "time": stamp(msg),
                    "pairs": [[c.collision1.name, c.collision2.name] for c in msg.contact],
                }
            )
            ...
            for name, channels in self.images.items():
                for depth_time, depth in reversed(channels["depth"]):
                    ...
                    rgb = next(
                        (
                            rgb
                            for t, rgb in reversed(channels["rgb"])
                            if abs(t - depth_time) < 0.001
                        ),
                        None,
                    )
                    if rgb is not None:
