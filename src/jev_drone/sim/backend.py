...
from jev_drone.perception.sensors import Frame, Sensors, detect_markers, points, swept_clearance
...
class World:
    ...
    def __init__(self, config, session):
        ...
        self.layout = Layout(
            session.scene["rooms"], session.scene["doors"], session.scene["room_connections"]
        )
    ...
    def command(self, choice):
        ...
        if choice.startswith("look_"):
            ...
            velocity = np.array(DIRECTIONS[direction]) * (
                0.25 if speed == "slow" else self.config.speed
            )
        receipt = self.flight.set_velocity(velocity)
        self.commands.append(
            {"time": self.time, "choice": choice, "velocity": velocity.tolist(), **receipt}
        )
    ...
    def update_controls(self, choice, patch, revision):
        ...
        if receipt["applied"]:
            ...
            self.commands.append(
                {"time": self.time, "choice": choice, "velocity": velocity, **receipt}
            )
        return receipt
    ...
    def _monitor(self):
        ...
        while self.monitoring:
            ...
            fresh = now - last_clock_wall <= 1 and now - self.flight.position_at <= 0.5
            ...
            if truth and (previous is None or truth[0] - previous[0] >= 0.095):
                dt = truth[0] - previous[0] if previous else 0.1
                if previous:
                    ...
                    float(
                        np.linalg.norm(
                            np.maximum(
                                np.abs(
                                    truth[1]
                                    - self.transport.entity_truth.get(
                                        f"{box['name']}_{box['body']}", box["center"]
                                    )
                                )
                                - box["half"],
                                0,
                ...
                self.near_miss_seconds += dt if clearance < 0.12 else 0
                self.trace.append(
                    {
                        "time": self.time,
                        "wall_time": now - self.started_wall,
                        "position": truth[1].tolist(),
                        "velocity": self.velocity.tolist(),
                        "estimate": self.pose_estimate().tolist(),
                        "camera": self.camera_direction.tolist(),
                        "yaw_rate_rps": self.flight.yaw_rate,
                        "clearance": clearance,
                        "actors": [
                            self.transport.entity_truth.get(
                                f"crossing_actor_{a['body']}", np.array([a["x"], 4, 0.9])
                            ).tolist()
                            for a in self.session.scene["actors"]
                        ],
                        "mode": self.flight.mode,
                    }
                )
                if self.config.case == "moved_target":
                    ...
                    if actual is not None:
                        self.targets = {
                            **self.targets,
                            "red": {**target, "position": actual.tolist()},
                        }
            ...
            if self.status == "running" and (
                not self.flight.armed or self.flight.mode != "OFFBOARD"
            ):
    ...
    def metadata(self):
        return {
            **self.session.scene,
            ...
            "camera_control": (
                "front RGB-D for objects; rear/left/right/up/down depth only; physical body yaw and forward/right/up velocity"
                if self.config.front_controls
                else "six fixed body cameras; look actions select attention while braking, yaw held east"
            ),
...
class CameraSensors:
    ...
    def __init__(self, world):
        ...
        self.tracker = (
            MotionTracker(self.config.prediction, body_radius=0.40)
    ...
    def observed_gap_at(self, position):
        if not len(self.observed_cloud):
            return None
        return round(
            float(np.min(np.linalg.norm(self.observed_cloud - position, axis=1))) - 0.40, 3
        )
    ...
    def _frames(self):
        ...
        frames = []
        for name, (stamp, rgb, raw_depth) in world.transport.latest_pairs(
            self.config.sensor_delay
        ).items():
            if name not in self.cameras:
                continue
            if world.transport.time - stamp > 0.65 or not history:
                continue
            nearest = min(history, key=lambda p: abs(p[0] - stamp))
            if abs(nearest[0] - stamp) > 0.06:
            ...
            depth[~np.isfinite(depth) | (depth < 0.08) | (depth >= 12)] = np.nan
            depth += self.rng.normal(0, self.config.depth_noise, depth.shape) * (
                1 + depth * depth * 0.1
            )
            depth[self.rng.random(depth.shape) < self.config.dropout] = np.nan
            depth = np.round(depth / 0.02) * 0.02
    ...
    def observation(self):
        ...
        for frame in frames:
            ...
            for name, direction in DIRECTIONS.items():
                value = swept_clearance(frame, pose, direction, radius=0.40)
                if value is not None:
                    clearance[name] = max(clearance[name] or 0.0, value)
        observation = {
            "position_estimate": np.round(pose, 3).tolist(),
            "velocity_estimate": np.round(world.flight.velocity, 3).tolist(),
            "camera_direction": np.round(world.camera_direction, 3).tolist(),
            "camera_target": world.camera_target.tolist(),
            "camera_turning": False,
            "frame_age_seconds": max((world.time - f.time for f in frames), default=None),
            "valid_depth_fraction": float(np.mean([np.isfinite(f.depth).mean() for f in frames]))
            if frames
            else 0.0,
        ...
        if self.config.front_controls:
            ...
            observation.update(
                yaw_degrees=round(math.degrees(heading), 2),
                yaw_rate_rps=round(world.flight.yaw_rate, 3),
                camera_turning=abs(world.flight.yaw_rate) > 0.1,
                body_velocity_mps={
                    "forward": round(float(world.flight.velocity @ forward), 3),
                    "right": round(float(world.flight.velocity @ right), 3),
                    "up": round(float(world.flight.velocity[2]), 3),
                },
                body_clearance_m={},
                front_frame_available=any(f.camera_name == "east" for f in frames),
                front_frame_time=front.time if front is not None else None,
                front_valid_depth_fraction=float(np.isfinite(front.depth).mean())
                if front is not None
                else 0.0,
                semantic_camera_names=sorted(
                    {f.camera_name for f in frames if f.camera_name == "east"}
                ),
        ...
        if self.tracker:
            ...
            observation.update(
                self.tracker.observation(pose, world.flight.velocity, self.config.speed, world.time)
            )
        return observation
