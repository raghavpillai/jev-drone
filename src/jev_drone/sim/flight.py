...
def attitude_enu(roll, pitch, yaw):
    ...
    return (
        ...
        @ Rotation.from_euler("xyz", [roll, pitch, yaw]).as_matrix()
        @ np.diag([1, -1, -1])
    )


def body_velocity_enu(controls, yaw):
    forward, right = controls["forward_mps"], controls.get("right_mps", 0.0)
    return np.array(
        [
...
class Flight:
    ...
    def __init__(self, clock, *, body_controls=True, lease=0.6):
        self.clock = clock
        self.lease = lease
        ...
        self.active = False
        self.controls = (
            ControlLatch(self.lease, axes=BODY_CHANNELS, state_key="body_controls")
            if body_controls
            else ControlLatch(self.lease)
        )
        ...
        while self.running:
            ...
            for _ in range(200):
                ...
                if kind == "HEARTBEAT":
                    ...
                    self.mode = (
                        "OFFBOARD"
                        if message.autopilot == mavutil.mavlink.MAV_AUTOPILOT_PX4
            ...
            if self.active and now >= next_send:
                with self.lock:
                    ...
                    velocity = (
                        body_velocity_enu(values, heading)
                        if body
                        else np.zeros(3)
                        if self.target_position is not None
                        else np.array(list(values.values()))
                    )
                    ...
                    v = (
                        np.array([values["forward_mps"], values["right_mps"], -values["up_mps"]])
                        if body
                        else enu_to_ned(velocity)
                    )
                    frame = (
                    ...
                    yaw_rate_ned = -values["yaw_rate_rps"] if body else 0.0
                    self.connection.mav.set_position_target_local_ned_send(
                        int(self.clock() * 1000) & 0xFFFFFFFF,
                        1,
                        1,
                        frame,
                        mask,
                ...
                self.sent.append(
                    {
                        ...
                        "frame": frame,
                        "mask": mask,
                        "wire_velocity": v.tolist(),
                        "yaw_rate_ned": yaw_rate_ned,
                        "yaw_enu": heading,
    ...
    def command_long(self, command, *params):
        ...
        with self.lock:
            self.connection.mav.command_long_send(
                1, 1, command, 0, *(list(params) + [0] * (7 - len(params)))
            )
        self.wait(
            lambda: any(t >= started and c == command for t, c, _ in self.acks),
            4,
            "MAVLink ACK timeout",
        )
        ...
        if result != mavutil.mavlink.MAV_RESULT_ACCEPTED:
            raise RuntimeError(
                f"PX4 rejected command {command}: {result}; {list(self.messages)[-8:]}"
            )

    def connect(self):
        self.wait(
            lambda: self.position is not None and self.mode is not None,
            60,
            "PX4 estimator not ready",
        )
        ...
        time.sleep(6)
        self.wait(
            lambda: np.linalg.norm(self.velocity) < 0.1,
            15,
            "Estimator did not settle on the ground",
        )
    ...
    def takeoff(self, target):
        ...
        self.wait(
            lambda: (
                np.linalg.norm(self.position - target) < 0.15
                and np.linalg.norm(self.velocity) < 0.12
            ),
            30,
            "Takeoff did not settle",
        )
        time.sleep(4)
    ...
    def set_velocity(self, velocity):
        with self.lock:
            ...
            if getattr(self, "body_controls", False) and np.any(velocity):
                raise ValueError(
                    "Body mode requires named body controls; only a zero world command is allowed"
                )
            patch = (
                dict.fromkeys(self.controls.axes, 0.0)
                if getattr(self, "body_controls", False)
                else dict(zip(self.controls.axes, map(float, velocity)))
            )
            return self.controls.update(patch, now)

    def _expire_controls(self, now):
        event = self.controls.expire(now, now - self.position_at > 0.5)
        if event:
