...
def freeze(output, config):
    ...
    versions = {"px4": px4_commit,
                "gazebo": subprocess.check_output(["gz", "sim", "--versions"], text=True).strip()}
    (output/"manifest.json").write_text(json.dumps({"sha256": hashes, "versions": versions}, indent=2))
...
def contact_check(world, sensors):
    ...
    while world.status == "running" and time.monotonic() < deadline:
        ...
        world.update_controls("scripted_contact_check", {"forward_mps": -.5}, snapshot["revision"])
        time.sleep(.1)
    assert world.status == "collision" and world.contacts, "Native contact sensor missed deliberate wall impact"
    return {"status": "contact_verified", "world": world.metadata(), "trace": world.trace,
            "contacts": world.contacts, "sim_seconds": world.time, "wall_seconds": time.monotonic()-world.started_wall}
...
def main():
    ...
    parser.add_argument("--find-only", action="store_true", help="Finish after verifying the first item; omit return to launch")
    parser.add_argument("--key-file", type=Path, default=Path("/run/secrets/openrouter.env"))
    parser.add_argument("--calibrate", "--body-check", "--patch-check", dest="calibrate", action="store_true")
    parser.add_argument("--contact-check", action="store_true")
    parser.add_argument("--controller-check", action="store_true")
    parser.add_argument("--force-newtons", type=float, default=0.)
    args = parser.parse_args()
    config = Config(case=args.case, seed=args.seed, seconds=args.seconds, speed=args.speed,
                    budget=args.budget, policy="v4", rig="front", wind=0, odometry_drift=0,
                    servo="px4", disturbance_newtons=args.force_newtons, find_only=args.find_only)
    ...
    try:
        ...
        print(f"PX4 airborne, mode={session.flight.mode}, armed={session.flight.armed}; starting "+("calibration" if args.calibrate or args.contact_check else "Jev mission"), flush=True)
        ...
        if args.calibrate:
            ...
            gateway = JevGateway("openrouter", load_credential("openrouter", args.key_file), journal=args.out/"calls.jsonl")
            experiment = ControllerCheck if args.controller_check else FrontExperiment
            result = experiment(config, gateway, args.out/"result-frames", world=world, sensors=sensors).run()
        result.update(id=args.out.name, setpoints=session.flight.sent, commands=world.commands,
        ...
        print(json.dumps({k: result[k] for k in ("status", "sim_seconds", "wall_seconds", "realtime_factor")}), flush=True)
    except Exception:
        (args.out/"failure.txt").write_text(traceback.format_exc())
        if session.flight:
            (args.out/"failure-flight.json").write_text(json.dumps({"mode": session.flight.mode,
                "armed": session.flight.armed, "messages": list(session.flight.messages),
                "setpoints": session.flight.sent, "trace": world.trace if world else [],
                "world_status": world.status if world else None, "raw_contacts": list(session.transport.contacts),
