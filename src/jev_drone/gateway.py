...
def load_credential(provider, path=None):
    ...
    if not value and path and Path(path).is_file():
        match = re.search(r"(?m)^\s*(?:export\s+)?" + name + r"\s*=\s*[\"']?([^\s\"'#]+)", Path(path).read_text())
...
class JevGateway:
    def __init__(self, provider, key, timeout=3., transport=None, journal=None):
        self.provider = provider
        self.journal = journal
        self.endpoint, self.model, _ = PROVIDERS[provider]
        self.client = httpx.Client(headers={"Authorization": f"Bearer {key}"}, timeout=timeout,
    ...
    def evaluate(self, state, questions):
        ...
        result = {"provider": self.provider, "requested_model": self.model, "state": state, "questions": questions}
        try:
            response = self.client.post(self.endpoint, json={"model": self.model, "state": state, "questions": questions})
    ...
    def choose(self, state, instructions, criteria):
        result = self.evaluate(state, {"selection": {"type": "choice", "instructions": instructions, "criteria": criteria}})
