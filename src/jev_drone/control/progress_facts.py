...
def enrich(state, criteria):
    ...
    state["control_effects"] = {
        name: dict(effect) for name, effect in state["control_effects"].items()
    }
    criteria = dict(criteria)
    observation = state["sensors"]
    limits = state.get("control_limits", {})
    slow, full = limits.get("slow_clearance_m", 0.4), limits.get("full_clearance_m", 1.3)
    creep = limits.get("creep_clearance_m", slow)
    current = state["current_controls"]["body_controls"]
    state["holding_position_away_from_goal"] = (
        not any(current.values()) and not state["at_position"]
    )
    path_clearance = state.get("goal_path_clearance_m")
    state["goal_path_status"] = (
        "ARRIVED"
        if state["at_position"]
        else "UNKNOWN"
        if path_clearance is None
        else "BLOCKED"
        if path_clearance < creep
        else "CREEP_ONLY"
        if path_clearance < slow
        else "OBSERVED_CLEAR"
    )
    state["body_depth_status"] = {
        name: "UNKNOWN"
        if value is None
        else "OPEN"
        if value >= full
    ...
    for name, effect in state["control_effects"].items():
        if name.startswith("detour_"):
            prefix, distance = next(
                (p, d)
                for p, d in (("detour_short_", 0.6), ("detour_medium_", 0.9), ("detour_", 1.3))
            ...
            criteria[name] += (
                f" Observed {direction} depth: {depth}m; {effect['bypass_visibility']}. Requested step: {distance}m. "
            )
            criteria[name] += (
                "This selects a reachable temporary bypass destination around an obstruction; YOU fly toward it on following calls."
                if effect["bypass_viable"]
                else "Insufficient observed space for this detour. Do not select this bypass."
            )
            if state["at_position"]:
                criteria[name] += (
                    " Position goal is ALREADY REACHED: do not leave it on a detour. Finish the requested turn in place."
                )
        if name == "keep_controls" and not any(current.values()):
            effect["progress"] = "continue_hovering"
            criteria[name] += (
                " All current controls are ZERO. This stays in place and makes NO navigation progress."
            )
