...
class ControlLatch:
    ...
    def __init__(self, lease=.6, *, axes=None, state_key="velocity_mps"):
        self.lease = lease
        self.axes = tuple(axes) if axes is not None else self.axes
        self.state_key = state_key
        self.values = dict.fromkeys(self.axes, 0.)
        self.revision = 0
        self.expires = None
    ...
    def update(self, patch, now, expected_revision=None):
        if any(axis not in self.axes or isinstance(value, bool) or not isinstance(value, (int, float))
               or not math.isfinite(value) or abs(value) > 1 for axis, value in patch.items()):
        ...
        return {"applied": applied, "patch": dict(patch), "expected_revision": expected_revision,
                "before": before, "after": self.snapshot(), "wall": now}
