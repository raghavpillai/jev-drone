...
def required_clearance(requested, measured, frame_age, limits):
    base = (limits["full_clearance_m"] if abs(requested) > .25 else
            limits.get("creep_clearance_m", limits["slow_clearance_m"]) if abs(requested) <= .1 else
            limits["slow_clearance_m"])
    ...
    horizon = ((frame_age if frame_age is not None else .65)+2*limits["command_lease_seconds"]
               + limits.get("braking_settling_allowance_s", 0.))
    ...
    return max(base, speed*horizon+speed*speed/(2*.5)+.08)
