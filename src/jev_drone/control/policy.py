...
from jev_drone.control.doorway_phase import describe as describe_doorway_phase, INSTRUCTIONS as DOOR_PHASE_INSTRUCTIONS
from jev_drone.planning.room_transition import describe as describe_room_transition, INSTRUCTIONS as TRANSITION_INSTRUCTIONS
...
from jev_drone.control.progress_facts import enrich, INSTRUCTIONS as PROGRESS_INSTRUCTIONS
from jev_drone.control.decision_instructions import BRAKING, braking_facts
from jev_drone.planning.search_memory import SearchMemory, INSTRUCTIONS as SEARCH_INSTRUCTIONS
from jev_drone.planning.planner_modes import available_modes, INSTRUCTIONS as MODE_INSTRUCTIONS, eligible
from jev_drone.planning.room_intent import describe as describe_room_intent, INSTRUCTIONS as ROOM_INSTRUCTIONS
from jev_drone.planning.return_memory import ReturnMemory, INSTRUCTIONS as RETURN_INSTRUCTIONS
from jev_drone.planning.return_intent import describe as describe_return_intent
from jev_drone.planning.detour_route import candidate as detour_candidate, advance as advance_detour, observed_obstacles, INSTRUCTIONS as DETOUR_INSTRUCTIONS
from jev_drone.control.stopping import required_clearance
from jev_drone.planning.door_memory import DoorMemory, INSTRUCTIONS as DOOR_INSTRUCTIONS
from jev_drone.control.doorway_control import heading as doorway_heading, describe as describe_doorway, INSTRUCTIONS as DOORWAY_INSTRUCTIONS
from jev_drone.control.goal_phase import describe as describe_goal_phase, INSTRUCTIONS as GOAL_PHASE_INSTRUCTIONS
from jev_drone.control.axis_progress import describe as describe_axis_progress, INSTRUCTIONS as AXIS_PROGRESS_INSTRUCTIONS
...
def patches(speed):
    ...
    actions = {
        "keep_controls": {}, "brake": zero, "brake_and_replan": zero,
        "forward_creep": {"forward_mps": .1}, "backward_creep": {"forward_mps": -.1},
        "strafe_left_creep": {"right_mps": -.1}, "strafe_right_creep": {"right_mps": .1},
        "up_creep": {"up_mps": .1}, "down_creep": {"up_mps": -.1},
        "forward_slow": {"forward_mps": .25}, "forward_full": {"forward_mps": speed},
        "backward_slow": {"forward_mps": -.25}, "backward_full": {"forward_mps": -speed},
        "stop_forward": {"forward_mps": 0.},
        "strafe_left_slow": {"right_mps": -.25}, "strafe_right_slow": {"right_mps": .25},
        "strafe_left_full": {"right_mps": -speed}, "strafe_right_full": {"right_mps": speed},
        "stop_strafe": {"right_mps": 0.},
        "pan_left": {"yaw_rate_rps": .6}, "pan_right": {"yaw_rate_rps": -.6},
        "pan_left_slow": {"yaw_rate_rps": .2}, "pan_right_slow": {"yaw_rate_rps": -.2},
        "stop_pan": {"yaw_rate_rps": 0.},
        "up": {"up_mps": .25}, "down": {"up_mps": -.25}, "stop_vertical": {"up_mps": 0.},
    }
    ...
    for direction, forward in (("forward", .25), ("backward", -.25)):
        for pan, rate in (("left", .2), ("right", -.2)):
            actions[f"arc_{direction}_{pan}"] = {**zero, "forward_mps": forward, "yaw_rate_rps": rate}
    for strafe, right in (("left", -.25), ("right", .25)):
        for pan, rate in (("left", .2), ("right", -.2)):
            actions[f"arc_strafe_{strafe}_pan_{pan}"] = {**zero, "right_mps": right, "yaw_rate_rps": rate}
    return actions


def movement_reasons(values, observation, combined_clearance=None, limits=None):
    reasons = []
    limits = limits or {"slow_clearance_m": .4, "full_clearance_m": 1.3}
    gap = observation.get("nearest_observed_body_gap_m")
    if values["yaw_rate_rps"] and gap is not None and gap < .05:
        reasons.append("rotation_close_to_observed_obstacle")
    for channel, positive, negative in (("forward_mps", "forward", "backward"), ("right_mps", "right", "left"), ("up_mps", "up", "down")):
        ...
        threshold = required_clearance(value, measured, observation.get("frame_age_seconds"), limits)
        if clearance is None or clearance < threshold:
            reasons.append("insufficient_or_unknown_"+direction+"_depth")
        if abs(measured) > .15 and measured*value < 0:
            reasons.append("must_brake_before_reversing_"+positive)
    speed = math.hypot(values["forward_mps"], values.get("right_mps", 0.))
    if speed > .25 and (abs(values["yaw_rate_rps"]) > .1 or abs(observation["yaw_rate_rps"]) > .12):
        reasons.append("cruise_while_turning")
    translation = [values.get(c, 0.) for c in CHANNELS[:3]]
    combined_threshold = required_clearance(float(np.linalg.norm(translation)),
        float(np.linalg.norm(list(observation["body_velocity_mps"].values()))), observation.get("frame_age_seconds"), limits)
    if sum(v != 0 for v in translation) > 1 and (combined_clearance is None or combined_clearance < combined_threshold):
...
class FrontExperiment(Experiment):
    ...
    def observe(self):
        ...
        if observation["front_frame_available"] and observation["valid_depth_fraction"] > .4:
            view = {"time": round(self.world.time, 2), "position": observation["position_estimate"],
                    "yaw_degrees": observation["yaw_degrees"], "room": observation["room"],
                    "markers": sorted({d["marker"] for d in observation["detections"]})}
            last = self.front_views[-1] if self.front_views else None
            if (last is None or np.linalg.norm(np.array(view["position"])-last["position"]) > .7
                    or abs(wrap(math.radians(view["yaw_degrees"]-last["yaw_degrees"]))) > math.radians(20)):
    ...
    def request(self, role, state, instructions, criteria):
        if role == "control":
            ...
            instructions += PROGRESS_INSTRUCTIONS + BRAKING + DETOUR_INSTRUCTIONS + DOORWAY_INSTRUCTIONS + GOAL_PHASE_INSTRUCTIONS
        ...
        if role == "planner":
            ...
            for name, option in state["options"].items():
                if option.get("observed_endpoint_unsafe") and name in criteria:
                    criteria[name] = "DO NOT SELECT this endpoint: observed depth places it in/too close to furniture or a wall. Choose a different position. "+criteria[name]
            ...
            mode, mode_log = super().request("planner_mode", state, MODE_INSTRUCTIONS, available_modes(state))
            if mode is None:
            ...
            instructions += "\nYou selected task kind "+mode+". Select the concrete task now, using current evidence. All controls will be chosen separately by Jev."
    ...
    def options(self, observation):
        ...
        if room in self.layout.rooms and 'marker' in objective:
            ...
            if np.linalg.norm(np.array(center[:2])-observation['position_estimate'][:2]) > .25:
                options['look_into_room'] = {'look_position':center,
            ...
            for name,option in options.items():
                if name.startswith('view_'):
                    ...
                    option['purpose'] = 'Move to this viewpoint THEN look into the room. Passing near its position alone is not a survey.'
        ...
        for name, option in options.items():
            ...
            if "position" in option:
                option["already_at_position"] = self.at_goal(np.array(option["position"])-observation["position_estimate"])
        ...
        if "approach_marker" in options:
            ...
            if detection:
                ...
                toward_drone[2] = 0.
                directions = {name: np.array(DIRECTIONS[name], dtype=float) for name in ("east", "west", "north", "south")}
                if np.linalg.norm(toward_drone) > .1:
                ...
                for side, direction in directions.items():
                    goal = target+.95*direction
                    if np.any(goal < lo+.45) or np.any(goal > hi-.45):
                        continue
                    name = "approach_marker_"+side
                    options[name] = {"position": goal.tolist(), "look_position": target.tolist(),
                        "purpose": "Inspect from the "+side+" at observed item height; front camera cannot look straight down.",
        ...
        for option in options.values():
            if "position" in option:
                ...
                option["observed_endpoint_unsafe"] = gap is not None and gap < .08
                option["observed_path_clearance_m"] = (fused_clearance(self.sensors.current, pose, delta.copy())
                                                        if np.linalg.norm(delta) > .1 else None)
        ...
        fresh_front = (observation.get('front_frame_available')
            and observation.get('front_valid_depth_fraction', 0.) > .4
            and observation.get('frame_age_seconds') is not None
            and observation['frame_age_seconds'] <= .65)
        settled = observation['speed'] <= .10 and abs(observation['yaw_rate_rps']) <= .10
        if fresh_front and settled:
            for name, option in list(options.items()):
                ...
                if heading is not None and abs(wrap(heading-math.radians(observation['yaw_degrees']))) <= math.radians(12):
                    del options[name]
        return options
    ...
    def at_goal(self, delta, detour=False):
        horizontal = .25 if detour else self.config.goal_horizontal_tolerance_m
        return bool(np.linalg.norm(delta[:2]) <= horizontal and abs(delta[2]) <= self.config.goal_vertical_tolerance_m)
    ...
    def desired_heading(self, observation, goal, at_position):
        ...
        tolerance = .25 if self.detour else self.config.goal_horizontal_tolerance_m
    ...
    def crossed_doorway(self, observation):
        ...
        destination = next(room for room in self.layout.connections[door]
                           if self.layout.door_direction(door, room) == direction)
        return (observation["room"] == destination
                and direction*(observation["position_estimate"][axis]-center) >= .65)
    ...
    def task_finished(self, observation):
        ...
        if event:
            self.tasks[-1].update(outcome=event, ended=self.world.time,
                                  end_position=observation["position_estimate"])
            self.task = self.detour = None
            return True
        distance = self.distance(observation)
        delta = np.array(self.task.get("position", observation["position_estimate"]))-observation["position_estimate"]
        at_position = self.at_goal(delta) or self.crossed_doorway(observation)
        aim = self.requested_heading(observation)
        error = abs(wrap(aim-math.radians(observation["yaw_degrees"]))) if aim is not None else 0.
        goal = self.detour['position'] if self.detour else self.task.get('position', observation['position_estimate'])
        goal_reached = self.at_goal(np.array(goal)-observation['position_estimate'], detour=self.detour is not None) or self.crossed_doorway(observation)
        ...
        travel_error = abs(wrap(desired-math.radians(observation['yaw_degrees']))) if desired is not None else 0.
        if distance < self.best_distance-.08 or travel_error < self.best_heading_error-.15:
        ...
        if self.detour is not None:
            ...
            angle = abs(wrap(math.atan2(delta[1], delta[0])-math.radians(observation["yaw_degrees"])))
            if detour_distance < self.detour.get("best_distance", math.inf)-.08 or angle < self.detour.get("best_heading_error", math.inf)-.15:
                self.last_progress = self.world.time
                self.detour["best_distance"] = min(detour_distance, self.detour.get("best_distance", math.inf))
                self.detour["best_heading_error"] = min(angle, self.detour.get("best_heading_error", math.inf))
        outcome = None
        if at_position and error <= math.radians(12) and observation["speed"] <= .10 and abs(observation["yaw_rate_rps"]) <= .10:
            outcome = "arrived"
    ...
    def mission_event(self, observation):
        ...
        if (facts["target_visible"] and self.task["name"].startswith(("view_", "look_"))
    ...
    def finish_for_replan(self, observation):
        self.tasks[-1].update(outcome="replan_requested", ended=self.world.time,
                              end_position=observation["position_estimate"])
        self.visits[self.task["name"]] += 1
        self.task = self.detour = None
    ...
    def control(self, observation):
        ...
        if self.detour is not None:
            reached = self.at_goal(pose-self.detour["position"], detour=True) and observation["speed"] <= .1
            if reached or self.world.time-self.detour["time"] > 45:
                self.detour_history.append({**self.detour, "outcome": "arrived" if reached else "expired",
                                            "end_position": pose.tolist(), "ended": self.world.time})
        ...
        error = wrap(desired-heading) if desired is not None else 0.
        ...
        if self.detour is None:
            fwd = np.array([math.cos(heading), math.sin(heading), 0.])
            left = np.array([-math.sin(heading), math.cos(heading), 0.])
            for name, direction in {"left": left, "right": -left, "forward": fwd, "backward": -fwd,
                                    "up": np.array([0., 0., 1.]), "down": np.array([0., 0., -1.])}.items():
                for prefix, distance in (("detour_", 1.3), ("detour_medium_", .9), ("detour_short_", .6)):
                    choices[prefix+name] = dict.fromkeys(CHANNELS, 0.)
                    detours[prefix+name] = detour_candidate(pose, self.task.get("position", pose),
                        direction, distance, name in ("left", "right", "up", "down"), self.layout)
        choices = {name: changed_channels(current, patch)
                   for name, patch in choices.items()}
        effects = {name: {"patch": patch, "resulting_controls": {**current, **patch}}
                   for name, patch in choices.items()}
        fwd = np.array([math.cos(heading), math.sin(heading), 0.])
        right = np.array([math.sin(heading), -math.cos(heading), 0.])
        body_delta = dict(forward=round(float(delta@fwd), 2), right=round(float(delta@right), 2), up=round(float(delta[2]), 2))
        combined = {}
        limits = {name: getattr(self.config, name) for name in
                  ("command_lease_seconds", "creep_clearance_m", "slow_clearance_m", "full_clearance_m", "stopping_margin", "braking_settling_allowance_s")}
        for name, effect in effects.items():
            ...
            if sum(v != 0 for v in translation) > 1:
                ...
                if key not in combined:
                    vector = fwd*translation[0]+right*translation[1]+np.array([0., 0., translation[2]])
                    combined[key] = fused_clearance(self.sensors.current, pose, vector)
                effect["combined_clearance_m"] = combined[key]
            effect["movement_violations"] = movement_reasons(values, observation, effect.get("combined_clearance_m"), limits)
            effect["travel_effect"] = {axis: ("stopped" if values[channel] == 0 else
                "axis_reached_brake" if abs(body_delta[axis]) < (.1 if axis == "up" else .2) else
                "toward_goal" if values[channel]*body_delta[axis] > 0 else "away_from_goal")
            ...
            if abs(error) <= math.radians(8):
                effect["turn_effect"] = "hold_aligned_heading" if rate == 0 else "turn_away_from_alignment"
            elif rate == 0:
                effect["turn_effect"] = "heading_stays_unaligned"
            else:
                effect["turn_effect"] = "turn_toward_heading" if rate*error > 0 else "turn_away_from_heading"
            if name in detours:
                effect["temporary_goal"] = detours[name][0]
                effect["planned_route"] = detours[name]
                effect["route_obstacles"] = observed_obstacles(detours[name], pose, self.sensors.current)
        state = {"sensors": observation, "current_controls": snapshot, "task": self.task, "control_limits": limits,
                 "goal": goal, "goal_offset": np.round(delta, 2).tolist(), "at_position": bool(at_position),
                 "body_goal_offset_m": body_delta, "required_heading_degrees": self.task.get("heading_degrees"),
                 "goal_tolerance_m": {"horizontal": .25 if self.detour else self.config.goal_horizontal_tolerance_m,
                 ...
                 "active_detour": self.detour, "control_effects": effects,
                 "goal_path_clearance_m": fused_clearance(self.sensors.current, pose, delta) if not at_position else None,
                 "recent_detours": self.detour_history[-6:],
                 "recent_front_views": self.front_views[-3:],
                 "recent_actions": [{"choice": c.get("choice"), "accepted": c.get("accepted")}
        ...
        state["goal_endpoint_observed_occupied"] = endpoint_gap is not None and endpoint_gap < .08
        if self.mission_handoffs:
            progress = self.facts(observation)
            progress["target_in_inspection_range"] = bool(progress["target_visible"]
                and progress["target_distance"] <= 1.45
                and observation["room"] == progress["objective"].get("room"))
            progress["dock_in_range"] = bool("dock" in progress["objective"]
                and np.linalg.norm(pose-progress["objective"]["dock"]) <= .40)
            state["mission_progress"] = progress
        criteria = {}
        for name, effect in effects.items():
            meaning = ("Keep ALL currently active controls, including any ongoing pan." if name == "keep_controls" else
                       "Stop translation AND stop pan." if name == "brake" else "Apply "+json.dumps(effect["patch"])+"; retain omitted channels.")
            criteria[name] = meaning+" Turn effect: "+effect["turn_effect"]+". Movement violations: "+json.dumps(effect["movement_violations"])
            if name == "brake_and_replan":
                criteria[name] += " Explicitly hand this blocked/failed route back to the planner. Not for ordinary braking or turning; no mission objective is completed."
            if name in detours:
                criteria[name] += " Stop and select this bypass route, preserving the offset through its second leg: "+json.dumps(detours[name])
                criteria[name] += " Observed route obstacles: "+json.dumps(effect["route_obstacles"])+". Prefer a route without observed blocked legs; unseen space is not guaranteed clear."
        choice, log = self.request("control", state, PILOT, criteria)
        accepted = (choice is not None and self.world.status == "running" and self.world.time < self.config.seconds
        ...
        if accepted:
            ...
            if accepted:
                ...
                if choice in detours:
                    self.detour = {"position": detours[choice][0], "remaining_route": detours[choice][1:],
                        "planned_route": detours[choice], "leg": 0, "time": self.world.time,
                        "selected_by": "jev", "choice": choice, "decision_time": log["time"]}
                    self.last_progress = self.world.time
