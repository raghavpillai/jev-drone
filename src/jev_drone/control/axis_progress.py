...
def describe(state, criteria):
    ...
    if (state['at_position'] or state.get('goal_path_status') not in ('BLOCKED','UNKNOWN')
            or state.get('goal_endpoint_observed_occupied')
            or state.get('required_heading_degrees') is not None
            or math.sqrt(sum(value*value for value in offset.values()))>1.):
        return state,criteria
    limits=state['goal_tolerance_m']
    horizontal_unfinished=state['horizontal_distance']>limits['horizontal']
    unfinished={'forward':horizontal_unfinished,'right':horizontal_unfinished,
    ...
    for name,effect in state['control_effects'].items():
        ...
        if controls['yaw_rate_rps']:
            continue
        moving=[axis for axis,channel in [('forward','forward_mps'),('right','right_mps'),('up','up_mps')]
                if controls[channel]]
        if len(moving)==1 and unfinished[moving[0]] and effect['travel_effect'][moving[0]]=='toward_goal':
            candidates.append(name)
    if not candidates:
        return state,criteria
    state,criteria=deepcopy(state),dict(criteria)
    state['near_goal_axis_progress']={'permitted_controls':candidates,
        'scope':'Short body-axis corrections supported by current depth; not proof the complete diagonal is clear.'}
    for name in candidates:
        criteria[name]+=' Permitted short axis progress toward the unfinished position goal; consider before a large bypass.'
    return state,criteria
