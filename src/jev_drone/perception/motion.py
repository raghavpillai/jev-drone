...
def colored_components(frame, cloud):
    ...
    visible = (maximum > 50) & (delta > .25*maximum) & np.isfinite(frame.depth)
    ...
    bins = np.floor(hue+.5).astype(int) % 6
    detections = []
    for color in range(6):
        ...
        for component in range(1, count+1):
            ...
            low, high = np.quantile(measured, [.05, .95], axis=0)
            detections.append({"color_bin": color, "position": position,
                               "half_extent": np.clip((high-low)/2+.12, .20, 1.5),
                               "pixels": int(mask.sum())})
    return detections


class MotionTracker:
    def __init__(self, prediction="linear", body_radius=.30):
        self.tracks = {}
        self.next_id = 1
        self.time = -1.
    ...
    def update(self, time, detections):
        ...
        for detection in detections:
            if not any(detection["color_bin"] == d["color_bin"] and np.linalg.norm(detection["position"]-d["position"]) < .65 for d in merged):
                merged.append(detection)
        assigned = set()
        for detection in merged:
            candidates = [(np.linalg.norm(track["position"]-detection["position"]), identity)
                          for identity, track in self.tracks.items()
                          if identity not in assigned and track["color_bin"] == detection["color_bin"] and time-track["last_seen"] < .8]
            ...
            if best and best[0] < .7:
                ...
                self.next_id += 1
                track = {"history": deque(maxlen=6), "velocity": np.zeros(3), "last_moving": -100.}
            ...
            if len(track["history"]) >= 4:
                ...
                times -= times.mean()
                velocity = (times[:, None]*(positions-positions.mean(axis=0))).sum(axis=0)/max(1e-8, (times*times).sum())
                track["velocity"] = velocity if np.linalg.norm(velocity) < 2.5 else np.zeros(3)
                if np.linalg.norm(track["velocity"]) >= .18:
                    track["last_moving"] = time
        self.tracks = {identity: track for identity, track in self.tracks.items() if time-track["last_seen"] < 1.2}
    ...
    def observation(self, position, velocity, speed, now):
        ...
        for identity, track in self.tracks.items():
            ...
            def risk(drone_velocity):
                for t in np.linspace(0, 1.4, 15):
                    ...
                    if self.prediction == "uncertain":
                        ...
                        after_delay = max(0., t-.45)
                        displacement = velocity*min(t, .45) + drone_velocity*after_delay + (velocity-drone_velocity)*.28*(1-np.exp(-after_delay/.28))
            ...
            objects.append({"track": identity, "position_estimate": np.round(center, 2).tolist(),
                            "velocity_estimate": np.round(track["velocity"], 2).tolist(),
                            "observed_half_extent": np.round(track["half_extent"], 2).tolist(),
                            "age_seconds": round(age, 2)})
        return {"moving_visual_tracks": objects, "current_course_risk": bool(course_risk),
                "predicted_risk_by_direction": {k: bool(v) for k, v in risks.items()},
                "prediction_horizon_seconds": 1.4, "prediction_method": self.prediction}
