...
def basis(direction):
    ...
    up = np.array([0.0, 0.0, 1.0]) if abs(forward[2]) < 0.95 else np.array([0.0, 1.0, 0.0])
...
def points(frame):
    ...
    rays = (
        frame.forward
        + ((col - width / 2) / frame.focal)[..., None] * frame.right
    ...
    for name, mask in masks.items():
        ...
        for component in range(1, count + 1):
            ...
            detections.append(
                {
                    "marker": name,
                    "position": np.median(cloud[pixels], axis=0).tolist(),
...
def swept_clearance(frame, origin, direction, radius=0.30):
    ...
    if np.dot(frame.forward, direction) < 0.91:
        return None
    height, width = frame.depth.shape
    last = 0.0
    for distance in (0.2, 0.4, 0.6, 0.9, 1.3, 1.8, 2.5, 3.5):
        samples = np.array(
            [
                origin + direction * (distance + radius) + radius * (x * a + y * b)
                for x in (-1, 0, 1)
                for y in (-1, 0, 1)
            ]
        )
        ...
        for x, y, axial_depth in zip(u, v, z):
            ...
            if axial_depth <= 0.06 or ix < 1 or ix >= width - 1 or iy < 1 or iy >= height - 1:
                return last if last else None
            patch = frame.depth[iy - 1 : iy + 2, ix - 1 : ix + 2]
            finite = patch[np.isfinite(patch)]
            if len(finite) < 3:
                return last if last else None
            if np.quantile(finite, 0.2) < axial_depth + 0.035:
                return last
        last = distance
    return last
...
class Sensors:
    ...
    def capture(self):
        ...
        directions = (
            ...
            if config.rig == "surround"
            else [world.camera_direction.copy()]
        )
        frames = []
        for direction in directions:
            ...
            rgb = (
                np.asarray(image[2], dtype=np.uint8)
            ...
            depth = depth + self.rng.normal(0, config.depth_noise, depth.shape) * (
                1 + depth * depth * 0.1
            )
            depth = np.round(depth / 0.02) * 0.02
            depth[
                (buffer > 0.9999) | (depth < 0.1) | (self.rng.random(depth.shape) < config.dropout)
            ] = np.nan
    ...
    def deliver(self):
        while (
            self.queue
            and self.queue[0][0].time <= self.world.time - self.config.sensor_delay + 1e-8
        ):
    ...
    def observation(self):
        ...
        observation = {
            "position_estimate": np.round(pose, 2).tolist(),
            ...
            "camera_turning": bool(
                np.dot(self.world.camera_direction, self.world.camera_target) < 0.995
            ),
            "frame_age_seconds": round(age, 3) if age is not None else None,
            "valid_depth_fraction": round(valid, 3),
            "clearance_m": clearance,
            "detections": detections,
            "position_uncertainty_m": round(0.03 + self.config.odometry_drift * self.world.time, 2),
        }
        if self.tracker:
            observation.update(
                self.tracker.observation(
                    pose, self.world.velocity, self.config.speed, self.world.time
                )
            )
        return observation
    ...
    def save_frame(self, path):
        ...
        np.savez_compressed(
            path,
            rgb=np.stack([f.rgb for f in self.current]),
