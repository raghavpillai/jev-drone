...
def observed_collision_distance(frames, origin, direction, radius):
    ...
    for frame in frames:
        ...
        if hit.any():
            entry = along[hit] - np.sqrt(np.maximum(0.0, radius * radius - perpendicular2[hit]))
            limit = min(limit, max(0.0, float(entry.min())))
    return limit


def clearance(frames, origin, direction, radius=0.40):
    direction, a, b = basis(np.array(direction, dtype=float, copy=True))
    occupied_limit = observed_collision_distance(frames, origin, direction, radius + 0.05)
    distances = np.array([0.2, 0.4, 0.6, 0.9, 1.3, 1.8, 2.5, 3.5])
    ...
    for frame in frames:
        ...
        valid = (z > 0.06) & (u >= 1) & (u < width - 1) & (v >= 1) & (v < height - 1)
        ...
        interpolate = hi != lo
        depth[interpolate] += (ordered[rows[interpolate], hi[interpolate]] - depth[interpolate]) * (
            rank[interpolate] - lo[interpolate]
        )
        seen = count >= 3
        observed[indices] |= seen
        free[indices] |= seen & (depth >= z[indices] + 0.035)
