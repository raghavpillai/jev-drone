...
def audit(path):
    ...
    lease = config.get("command_lease_seconds", .6)
    def clearance_threshold(requested, measured, observation):
        base = (config.get("full_clearance_m", 1.3) if abs(requested) > .25 else
                config.get("creep_clearance_m", config.get("slow_clearance_m", .4)) if abs(requested) <= .1 else
                config.get("slow_clearance_m", .4))
        if config.get("stopping_margin"):
            speed = max(abs(requested), abs(measured))
            age = observation.get("frame_age_seconds")
            base = max(base, speed*((age if age is not None else .65)+2*lease
                                   +config.get("braking_settling_allowance_s", 0.))+speed**2+.08)
        return base
    ...
    for call, recorded in zip(calls, journal):
        if call.get("answers") != recorded.get("answers") or call.get("model") != recorded.get("model"):
    ...
    channels = ("forward_mps", "right_mps", "up_mps", "yaw_rate_rps") if modern else ("forward_mps", "up_mps", "yaw_rate_rps")
    for index, call in enumerate(calls):
        ...
        if call["role"] == "planner" and kind is not None:
            ...
            if (previous.get("role") != "planner_mode" or previous.get("choice") != kind
                    or previous.get("error")
                    or previous.get("state", {}).get("stage") != call["state"].get("stage")):
                problems.append("Concrete planner task lacks a matching preceding Jev task-kind decision")
    ...
    for call in accepted:
        ...
        if name == "brake" or name.startswith(("detour_", "brake_and_", "straight_")):
            patch = dict.fromkeys(channels, 0.)
        if name in ("stop_forward", "stop_strafe", "stop_vertical", "stop_pan"):
            patch = {dict(stop_forward="forward_mps", stop_strafe="right_mps", stop_vertical="up_mps", stop_pan="yaw_rate_rps")[name]: 0.}
        elif name in ("up", "down", "straight_up", "straight_down", "up_creep", "down_creep"):
            patch["up_mps"] = (.1 if name.endswith("_creep") else .25)*(1 if "up" in name else -1)
        elif name.startswith("strafe_") or name in ("straight_left_slow", "straight_right_slow"):
            patch["right_mps"] = (.1 if name.endswith("_creep") else .25 if name.endswith("_slow") else config["speed"])*(-1 if "left" in name else 1)
        elif "pan_left" in name or "pan_right" in name:
            patch["yaw_rate_rps"] = (.08 if name.endswith("_creep") else .2 if name.endswith("_slow") else .6)*(1 if "left" in name else -1)
        elif name.endswith(("forward_slow", "forward_full", "backward_slow", "backward_full", "forward_creep", "backward_creep")):
            patch["forward_mps"] = (.1 if name.endswith("_creep") else .25 if name.endswith("_slow") else config["speed"])*(-1 if "backward" in name else 1)
        if config.get("pilot_representation", "").startswith("intent-v"):
            if name != "keep_controls" and not name.startswith("stop_"):
                patch = {**dict.fromkeys(channels, 0.), **patch}
            if name.startswith("arc_"):
                parts = name.split("_")
                pan = parts[-1]
                if parts[1] == "strafe":
                    patch["right_mps"] = .25 if parts[2] == "right" else -.25
                else:
                    patch["forward_mps"] = .25 if parts[1] == "forward" else -.25
                patch["yaw_rate_rps"] = .2 if pan == "left" else -.2
        ...
        if (effect["patch"] != patch or receipt["patch"] != patch
                ...
                or receipt["expected_revision"] != receipt["before"]["revision"] or not receipt["applied"]):
        ...
        if expected_values["yaw_rate_rps"] and gap is not None and gap < .05:
        ...
        for channel, positive, negative in (("forward_mps", "forward", "backward"), ("right_mps", "right", "left"), ("up_mps", "up", "down")):
            value = expected_values.get(channel, 0.)
            if value:
                direction = positive if value > 0 else negative
                depth = observation["body_clearance_m"][direction]
                if depth is None or depth < clearance_threshold(value, observation["body_velocity_mps"][positive], observation):
                    reasons.append("insufficient_or_unknown_"+direction+"_depth")
                measured = observation["body_velocity_mps"][positive]
                if abs(measured) > .15 and value*measured < 0:
                    reasons.append("must_brake_before_reversing_"+positive)
        if math.hypot(expected_values["forward_mps"], expected_values.get("right_mps", 0.)) > .25 and (abs(expected_values["yaw_rate_rps"]) > .1 or abs(observation["yaw_rate_rps"]) > .12):
        ...
        if modern:
            ...
            if sum(v != 0 for v in translation) > 1 and (depth is None or depth < clearance_threshold(np.linalg.norm(translation), measured_speed, observation)):
    ...
    events = [*commands, *[e for e in result["command_expirations"] if e["wall"] >= commands[0]["wall"]]]
    ...
    for event in sorted(events, key=lambda e: (e["wall"], e["before"]["revision"])):
        ...
        values = ({**previous["body_controls"], **event["patch"]}
                  if "patch" in event else dict.fromkeys(channels, 0.))
        ...
        if "patch" in event and event["expected_revision"] is not None:
            matching = [c for c in accepted if c["choice"] == event["choice"]
                        and c["control_receipt"]["after"] == event["after"]
                        and 0 <= event["time"]-c["response_time"] <= .12]
            if not matching:
    ...
    for sample in wire:
        ...
        if not np.allclose(sample["wire_velocity"], [values["forward_mps"], values.get("right_mps", 0.), -values["up_mps"]]) or not math.isclose(sample["yaw_rate_ned"], -values["yaw_rate_rps"]):
            problems.append("Incorrect forward/up/yaw sign on MAVLink wire")
        expected = [values["forward_mps"]*math.cos(sample["yaw_enu"])+values.get("right_mps", 0.)*math.sin(sample["yaw_enu"]), values["forward_mps"]*math.sin(sample["yaw_enu"])-values.get("right_mps", 0.)*math.cos(sample["yaw_enu"]), values["up_mps"]]
        ...
        if any(values.values()) and (event is None or event["after"]["body_controls"] != values
                                    or sample["wall"]-event["wall"] > lease+.01 or sample["expired"]):
    ...
    return {"id": result["id"], "cohort": path.parent.parent.name,
            "directory": str(path.parent), "case": config["case"], "status": result["status"],
            "stages": result["stage"], "mission_stages": len(result.get("course", result["world"]["mission"])),
            ...
            "sim_seconds": result["sim_seconds"], "wall_seconds": result["wall_seconds"], "realtime_factor": result["realtime_factor"],
            "calls": len(calls), "accepted_controls": len(accepted), "accepted_controls_per_wall_second": len(accepted)/result["wall_seconds"],
            "pan_left_decisions": sum(v["yaw_rate_rps"] > 0 for v in values), "pan_right_decisions": sum(v["yaw_rate_rps"] < 0 for v in values),
            "strafe_left_decisions": sum(v.get("right_mps", 0.) < 0 for v in values),
            "strafe_right_decisions": sum(v.get("right_mps", 0.) > 0 for v in values),
            "forward_decisions": sum(v["forward_mps"] > 0 for v in values), "backward_decisions": sum(v["forward_mps"] < 0 for v in values),
            "simultaneous_pan_translation_decisions": sum(v["yaw_rate_rps"] != 0 and
                any(v.get(c, 0.) != 0 for c in (("forward_mps", "right_mps", "up_mps") if modern else ("forward_mps",))) for v in values),
            ...
            "recovered_telemetry_outages": sum("recovered_wall" in event for event in result.get("telemetry_outages", [])),
            "stale_responses": result["stale_responses"], "cost_usd": result["cost_usd"],
            "assigned_channels_per_accepted_control": sum(len(c["control_receipt"]["patch"]) for c in accepted)/len(accepted) if accepted else None,
