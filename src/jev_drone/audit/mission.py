...
def audit_reports(result):
    ...
    for report in reports:
        matching = [c for c in planners if c["choice"] == report["choice"]
                    and c["state"]["stage"] == report["stage"]
                    and 0 <= report["time"]-c["response_time"] <= .5]
        if not matching:
        ...
        supported = observation["speed"] <= .10 and abs(observation["yaw_rate_rps"]) <= .10
        if report["choice"] == "inspect":
            ...
            supported = (supported and target is not None and observation["front_frame_available"]
                         and observation["room"] == objective.get("room")
                         and np.linalg.norm(pose-target["position"]) <= 1.5
                         and truth is not None and np.linalg.norm(np.array(report["position"])-truth["position"]) <= 1.75)
        elif report["choice"] == "dock":
            supported = (supported and "dock" in objective
                         and np.linalg.norm(pose-objective["dock"]) <= .45
                         and np.linalg.norm(np.array(report["position"])-result["world"]["start"]) <= .55)
        else:
            supported = False
        if not supported:
