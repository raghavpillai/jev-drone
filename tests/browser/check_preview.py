...
def main(url):
    output = Path('report/px4-house');output.mkdir(parents=True,exist_ok=True)
    with sync_playwright() as p:
        ...
        options=page.locator('#run option').evaluate_all('(items)=>items.map(e=>({value:e.value,text:e.textContent}))')
        index=page.request.get(url+'/data/index.json').json()
        successful=next(o['path'] for o in index if o.get('success') and not o.get('safety_failure'))
        ...
        replan=next((d for d in recording['decisions'] if d['role']=='control' and d['choice']=='brake_and_replan'),None)
        if replan:
            page.locator('#scrub').evaluate('(e,time)=>{e.value=time;e.dispatchEvent(new Event("input",{bubbles:true}))}',replan['time']+.0001)
            expect(page.locator('#now')).to_have_text('Replanning the route')
        before=page.locator('#position').inner_text()
        page.locator('#scrub').evaluate('(e)=>{e.value=Number(e.max)*.45;e.dispatchEvent(new Event("input",{bubbles:true}))}')
        page.wait_for_function('(before)=>document.querySelector("#position").textContent!==before',arg=before)
        ...
        expect(page.get_by_role('button',name='Explore house',exact=True)).to_have_attribute('aria-pressed','true')
        page.locator('#cutaway').uncheck();page.locator('#cutaway').check()
        page.locator('#trail').uncheck();page.locator('#trail').check()
        ...
        page.get_by_role('button',name='Pause replay',exact=True).click()
        page.locator('#scrub').evaluate('(e)=>{e.value=e.max;e.dispatchEvent(new Event("input",{bubbles:true}))}')
        expect(page.locator('#stages')).to_have_text(f'{objective_count} / {objective_count} OBJECTIVES')
        expect(page.locator('#now')).to_have_text('Mission complete')
        page.locator('#scrub').evaluate('(e)=>{e.value=0;e.dispatchEvent(new Event("input",{bubbles:true}))}')
        ...
        expect(page.locator('#outcome')).to_have_class('failed')
        page.get_by_role('button',name='Pause replay',exact=True).click()
        page.locator('#scrub').evaluate('(e)=>{e.value=Number(e.max)*.5;e.dispatchEvent(new Event("input",{bubbles:true}))}')
        ...
        page.wait_for_selector('#loading.hidden');expect(page.locator('#outcome')).to_have_text('SCENE PREVIEW')
        ...
        assert all(r['error']<1e-6 and r['up']>.99 for r in orientation),orientation
        assert not errors,errors
        browser.close()
        print('PASS: successful/failed flight replay, time-correct objectives, scrubbing, play/restart, three cameras, cutaway/trail, scene switching, fullscreen, 1920×1080 and 3840×2160 canvases, mobile viewport, quiet default, details drawer, progress rewind, no browser errors.')
