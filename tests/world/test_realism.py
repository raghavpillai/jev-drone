...
def synthetic_frame(depth=4.):
    forward, right, up = basis((1, 0, 0))
    return Frame(0., np.zeros((72, 96, 3), np.uint8), np.full((72, 96), depth),
                 np.zeros(3), forward, right, up, 43.)
...
def test_camera_depth_clearance_accounts_for_body_width_and_unknown():
    ...
    frame.depth[32:40, 65:76] = .7
    assert swept_clearance(frame, np.zeros(3), (1, 0, 0)) < .4
    frame.depth[:] = np.nan
    assert swept_clearance(frame, np.zeros(3), (1, 0, 0)) is None
...
def test_marker_detection_uses_pixels_and_measured_depth():
    ...
    assert abs(detections[0]["position"][0]-2.) < .01
    frame.rgb[:] = 0
...
def test_renderer_occludes_target_and_exposes_no_segmentation():
    ...
    try:
        ...
        assert observation["frame_age_seconds"] >= .1-1e-6
...
def test_camera_turns_gradually_and_expired_command_brakes():
    ...
    try:
        ...
        world.advance(.25)
        assert .4 < world.camera_direction[1] < .6
        world.advance(.6)
        assert world.camera_direction[1] > .999
        world.command("cruise_east")
        world.advance(.5)
        assert world.velocity[0] > .35
        world.advance(1.5)
        assert np.linalg.norm(world.velocity) < .02
...
def test_jev_detour_sets_intent_without_automatic_flight():
    class Gateway:
        def choose(self, state, instructions, criteria):
            ...
            return {"choice": choice, "latency_seconds": .1, "state": state}
    ...
    try:
        ...
        assert np.linalg.norm(experiment.world.velocity) < .001
        experiment.control(experiment.observe())
        assert np.linalg.norm(experiment.world.velocity) < .001
    finally:
...
def test_observed_motion_predicts_crossing_and_static_tracks_do_not():
    ...
    for t in np.arange(0, 1., .125):
        tracker.update(t, [{"color_bin": 2, "position": np.array([1.2, .65-.5*t, 1.]),
                            "half_extent": np.array([.2, .2, .7]), "pixels": 30}])
    observation = tracker.observation(np.array([0., 0., 1.]), np.array([.6, 0, 0]), .6, .875)
    ...
    assert abs(observation["moving_visual_tracks"][0]["velocity_estimate"][1]+.5) < .02
    tracker = MotionTracker()
    for t in np.arange(0, 1., .125):
        tracker.update(t, [{"color_bin": 2, "position": np.array([1.2, .2, 1.]),
                            "half_extent": np.array([.2, .2, .7]), "pixels": 30}])
    assert not tracker.observation(np.zeros(3), np.zeros(3), .6, .875)["moving_visual_tracks"]


def test_integral_stabilizer_reduces_wind_drift_without_navigation():
    drift = {}
    for servo in ("p", "pi"):
        world = World(Config(wind=.6, servo=servo))
        try:
            world.command("brake")
            world.advance(12.)
