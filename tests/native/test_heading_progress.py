...
def experiment():
    ...
    agent.world = SimpleNamespace(time=0.0)
    agent.task = {
        "name": "view_test",
        "position": [4.0, 0.0, 1.3],
        "look_position": [0.0, 0.0, 1.3],
    }
    agent.tasks = [dict(agent.task)]
    agent.detour = None
    agent.visits = Counter()
    agent.task_started = agent.last_progress = 0.0
    agent.best_distance = 2.0
    agent.best_heading_error = math.inf
    agent.progress_heading_phase = None
    return agent


def observation(yaw, position=(2.0, 0.0, 1.3), rate=0.2):
    return dict(position_estimate=list(position), yaw_degrees=yaw, speed=0.0, yaw_rate_rps=rate)
...
def test_arrival_starts_a_separate_final_heading_progress_phase():
    ...
    for time, yaw in ((8, 0), (15, 45), (22, 90), (29, 135)):
        agent.world.time = time
        assert not agent.task_finished(observation(yaw, position=(4.0, 0.0, 1.3)))
    agent.world.time = 36
    assert agent.task_finished(observation(180, position=(4.0, 0.0, 1.3), rate=0.0))
...
def test_altitude_only_goal_has_no_tiny_offset_bearing_or_premature_final_heading():
    agent = experiment()
    agent.task["position"] = [2.0, 0.0, 2.2]
    obs = observation(90, position=(2.1, 0.05, 1.3))
    assert agent.desired_heading(obs, agent.task["position"], False) == (None, "altitude")
    assert not agent.task_finished(obs)
    agent.world.time = 8
    raised = observation(90, position=(2.1, 0.05, 2.2))
    ...
    agent.task["heading_degrees"] = 90.0
    assert agent.desired_heading(obs, agent.task["position"], False) == (
        math.pi / 2,
        "fixed_heading",
    )
