...
def test_vertical_bypass_keeps_altitude_until_past_the_obstacle():
    route = candidate([3., 2., 1.3], [5., 2., 1.3], [0., 0., 1.], 1.3, True)
    assert np.allclose(route, [[3., 2., 2.6], [5., 2., 2.6]])
    selected = {"position": route[0], "remaining_route": route[1:], "selected_by": "jev", "time": 1.}
    second = advance(selected, 4.)
    assert second["position"] == route[1] and second["selected_by"] == "jev"
    assert selected["position"] == route[0]
    assert advance(second, 8.) is None


def test_lateral_bypass_keeps_offset_and_does_not_choose_outside_building():
    assert candidate([3., 3., 1.3], [5., 3., 1.3], [0., 1., 0.], 1.3, True) == [[3., 4.3, 1.3], [5., 4.3, 1.3]]
    assert candidate([3., 6., 1.3], [5., 7., 1.3], [0., 1., 0.], 1.3, True) == [[3., 7.3, 1.3]]


def test_advancing_leg_resets_progress_without_nonfinite_request_values():
    selected = {"position": [2., 2., 2.6], "remaining_route": [[5., 2., 2.6]],
                "best_distance": .1, "best_heading_error": .02,
                "selected_by": "jev", "leg": 0}
    second = advance(selected, 4.)
    assert "best_distance" not in second and "best_heading_error" not in second
    assert selected["best_distance"] == .1
...
def test_forward_back_reposition_does_not_install_an_unasked_parallel_route():
    assert candidate([3., 3., 1.3], [5., 3., 1.3], [-1., 0., 0.], .6, False) == [[2.4, 3., 1.3]]
...
def test_clear_first_step_can_have_an_observed_blocked_second_leg():
    ...
    frames = [SimpleNamespace(clearance_cloud=np.array([[3.6, 3., 1.3]]))]
    side = observed_obstacles([[2.,3.3,1.3],[5.,3.3,1.3]], [2.,2.,1.3], frames)
    above = observed_obstacles([[2.,2.,2.6],[5.,2.,2.6]], [2.,2.,1.3], frames)
    assert not side[0]["observed_blocked"] and side[1]["observed_blocked"]
    assert not any(leg["observed_blocked"] for leg in above)
    assert observed_obstacles([[3.,2.,1.3]], [2.,2.,1.3], [])[0]["observed_obstacle_at_m"] is None


def test_upward_bypass_does_not_descend_toward_a_lower_parent_goal_mid_crossing():
    route = candidate([9.,3.,2.3], [7.,2.,1.3], [0.,0.,1.], .9, True)
    assert np.allclose(route, [[9.,3.,3.2], [7.,2.,3.2]])


def test_lateral_bypass_preserves_flight_altitude_until_past_the_obstruction():
    route = candidate([9.,3.,2.9], [7.,2.,1.3], [0.,1.,0.], .6, True)
    assert np.allclose(route, [[9.,3.6,2.9], [7.,2.6,2.9]])
...
def test_medium_upward_route_fits_between_observed_partition_and_ceiling():
    ...
    frames = [SimpleNamespace(clearance_cloud=np.array([[8.,2.,2.7],[9.,2.,4.]]))]
    pose, goal, direction = [9.,2.,2.3], [7.,2.,1.3], [0.,0.,1.]
    short = candidate(pose, goal, direction, .6, True)
    medium = candidate(pose, goal, direction, .9, True)
