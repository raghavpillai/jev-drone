...
def observation(**changes):
    return dict(
        room="workshop",
        position_estimate=[19.5, 11.0, 1.3],
        yaw_degrees=90.0,
        speed=0.0,
        yaw_rate_rps=0.0,
        front_frame_available=True,
        front_valid_depth_fraction=0.9,
...
def test_front_quality_and_freshness_cannot_be_replaced_by_auxiliary_depth():
    for change in (
        {"front_valid_depth_fraction": 0.0},
        {"front_frame_available": False},
        {"frame_age_seconds": 0.8},
    ):
        memory = SearchMemory(HOUSE)
        obs = {**observation(), **change}
        memory.observe(obs, 1.0)
...
def test_local_heading_coverage_does_not_follow_the_drone_to_other_positions_or_heights():
    ...
    assert memory.nearby_station("workshop", np.array(obs["position_estimate"]))["surveyed"] == set(
        range(0, 360, 45)
    )
    assert memory.nearby_station("workshop", np.array([21.0, 11.0, 1.3])) is None
    assert memory.nearby_station("workshop", np.array([19.5, 11.0, 2.2])) is None
...
def test_phase_and_look_novelty_are_facts_not_automatic_choices():
    ...
    memory.observe(obs, 1.0)
    state = {
        "objective": {"marker": "red", "room": "workshop"},
        "sensors": obs,
        "options": {
            "look_north": {"look": "north"},
            "look_south": {"look": "south"},
            "give_up": {},
        },
    }
    ...
    assert (
        memory.describe(outside, criteria)[0]["search_phase"]["situation"] == "GO_TO_REQUESTED_ROOM"
    )
    lead = {**state, "observed_markers": {"red": {"position": [22.0, 10.0, 1.6]}}}
    assert (
        memory.describe(lead, criteria)[0]["search_phase"]["situation"]
        == "APPROACH_OBSERVED_TARGET"
    )
    assert (
        memory.describe({**lead, "inspection_ready": True}, criteria)[0]["search_phase"][
            "situation"
        ]
        == "REPORT_READY"
    )
...
def test_current_house_view_tasks_face_inward_and_share_feasible_heights_with_memory():
    ...
    for name, option in choices.items():
        if name.startswith("view_"):
            ...
            assert option["look_position"][:2] == [21.0, 9.0]
...
def test_completed_stationary_look_cannot_form_an_immediate_replanning_loop():
    agent = experiment()
    obs = {**observation(), "position_estimate": [22.23, 8.96, 2.06], "yaw_degrees": 179.0}
    ...
    assert "look_into_room" in agent.options({**obs, "yaw_degrees": 90.0})
    assert "look_west" not in agent.options({**obs, "yaw_degrees": -179.0})
...
def test_outbound_room_intent_does_not_leak_into_return_or_next_item():
    state = {
        "stage": 1,
        "sensors": {"room": "library"},
        "recent_tasks": [
            {
                "name": "cross_library_workshop",
                "mission_stage": 0,
                "leads_to_room": "workshop",
                "outcome": "arrived",
            }
        ],
