...
def test_creep_has_a_smaller_stopping_envelope_but_cannot_use_unknown_depth():
    limits = dict(
        creep_clearance_m=0.4,
        slow_clearance_m=0.7,
        full_clearance_m=1.5,
        command_lease_seconds=0.9,
        stopping_margin=True,
    )
    observation = dict(
        body_clearance_m={"forward": 0.6},
        body_velocity_mps={"forward": 0.0},
        yaw_rate_rps=0.0,
        frame_age_seconds=0.15,
    )
    assert movement_reasons(patches(0.5)["forward_slow"], observation, limits=limits)
    assert not movement_reasons(patches(0.5)["forward_creep"], observation, limits=limits)
    observation["body_clearance_m"]["forward"] = None
    assert movement_reasons(patches(0.5)["forward_creep"], observation, limits=limits)
    assert required_clearance(0.1, 0.1, 0.65, limits) <= 0.4


def test_replan_brakes_every_axis_and_preserves_the_mission_stage():
    assert not any(patches(0.5)["brake_and_replan"].values())
    experiment = object.__new__(FrontExperiment)
    experiment.world = SimpleNamespace(time=7.0)
    experiment.stage = 1
    experiment.task = {"name": "cross_living_bedroom"}
    experiment.tasks = [{**experiment.task, "outcome": "active"}]
    experiment.detour = {"position": [5.0, 2.5, 1.3]}
