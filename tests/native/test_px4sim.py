...
from jev_drone.sim.flight import enu_to_ned, attitude_enu
...
def test_enu_ned_axis_signs_and_body_orientation():
    ...
    assert np.allclose(np.linalg.det(attitude_enu(.2, -.1, .4)), 1)
...
def test_real_simulator_is_not_advanced_twice_for_network_latency():
    ...
    class Gateway:
        def choose(self, *args):
            return {"latency_seconds": .4, "usage": {}, "choice": "brake"}
    for realtime in (False, True):
        experiment = object.__new__(Experiment)
        experiment.world = SimpleNamespace(time=1., realtime=realtime)
        advances = []
        ...
        assert advances == ([] if realtime else [.4])
...
def test_dynamic_scene_records_reproducible_motion_recipe(tmp_path):
    ...
    assert metadata["geometry"][actor["body"]]["center"][1] == pytest.approx(2+1.35*np.sin(actor["phase"]))
...
def test_action_facts_do_not_filter_or_override_jev():
    ...
    class Gateway:
        def choose(self, state, instructions, criteria):
            ...
            return {"latency_seconds": .1, "usage": {}, "choice": "cruise_north"}
    experiment = object.__new__(JevExperiment)
    experiment.config = SimpleNamespace(action_facts=True)
    experiment.world = SimpleNamespace(time=1., realtime=True)
    experiment.gateway = Gateway()
    experiment.cost, experiment.calls, experiment.error_streak = 0, [], 0
    choice, _ = experiment.request("control", {"sensors": {
        "velocity_estimate": [.5, 0, 0], "clearance_m": {"north": 3.5}}}, "", {"brake": "Stop", "cruise_north": "Move north"})
...
def test_motion_predictor_uses_the_x500_envelope():
    ...
    for radius in (.30, .40):
        ...
        for index in range(4):
            tracker.update(index*.1, [{"color_bin": 2, "position": np.array([1.4-index*.1, .58, 1.3]),
                                      "half_extent": np.array([.2, .2, .2]), "pixels": 50}])
        risks.append(tracker.observation(np.array([0., 0., 1.3]), np.zeros(3), .5, .3)["current_course_risk"])
...
def test_flight_expires_control_before_accepting_or_snapshotting_a_patch(monkeypatch):
    ...
    transport.clock = lambda: 20.
    transport.position_at = 10.7
    transport.controls.update({"x": .5, "z": .25}, 10.)
    pending = transport.controls.revision
    monkeypatch.setattr(flight.time, "monotonic", lambda: 10.7)
    receipt = transport.patch_velocity({"x": 0.}, pending)
    assert not receipt["applied"]
    assert transport.control_snapshot()["velocity_mps"] == {"x": 0., "y": 0., "z": 0.}
