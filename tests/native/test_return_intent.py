...
from jev_drone.planning.planner_modes import available_modes, MODES
from jev_drone.world.layout import HOUSE


def state(room='library'):
    return {'objective':{'dock':[1.4,3.,1.3]},'ACTIVE_OBJECTIVE':{'room':'entry'},
        'sensors':{'room':room},'known_room_connections':HOUSE.neighbors(),
        'options':{'retrace_8':{'position':[11.3,8.8,1.3],'room':'bedroom'},
                   'retrace_7':{'position':[12.8,9.,1.3],'room':'library'},
                   'approach_bedroom_library':{'position':[13.,9.,1.3]},
                   'cross_bedroom_library':{'position':[11.,9.,1.3]},'give_up':{}}}


def test_return_facts_expose_boundary_without_selecting_or_changing_waypoints():
    original=state();before=deepcopy(original);criteria=dict.fromkeys(original['options'],'Original.')
    ...
    assert waypoint['available_doorway_tasks']==['approach_bedroom_library','cross_bedroom_library']
...
def test_no_crossing_is_fabricated_for_nonadjacent_room_and_docking_phase_is_local():
    original=state('entry');updated,_=describe(original,dict.fromkeys(original['options'],''))
...
def test_return_has_no_marker_search_or_approach_but_retains_navigation_choices():
    assert set(available_modes(state()))=={'report','change_room','transit','retrace','give_up'}
