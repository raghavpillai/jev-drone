...
def state():
    effects = {
        name: {
            "movement_violations": ["insufficient_or_unknown_down_depth"]
            if name.startswith("down")
            else [],
            "resulting_controls": {
                "up_mps": -0.1 if name == "down_creep" else -0.25 if name == "down" else 0.0
            },
        }
        for name in ("down", "down_creep", "up", "up_creep", "brake", "brake_and_replan")
    }
    return {
        "body_goal_offset_m": {"up": -0.3},
...
def test_creep_remains_possible_when_faster_descent_is_forbidden():
    original = state()
    original["control_effects"]["down_creep"]["movement_violations"] = []
...
def test_no_altitude_correction_at_arrival_or_within_tolerance():
    for change in [
        {"at_position": True},
        {"body_goal_offset_m": {"up": -0.15}},
        {"body_goal_offset_m": {"up": 0}},
    ]:
...
def test_ascent_uses_up_controls_and_preserves_sign():
    original = state()
    original["body_goal_offset_m"]["up"] = 0.4
    ...
    assert updated["altitude_access"]["remaining_m"] == 0.4
