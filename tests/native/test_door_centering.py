...
@pytest.mark.parametrize(
    "heading,yaw,toward",
    [(90, 95, "left"), (-90, -95, "right"), (0, 5, "right"), (180, 175, "left")],
)
def test_centerline_reference_survives_far_waypoint_projection(heading, yaw, toward):
    original = state(heading, yaw, 0.206)
    before = deepcopy(original)
    updated, criteria = describe(
        original, dict.fromkeys(original["control_effects"], ""), local_reference=True
    )
    assert original == before
    assert abs(updated["doorway_centering"]["strafe_to_centerline_m"]) > 0.206
    ...
    for name, effect in updated["control_effects"].items():
        assert (
            effect["resulting_controls"] == original["control_effects"][name]["resulting_controls"]
        )
        assert (
            effect["movement_violations"]
            == original["control_effects"][name]["movement_violations"]
        )
        assert effect["far_waypoint_right_effect"] == "axis_reached_brake"


def test_vertical_only_alignment_stops_lateral_motion():
    original = state(90, 90, 0.05)
...
def test_other_phases_bypasses_and_completed_passages_are_unchanged():
    for override in (
        {"doorway_phase": {"phase": "FACE_OPENING"}},
        {"doorway_phase": {"phase": "CROSS_OPENING"}},
        {"active_detour": {"position": [1, 2, 3]}},
        {"at_position": True},
        {"door_passage_complete": True},
    ):
        original = {**state(90, 95, 0.206), **override}
