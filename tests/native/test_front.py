...
from jev_drone.control.policy import CHANNELS, patches, movement_reasons
...
def test_pan_and_translation_are_independent_persistent_channels():
    ...
    controls.update({"forward_mps": .25, "yaw_rate_rps": .6}, 0.)
    controls.update(patches(.5)["stop_forward"], .1, controls.revision)
    assert controls.snapshot()["body_controls"] == {"forward_mps": 0., "right_mps": 0., "up_mps": 0., "yaw_rate_rps": .6}
    controls.update(patches(.5)["backward_slow"], .2, controls.revision)
    controls.update(patches(.5)["stop_pan"], .3, controls.revision)
    assert controls.values == {"forward_mps": -.25, "right_mps": 0., "up_mps": 0., "yaw_rate_rps": 0.}
    controls.expire(.91)
    assert all(v == 0 for v in controls.values.values())


def test_keeping_a_turn_does_not_hide_a_fast_translation_violation():
    observation = {"body_clearance_m": dict(forward=3.5, backward=None, up=2., down=1.),
                   "body_velocity_mps": dict(forward=.4, up=0.), "yaw_rate_rps": .4}
    assert movement_reasons(dict(forward_mps=.5, up_mps=0., yaw_rate_rps=0.), observation) == ["cruise_while_turning"]
    reasons = movement_reasons(dict(forward_mps=-.25, up_mps=0., yaw_rate_rps=0.), observation)
...
def test_default_rig_covers_strafe_and_keeps_wide_front_view(tmp_path):
    from jev_drone.sim.scene import generate, HFOV
    ...
    cameras = ET.parse(tmp_path/"world.sdf").findall("world/model[@name='drone']/link/sensor[@type='rgbd_camera']")
...
def test_rear_pixels_cannot_reveal_targets_even_when_front_is_missing():
    ...
    front = Frame(.8, front_rgb, np.full((24, 32), 3.), np.array([.3, 0., 1.4]),
                  np.array([1., 0., 0.]), np.array([0., -1., 0.]), np.array([0., 0., 1.]), 32/(2*math.tan(1.2)), "east")
    rear = Frame(.8, rear_rgb, np.full((24, 32), 3.), np.array([-.3, 0., 1.4]),
                 np.array([-1., 0., 0.]), np.array([0., 1., 0.]), np.array([0., 0., 1.]), 32/(2*math.tan(1.2)), "west")
    sensor = object.__new__(CameraSensors)
    sensor.config = Config(front_controls=True)
    sensor.world = SimpleNamespace(time=1., pose_estimate=lambda: np.array([0., 0., 1.3]),
        camera_direction=np.array([1., 0., 0.]), camera_target=np.array([1., 0., 0.]),
        flight=SimpleNamespace(velocity=np.zeros(3), yaw_rate=0.))
    sensor.tracker = None
...
def test_body_velocity_tracks_heading_without_pitching_vertical_velocity():
    ...
    controls = dict(forward_mps=.5, up_mps=.25, yaw_rate_rps=.6)
    assert np.allclose(body_velocity_enu(controls, math.pi/2), [0., .5, .25])
    controls["forward_mps"] = -.5
    assert np.allclose(body_velocity_enu(controls, math.pi/2), [0., -.5, .25])


@pytest.mark.parametrize("yaw_rate", [-.6, .6])
def test_native_wire_uses_body_frame_and_correct_yaw_and_vertical_signs(yaw_rate):
    ...
    flight.controls.update(dict(forward_mps=.25, right_mps=.2, up_mps=.1, yaw_rate_rps=yaw_rate), time.monotonic())
    flight.active = flight.running = True
    flight.target_position = None
    flight.position_at = time.monotonic()
    flight.clock = lambda: 42.
    flight.rotation = np.array([[0., -1., 0.], [1., 0., 0.], [0., 0., 1.]])
    flight.expirations, flight.sent, packets = [], [], []
    def publish(*args):
        packets.append(args)
        flight.running = False
    flight.connection = SimpleNamespace(target_system=1, recv_match=lambda **kwargs: None,
        mav=SimpleNamespace(heartbeat_send=lambda *args: None, set_position_target_local_ned_send=publish))
    flight._loop()
    assert packets[0][3:5] == (8, 1479)
    assert np.allclose(packets[0][8:11], [.25, .2, -.1])
    assert packets[0][15] == -yaw_rate
    assert np.allclose(flight.sent[0]["velocity"], [.2, .25, .1])


def test_signed_strafe_replaces_opposite_direction_without_changing_pan():
    controls = ControlLatch(axes=CHANNELS, state_key="body_controls")
    controls.update({"right_mps": -.25, "yaw_rate_rps": .2}, 0.)
    controls.update({"right_mps": .25}, .1, controls.revision)
    assert controls.values["right_mps"] == .25
    assert controls.values["yaw_rate_rps"] == .2
    controls.update(patches(.5)["stop_strafe"], .2, controls.revision)
    assert controls.values["right_mps"] == 0.
    assert controls.values["yaw_rate_rps"] == .2


def test_diagonal_motion_needs_observed_diagonal_clearance():
    observation = {"body_clearance_m": dict(forward=3.5, right=3.5),
                   "body_velocity_mps": dict(forward=0., right=0.), "yaw_rate_rps": 0.}
    values = dict(forward_mps=.25, right_mps=.25, up_mps=0., yaw_rate_rps=0.)
    assert movement_reasons(values, observation) == ["insufficient_or_unknown_combined_depth"]
    assert movement_reasons(values, observation, .9) == ["insufficient_or_unknown_combined_depth"]
...
def test_fused_depth_supports_diagonals_and_keeps_blind_regions_unknown():
    ...
    from jev_drone.perception.sensors import Frame
    pose = np.array([0., 0., 1.3])
    def frame(angle, distance):
        forward = np.array([math.cos(angle), math.sin(angle), 0.])
        right = np.array([math.sin(angle), -math.cos(angle), 0.])
        return Frame(0., np.zeros((120, 160, 3)), np.full((120, 160), distance), pose,
                     forward, right, np.array([0., 0., 1.]), 160/(2*math.tan(1.2)))
    frames = [frame(0., 4.), frame(math.pi/2, 4.)]
    assert clearance(frames, pose, np.array([1., 1., 0.])) >= 1.3
    assert clearance([], pose, np.array([1., 1., 0.])) is None
    assert clearance([frame(0., .3)], pose, np.array([1., 0., 0.])) == 0.


def test_progress_facts_cannot_filter_or_change_a_movement():
    from jev_drone.control.progress_facts import enrich
    state = {"sensors": {"body_clearance_m": dict(forward=0., right=2.5, left=None)},
             "current_controls": {"body_controls": dict.fromkeys(CHANNELS, 0.)},
             "at_position": False,
             "control_effects": {"keep_controls": {"patch": {}},
                                 "detour_right": {"patch": dict.fromkeys(CHANNELS, 0.)},
                                 "detour_left": {"patch": dict.fromkeys(CHANNELS, 0.)},
                                 "forward_slow": {"patch": {"forward_mps": .25}}}}
    ...
    reached, described = enrich({**state, "at_position": True, "goal_path_clearance_m": 0.}, criteria)
...
def test_vertical_transition_releases_forward_and_repetition_is_empty():
    ...
    controls = ControlLatch(axes=CHANNELS, state_key="body_controls")
    controls.update({"forward_mps": .25}, 0.)
    desired = patches(.5)["down"]
    patch = changed_channels(controls.values, desired)
    assert patch == {"forward_mps": 0., "up_mps": -.25}
    controls.update(patch, .1, controls.revision)
    assert controls.values == desired
    patch = changed_channels(controls.values, desired)
    assert patch == {}
    controls.update(patch, .5, controls.revision)
    assert controls.expire(.9) is None
    assert controls.expire(1.11)["after"]["body_controls"] == dict.fromkeys(CHANNELS, 0.)


def test_panning_near_an_observed_obstacle_requires_translation_away_first():
    observation = {"nearest_observed_body_gap_m": -.01, "body_clearance_m": {"backward": 2.5},
                   "body_velocity_mps": {"forward": 0.}, "yaw_rate_rps": 0.}
    assert movement_reasons(patches(.5)["pan_left"], observation) == ["rotation_close_to_observed_obstacle"]
    assert movement_reasons(patches(.5)["backward_slow"], observation) == []
...
def test_altitude_goal_is_not_satisfied_by_the_old_horizontal_radius():
    ...
    assert not experiment.at_goal(np.array([0., 0., .30]))
    assert experiment.at_goal(np.array([.30, 0., .10]))


def test_short_bypass_can_fit_below_a_ceiling_when_full_step_cannot():
    from jev_drone.control.progress_facts import enrich
    state = {"sensors": {"body_clearance_m": {"up": .9}},
             "current_controls": {"body_controls": dict.fromkeys(CHANNELS, 0.)}, "at_position": False,
    ...
    assert enriched["control_effects"]["detour_medium_up"]["bypass_distance_m"] == .9
...
def test_target_approaches_allow_front_view_instead_of_hovering_above_item():
    ...
    experiment.sensors = SimpleNamespace(observed_gap_at=lambda point: .3, current=[])
    observation = {"position_estimate": [10.6, 2.8, 3.1], "room": "bedroom", "speed": 0.,
                   "yaw_rate_rps": 0., "detections": []}
    approaches = {k: v for k, v in experiment.options(observation).items() if k.startswith("approach_marker")}
    assert "approach_marker_south" in approaches and "approach_marker_west" in approaches
    assert all(a["position"][2] == 1.5 and a["look_position"] == [10.7, 2.8, 1.5] for a in approaches.values())
    assert all(6.45 <= a["position"][0] <= 11.55 and .45 <= a["position"][1] <= 3.55 for a in approaches.values())
...
def handoff_experiment():
    ...
    experiment = object.__new__(FrontExperiment)
    experiment.config = Config()
    experiment.world = SimpleNamespace(mission=[{"marker": "red", "room": "bedroom"}], time=10.)
    experiment.stage = 0
    experiment.target_notified_stage = None
    experiment.task = {"name": "view_bedroom_0_0_1.3", "position": [7.5, 1., 1.3]}
    experiment.tasks = [{**experiment.task, "outcome": "active"}]
    experiment.detour = {"position": [10., 3., 1.3]}
    observation = {"room": "bedroom", "position_estimate": [9.5, 2.8, 1.5],
        "speed": 0., "yaw_rate_rps": 0.,
...
def test_neutral_but_coasting_is_braking_not_a_stalled_hover():
    ...
    state = {"current_controls": {"body_controls": dict.fromkeys(CHANNELS, 0.)},
             "sensors": {"speed": .25, "yaw_rate_rps": 0.}, "at_position": False,
             "control_effects": {"keep_controls": {"patch": {}, "movement_violations": []},
                 "up": {"patch": {"up_mps": .25}, "movement_violations": ["must_brake_before_reversing_up"]}}}
...
def test_keeping_an_unsafe_nonzero_control_is_still_marked_unsafe():
    from jev_drone.control.decision_instructions import braking_facts
    state = {"current_controls": {"body_controls": {**dict.fromkeys(CHANNELS, 0.), "forward_mps": .25}},
             "sensors": {"speed": .25, "yaw_rate_rps": 0.}, "at_position": False,
             "control_effects": {"keep_controls": {"patch": {}, "movement_violations": ["must_brake_before_reversing_forward"]}}}
...
@pytest.mark.parametrize("goal,room,pose,complete", [
    ([7., 2., 1.3], "bedroom", [7.6, 2.5, 1.5], True),
    ([7., 2., 1.3], "bedroom", [6.3, 2., 1.3], False),
    ([5., 2., 1.3], "living", [5.3, 2., 1.3], True),
    ([5., 2., 1.3], "bedroom", [7., 2., 1.3], False),
])
def test_crossing_finishes_beyond_door_body_margin_not_at_exact_waypoint(goal, room, pose, complete):
    from jev_drone.control.policy import FrontExperiment
    experiment = object.__new__(FrontExperiment)
...
@pytest.mark.parametrize("objective", [{"dock": [1.4, 2., 1.3]}, {"marker": "red", "room": "bedroom"}])
def test_travel_planner_has_intermediate_goals_available_to_transit_mode(objective):
    ...
    observation = {"room": "living", "position_estimate": [3.9, 6.5, 1.3],
                   "speed": 0., "yaw_rate_rps": 0., "detections": []}
    options = experiment.options(observation)
    assert options["transit_living_0_1_1.3"]["position"] == [1.5, 7., 1.3]
...
def test_search_endpoint_on_observed_furniture_is_flagged_without_removing_it():
    ...
    experiment.sensors.observed_cloud = np.array([[10.4, 1., .9]])
    observation = {"room":"bedroom", "position_estimate":[7.5,1.,1.3],
                   "speed":0., "yaw_rate_rps":0., "detections":[]}
    options = experiment.options(observation)
