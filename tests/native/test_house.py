...
from jev_drone.world.layout import HOUSE, HOUSE_CASES, SMALL, Layout
...
@pytest.mark.parametrize(
    "case", [case for case in HOUSE_CASES if not case.startswith("house_search")]
)
def test_house_has_distant_reachable_objectives_and_clear_door_centers(case):
    ...
    for name, center in HOUSE.doors.items():
        gaps = [
            np.linalg.norm(np.maximum(np.abs(np.array(center) - b["center"]) - b["half"], 0))
            for b in scene.geometry
            if b["name"] != "unexpected_closed_door"
        ]
        assert min(gaps) >= 0.8 - 1e-6, name
...
def test_search_diagnostics_share_hidden_geometry_without_disclosing_target_presence():
    ...
    def geometry(scene):
        return [b for b in scene.geometry if not b["name"].startswith("red_")]

    # Body identifiers shift when a marker is removed; compare physical parts.
    def physical(scene):
        return [{k: v for k, v in b.items() if k != "body"} for b in geometry(scene)]
...
def test_room_names_do_not_encode_axis_direction_or_launch_room():
    layout = Layout(
        {"north_room": ((0, 6, 0), (6, 12, 4)), "south_room": ((0, 0, 0), (6, 6, 4))},
        {"portal_1": (3, 6, 1.3)},
        {"portal_1": ("north_room", "south_room")},
    )
    ...
    obs = dict(
        room="south_room", position_estimate=[3, 5, 1.3], detections=[], speed=0.0, yaw_rate_rps=0.0
    )
    ...
    assert heading(task, layout) == 90.0
    experiment.task = task
    assert experiment.crossed_doorway(
        {**obs, "room": "north_room", "position_estimate": [3, 6.7, 1.3]}
    )
    north = experiment.options({**obs, "room": "north_room", "position_estimate": [3, 7, 1.3]})
    assert north["cross_portal_1"]["position"] == [3, 5, 1.3]
    assert north["cross_portal_1"]["heading_degrees"] == -90.0
...
def test_observed_obstruction_uses_layout_specific_door_axis():
    ...
    assert heading(task, HOUSE) == 90.0
    assert heading({"name": "cross_living_bedroom", "position": [7, 2, 1.3]}, SMALL) == 0.0
    cloud = np.array(
        [[9 + a, 5.92, z] for a in np.linspace(-0.45, 0.45, 10) for z in np.linspace(0.85, 2, 10)]
    )
    memory = DoorMemory(HOUSE)
    memory.observe(cloud, 2.0)
    assert set(memory.obstructions) == {"living_bedroom"}
    state, _ = describe(
        {"task": task, "sensors": {"position_estimate": [9.2, 5, 1.3], "yaw_degrees": 90}},
        {},
        HOUSE,
    )
    assert state["doorway_alignment"]["centerline_offset_m"] == 0.2


def test_detour_candidates_and_search_memory_extend_beyond_old_house():
    route = candidate([20, 9, 1.3], [22, 9, 1.3], [0, 1, 0], 0.9, True, HOUSE)
...
def test_edge_candidates_offer_an_alternative_to_a_shelf_blocked_corner():
    ...
    def evaluator_gap(position):
        return min(
            np.linalg.norm(np.maximum(np.abs(np.array(position) - box["center"]) - box["half"], 0))
            - 0.4
            for box in scene.geometry
        )

    for height in (1.3, 2.2):
        assert evaluator_gap(views[f"view_workshop_1_1_{height}"]["position"]) < 0.0
        assert evaluator_gap(views[f"view_workshop_edge_east_{height}"]["position"]) > 0.08
...
def test_reverse_launch_sdf_and_known_layout_match_metadata(tmp_path):
    ...
    assert list(map(float, drone.findtext("pose").split()))[:3] == [22.5, 9.0, 0.24]
