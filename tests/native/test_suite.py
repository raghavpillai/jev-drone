...
def test_suite_finishes_all_trials_and_reports_native_startup_failure(tmp_path, monkeypatch, exit_codes, expected_exit):
    protocol = tmp_path/"protocol.json"
    protocol.write_text(json.dumps({"control_schema": "joystick-v1", "speed_mps": .5,
        "trials": [{"case": "clutter", "seed": 1, "seconds": 1},
                   {"case": "sequence", "seed": 2, "seconds": 1}]}))
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr(sys, "argv", ["suite", "--protocol", str(protocol), "--out", str(tmp_path/"results"), "--workers", "1"])
    calls = []
