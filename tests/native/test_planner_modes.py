...
def test_jev_mode_limits_next_task_family_and_records_both_decisions():
    ...
    def choose(state, instructions, criteria):
        ...
        return {"choice": choice, "latency_seconds": 0.2, "usage": {}, "state": deepcopy(state)}

    experiment = object.__new__(FrontExperiment)
    experiment.config = Config()
    experiment.world = SimpleNamespace(time=20.0, realtime=True)
    ...
    state = {
        "objective": {"dock": [1.4, 2.0, 1.3]},
        "sensors": {"room": "bedroom"},
        "ACTIVE_OBJECTIVE": {"return_to_launch": [1.4, 2.0, 1.3], "room": "living"},
        "options": {"cross_living_bedroom": {}, "transit_bedroom_0_0_2.9": {}, "give_up": {}},
    }
    choice, _ = experiment.request(
        "planner", state, "Plan.", {k: "Task." for k in state["options"]}
    )
