...
def test_active_route_legs_require_the_actual_accepted_model_selection():
    route = [[3., 2., 2.6], [5., 2., 2.6]]
    selected = {"role": "control", "time": 1., "choice": "detour_up", "accepted": True,
        "state": {"control_effects": {"detour_up": {"planned_route": route}}}}
    later = {"role": "control", "accepted": False,
        "state": {"active_detour": {"planned_route": route, "position": route[1],
            "remaining_route": [], "leg": 1, "decision_time": 1., "choice": "detour_up"}}}
    assert not audit_routes([selected, later])
    assert audit_routes([later])
    wrong = deepcopy(later)
    wrong["state"]["active_detour"]["position"][0] = 8.
...
def test_replanning_outcome_requires_an_accepted_model_handoff():
    task = {"name":"cross_living_bedroom", "time":1., "ended":5., "outcome":"replan_requested"}
    call = {"role":"control", "time":4.5, "response_time":5., "accepted":True,
        "choice":"brake_and_replan", "state":{"task":{"name":task["name"]},
            "control_effects":{"brake_and_replan":{}}}}
