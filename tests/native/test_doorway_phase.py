...
def test_diagonal_blockage_does_not_skip_unfinished_altitude_alignment():
    state = {'doorway_alignment':{'heading_error_degrees':0.,'centerline_offset_m':.03,'altitude_error_m':.8},
        'goal_tolerance_m':{'vertical':.15},'goal_path_status':'BLOCKED',
        'control_effects':{'down':{'movement_violations':[]},'forward_slow':{'movement_violations':['blocked']}}}
    before=deepcopy(state);criteria={'down':'descend','forward_slow':'move','brake':'stop','brake_and_replan':'replan'}
...
def test_turn_then_center_then_cross_follows_measured_errors():
    state={'doorway_alignment':{'heading_error_degrees':90.,'centerline_offset_m':.4,'altitude_error_m':0.},
           'goal_tolerance_m':{'vertical':.15}}
    assert describe(state,{})[0]['doorway_phase']['phase']=='FACE_OPENING'
    state['doorway_alignment']['heading_error_degrees']=0.
    assert describe(state,{})[0]['doorway_phase']['phase']=='CENTER_IN_OPENING'
    state['doorway_alignment']['centerline_offset_m']=.1
...
def test_phase_does_not_replace_a_selected_bypass_or_non_door_task():
    ...
    state={'doorway_alignment':{'heading_error_degrees':90.},'active_detour':{'position':[1,2,3]}}
