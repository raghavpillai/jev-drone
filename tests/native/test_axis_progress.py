...
def state():
    effects={}
    for name,forward,up,violations in [('forward_creep',.1,0.,[]),('down',0.,-.25,[]),
                                      ('backward_slow',-.25,0.,[]),('forward_full',.5,0.,['insufficient_depth'])]:
        effects[name]={'movement_violations':violations,
            'resulting_controls':{'forward_mps':forward,'right_mps':0.,'up_mps':up,'yaw_rate_rps':0.},
            'travel_effect':{'forward':'toward_goal' if forward>0 else 'away_from_goal','right':'stopped','up':'toward_goal'}}
    return {'at_position':False,'goal_path_status':'BLOCKED','horizontal_distance':.48,
        'body_goal_offset_m':{'forward':.48,'right':0.,'up':-.05},
        'goal_tolerance_m':{'horizontal':.35,'vertical':.15},'control_effects':effects}
...
def test_ready_occupied_fixed_heading_and_far_goals_are_not_promoted():
    for change in [{'at_position':True},{'goal_path_status':'OBSERVED_CLEAR'},
                   {'goal_endpoint_observed_occupied':True},{'required_heading_degrees':180.},
                   {'body_goal_offset_m':{'forward':1.1,'right':0.,'up':0.}}]:
        original={**state(),**change};updated,_=describe(original,dict.fromkeys(original['control_effects'],''))
        assert updated is original


def test_horizontal_arrival_defers_to_remaining_altitude_and_unsafe_controls_stay_unsafe():
    original=state();original['horizontal_distance']=.2;original['body_goal_offset_m']={'forward':.2,'right':0.,'up':-.4}
