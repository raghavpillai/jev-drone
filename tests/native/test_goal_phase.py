...
def state():
    return {"active_detour": {"position": [2., 2., 1.3]}, "required_heading_degrees": None,
            "requested_heading_degrees": 90., "goal_bearing_error_degrees": 40.,
            "horizontal_distance": .15, "goal_tolerance_m": {"horizontal": .25, "vertical": .15},
            "body_goal_offset_m": {"up": -1.49}, "at_position": False,
            "control_effects": {
                "down": {"turn_effect": "heading_stays_unaligned", "movement_violations": [],
                         "resulting_controls": {"yaw_rate_rps": 0., "up_mps": -.25}},
                "pan_left": {"turn_effect": "turn_toward_heading", "movement_violations": [],
                             "resulting_controls": {"yaw_rate_rps": .6, "up_mps": 0.}}}}
...
def test_unfinished_vertical_bypass_does_not_request_parent_inspection_heading():
    ...
    for name in criteria:
        assert updated["control_effects"][name]["resulting_controls"] == original["control_effects"][name]["resulting_controls"]


def test_door_heading_and_parent_task_survive_outside_the_bypass_phase():
    for override in ({"required_heading_degrees": 0.},
                     {"active_detour": None, "body_goal_offset_m": {"up": 0.}},
                     {"horizontal_distance": .8}):
...
def test_altitude_phase_never_erases_a_depth_violation():
    original = state()
    original["control_effects"]["down"]["movement_violations"] = ["insufficient_or_unknown_down_depth"]
    criteria = {n: "DO NOT SELECT: "+e["turn_effect"] for n,e in original["control_effects"].items()}
    updated, choices = describe(original, criteria)
    assert updated["control_effects"]["down"]["movement_violations"] == ["insufficient_or_unknown_down_depth"]
