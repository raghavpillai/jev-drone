...
def test_visibility_requires_front_projection_and_depth_beyond_sample():
    samples=np.array([[1.,0,0],[3.,0,0],[-1.,0,0],[1.,10,0]])
    visible=visible_samples(samples,np.full((12,16),2.),np.zeros(3),
        np.array([1.,0,0]),np.array([0.,-1,0]),np.array([0.,0,1]),8.)
    assert visible.tolist()==[True,False,False,False]


def test_missing_depth_and_foreground_patch_do_not_certify_space():
    args=(np.array([[1.,0,0]]),np.full((12,16),np.nan),np.zeros(3),
        np.array([1.,0,0]),np.array([0.,-1,0]),np.array([0.,0,1]),8.)
