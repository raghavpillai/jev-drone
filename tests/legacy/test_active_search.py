...
def args(memory='coverage'):
    return SimpleNamespace(seed=501,case='near',item='bookcase',room='living_room',memory=memory,seconds=120,budget=.15)


def test_occlusion_does_not_mark_hidden_region_as_seen():
    world=Scenario('test',0,(1.,5.,1.4),(-100.,)*3,(Box((2.,0.,0.),(2.2,10.,4.)),),(18.,10.,4.))
...
def test_all_looks_at_one_pose_do_not_exhaust_room_and_memory_survives_reentry():
    world=Scenario('test',0,(1.,5.,1.4),(-100.,)*3,(),(18.,10.,4.))
    attention=SearchAttention()
    for i,look in enumerate(('forward','back','left','right','up','down')):
        attention.record(visible_probes(world,world.start,look),world.start,look,i,'living_room')
    first=attention.summary('living_room')
    assert 0<first['fraction']<1
    attention.record([], (7.,5.,1.4),'forward',7,'bedroom')
    event=attention.record(visible_probes(world,world.start,'forward'),world.start,'forward',8,'living_room')
...
def test_arrival_without_camera_verification_does_not_complete_approach():
    experiment=ActiveSearch(args(),None)
    experiment.task={'kind':'approach','xyz':experiment.sim.position,'object':'object_x',
                     'started_at':0,'objects_before':set()}
...
def test_failed_attempts_and_coverage_are_explicit_and_ablatable():
    experiment=ActiveSearch(args(),None)
    experiment.observe()
    experiment.task={'choice':'look_forward','started_at':0,'seen_before':len(experiment.attention.seen)}
...
def test_potential_never_consults_hidden_obstacles():
    ...
    sample=next(p for p in PROBES if p in visible_probes(
        Scenario('test',0,(2,5,1.4),(-100.,)*3,(),(18,10,4)),(2,5,1.4),'forward'))
...
def test_correct_object_report_requires_clear_perceived_evidence():
    ...
    observation={'id':obj.id,'room':'living_room','appearance':'A tall wooden storage unit; shelving and contents are unclear from this view.',
                 'visible_surface_xyz':list(obj.box.lo),'approach_xyz':[obj.box.lo[0],7.5,1.4],
                 'first_seen_at':0,'last_seen_at':0}
    experiment.memory[obj.id]=observation
    experiment.visible=[observation]
    class Reporter:
        def choose(self,*unused):
            return {'choice':'report_found_'+obj.id,'latency_seconds':.1}
...
def test_anywhere_mission_accepts_matching_object_in_each_room():
    from experiments.legacy.search_world import visible_objects, update_memory
    for room,x in (('living_room',5.1),('bedroom',11.1),('study',17.1)):
        config=args(); config.item='plant'; config.room='anywhere'
        experiment=ActiveSearch(config,None)
        experiment.sim.position=(x,6.5,1.4)
        experiment.pilot.look='left'
        experiment.visible=visible_objects(experiment.scene.home,experiment.sim.position,view_direction=(0,1,0))
        update_memory(experiment.memory,experiment.visible,experiment.sim.position,0)
        target=next(o for o in experiment.scene.home.objects if o.kind=='plant' and o.id in experiment.memory)
        class Reporter:
            def choose(self,*unused):
                return {'choice':'report_found_'+target.id,'latency_seconds':.1}
        experiment.gateway=Reporter()
        experiment.plan()
