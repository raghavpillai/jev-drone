from experiments.legacy.continuation_report import stationary_task_streak, control_violations
...
def test_depth_audit_respects_speed_but_rejects_unknown_or_short_clearance():
    ...
    def call(reading,speed):
        return {'role':'control','accepted':True,'action':'up','speed':speed,
                'state':'header\n'+json.dumps(common)+'\nup: '+reading}
    valid=call('BLOCKED, clearance 0.6 m',.3)
    assert not control_violations([valid])
    invalid=[call('UNKNOWN',.3),call('BLOCKED, clearance 0.4 m',.3),call('BLOCKED, clearance 0.6 m',1)]
