...
from experiments.legacy.search_world import Home, Object, evaluate_report, make_home, room_at, search_waypoints, update_memory, visible_objects
...
def sensor_scene(obstacles=(), box=None):
    ...
    world = Scenario("sensor", 0, (1., 1.5, 1.), (-100., -100., -100.), (*obstacles, box), (10., 10., 4.))
...
def test_low_obstacle_can_be_seen_over_in_3d():
    home = sensor_scene((Box((2, 0, 0), (2.2, 4, .75)),))
    assert visible_objects(home, (1., 1.5, 1.5))
...
def test_unseen_target_relocation_does_not_change_model_input():
    ...
    for room in ["bedroom", "study"]:
        ...
        memory = {}
        update_memory(memory, seen, sim.position, 0.)
        state, criteria, _ = planner_state("Find a bookcase", sim, memory, seen, [], [], None, "starting", "none")
...
def test_memory_remembers_only_seen_objects_and_preserves_initial_approach():
    home = sensor_scene()
    memory = {}
    update_memory(memory, visible_objects(home, home.world.start), home.world.start, 1.)
    approach = memory[home.target_id]["approach_xyz"]
    update_memory(memory, [], (1., 9., 1.), 2.)
    assert memory[home.target_id]["last_seen_at"] == 1.
    position = (1.2, 1.5, 1.)
    update_memory(memory, visible_objects(home, position), position, 3.)
    assert memory[home.target_id]["first_seen_at"] == 1.
...
def test_report_requires_correct_identity_visibility_proximity_and_stopping():
    ...
    sim.velocity = (0, 0, 0)
    sim.position = (.3, 1.5, 1)
    assert evaluate_report("report_found_opaque_7", home, sim, seen) == "premature_report"
    assert evaluate_report("report_not_found", home, sim, seen) == "false_absence"
    assert evaluate_report("report_not_found", replace(home, target_id=None), sim, seen) == "correct_absence"
...
def test_recognition_that_exhausts_time_does_not_start_another_api_call(monkeypatch):
    ...
    def fake_ask(client, state, instructions, criteria):
        calls.append(state)
        return {"choice": "none", "latency_seconds": 2., "state": state}
    monkeypatch.setattr("experiments.legacy.search.ask", fake_ask)
    args = SimpleNamespace(seed=0, target_room="bedroom", absent=False, seconds=1.,
                           budget=.08, planner="jev", mission="Find a bookcase")
