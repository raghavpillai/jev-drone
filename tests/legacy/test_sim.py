...
def test_ranges_include_drone_size_and_do_not_see_through_walls():
    world = Scenario("test", 0, (1, 5, 1), (10, 5, 1),
                     (Box((3, 4, 0), (4, 6, 4)), Box((6, 4, 0), (7, 6, 4))))
    sim = Simulation(world)
    assert sim.clearance((1, 0, 0)) == pytest.approx(1.8)
    assert sim.clearance((-1, 0, 0)) == pytest.approx(.8)


def test_swept_collision_catches_thin_obstacles():
    sim = Simulation(Scenario("thin", 0, (1, 5, 1), (10, 5, 1),
                              (Box((1.6, 4, 0), (1.601, 6, 4)),)))
    sim.command("forward")
    sim.advance(.8)
...
def test_hover_brakes_over_time_instead_of_stopping_instantly():
    ...
    sim.command("brake")
    sim.advance(.5)
    assert sim.position[0]-x == pytest.approx(.25)
...
def test_network_wait_continues_old_command_and_lease_then_brakes():
    ...
    sim.command("forward")
    sim.advance(2.)
    assert sim.position[0] == pytest.approx(sim.world.start[0]+.8)
    assert norm(sim.velocity) == pytest.approx(0, abs=1e-9)
    assert sim.time == pytest.approx(2.)
...
def test_all_courses_are_physically_reachable_with_manual_waypoints(name, seed):
    ...
    if name == "open":
        ...
        exit_x = 10. if seed % 2 else 2.
        waypoints = [(exit_x, start[1], 1.), (exit_x, 8.9, 1.), (goal[0], 8.9, 1.), goal]
    else:
        waypoints = [(start[0], 8.9, 1.), (goal[0], 8.9, 1.), goal]
    for waypoint in waypoints:
        for _ in range(2000):
            offset = subtract(waypoint, sim.position)
            if norm(offset) < .35:
                sim.command("brake")
                if norm(sim.velocity) < .01:
                    break
            else:
                axis = max(range(3), key=lambda i: abs(offset[i]))
                sim.command(["forward", "left", "up"][axis] if offset[axis] > 0 else ["back", "right", "down"][axis])
            sim.advance(.02)
            assert sim.status != "collision"
        else:
