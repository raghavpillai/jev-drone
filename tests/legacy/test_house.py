...
from experiments.legacy.sim import Simulation, norm, subtract, ray_box
...
def test_object_destination_is_an_approach_point_outside_solid_geometry():
    for name in DESTINATIONS:
        ...
        for box in room.obstacles:
            assert ray_box(room.goal,(0,0,0),box.expanded(.2)) is None
...
def test_authored_portal_segment_is_clear_for_vehicle():
    ...
    for box in room.obstacles:
        hit=ray_box(start,subtract(end,start),box.expanded(.2))
