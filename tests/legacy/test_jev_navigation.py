...
def test_map_does_not_fill_missing_or_beyond_range():
    memory = ObservedMap()
    memory.record((1, 1, 1), {"forward": 2.0}, 0)
...
def test_map_retains_conflicting_measurements():
    memory = ObservedMap()
    memory.record((1, 1, 1), {"forward": 1.0}, 0)
    memory.record((1, 1, 1), {"forward": 3.0}, 1)
...
def test_gateway_uses_provider_specific_endpoint_and_validates_choice(provider):
    def handler(request):
        ...
        assert request.url.path == (
            "/v1/evaluate" if provider == "vercel" else "/api/alpha/decisions"
        )
        return httpx.Response(
            200,
            json={
                "model": body["model"],
                "answers": {"selection": {"choice": "yes", "probabilities": {"yes": 1, "no": 0}}},
                "usage": {},
...
def test_invalid_answer_fails_closed():
    with pytest.raises(ValueError):
        validate_answers(
            {"q": {"choice": "forward", "probabilities": {"forward": float("nan")}}},
...
def test_all_roles_use_same_jev_gateway():
    ...
    class Gateway:
        def choose(self, *args):
            calls.append(args)
            return {"choice": "yes", "latency_seconds": 0.01, "model": "typesafe/jev-test"}
...
def test_inspection_handoff_contains_only_remembered_target():
    ...
    nav.observed_target = {
        "appearance": "observed shelving",
        "last_seen_at": 2.0,
        "visible_surface_xyz": [2.0, 7.0, 1.4],
    }
    nav.decide(sim, sim.world.goal)
    assert (
        "target_observation" in gateway.state
        and "surface_offset_forward_left_up_m" in gateway.state
    )
...
def test_joint_scan_preserves_raw_jev_choice_and_only_executes_selected_action():
    ...
    result = nav.decode(
        {"answers": {"movement": {"choice": "look_up"}, "intent": {"choice": "direct"}}}
    )
    assert result["action"] == "brake" and result["look"] == "up"
    assert result["answers"]["movement"]["choice"] == "look_up"
    result = nav.decode(
        {"answers": {"movement": {"choice": "forward"}, "intent": {"choice": "direct"}}}
    )
