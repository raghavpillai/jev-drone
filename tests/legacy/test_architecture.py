...
def args(**overrides):
    values = dict(
        seed=101, case="far", architecture="task", planner="deepseek", seconds=210.0, budget=0.25
    )
...
def test_non_planner_roles_cannot_call_deepseek(monkeypatch):
    ...
    def fake(name):
        def request(*a, **kw):
            called.append(name)
            return {"choice": "a", "latency_seconds": 0.01}
...
def test_new_initial_planner_state_hides_unseen_object_geometry():
    ...
    state = planning_state(
        scene, sim, {}, visible_objects(scene.home, sim.position), set(), "none", {}
    )
...
def test_speed_choice_is_executed_without_changing_max_speed():
    ...
    sim.command("up", speed=0.4)
    sim.advance(0.5)
    assert norm(sim.velocity) == pytest.approx(0.4)
    assert sim.speed == 1.0
    sim.command("up")
    sim.advance(0.5)
    assert norm(sim.velocity) == pytest.approx(1.0)
