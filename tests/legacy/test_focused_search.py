...
class ScriptedGateway:
    ...
    def choose(self, state, instructions, criteria):
        ...
        return {"choice": choice, "latency_seconds": 0.1, "usage": {}}
...
def test_continue_and_task_choices_are_separate_jev_requests():
    ...
    assert abs(experiment.sim.time - 0.2) < 1e-6
...
def test_confirmation_readiness_uses_observed_range_and_velocity():
    ...
    experiment.sim.velocity = (0.2, 0, 0)
...
def test_approach_uses_latest_surface_with_confirmation_margin():
    ...
    experiment.memory["object_test"] = {
        "id": "object_test",
        "room": "living_room",
        "appearance": "An observed target.",
        "visible_surface_xyz": [4, 5.5, 1.4],
        "approach_xyz": [1, 1, 1],
    }
    point = experiment.options()["approach_object_test"]["xyz"]
    assert abs(norm(subtract(point, [4, 5.5, 1.4])) - 1.25) < 1e-6
    assert norm(subtract(point, [4, 5.5, 1.4])) + 0.65 < 2
...
def test_task_planner_and_stopping_decision_share_confirmation_contract():
    ...
    obj = {
        "id": "object_test",
        "room": "living_room",
        "appearance": "A clearly observed target.",
        "visible_surface_xyz": [4.1, 5.5, 1.4],
        "approach_xyz": [1, 1, 1],
        "first_seen_at": 0,
        "last_seen_at": 0,
    }
    experiment.memory[obj["id"]] = obj
    experiment.visible = [obj]
    options = experiment.options()
    planning = experiment.state(options)
    stopping = experiment.stopping_state(options)
...
def test_deadline_interrupt_requests_jev_status_before_budget_expires():
    ...
    experiment.task = {
        "choice": "go_search_living_room_high",
        "kind": "go",
        "xyz": [3, 6, 2.8],
        "look": "left",
        "started_at": 140,
...
def test_control_response_deadline_matches_actuator_lease():
    class SlowPilot:
        def evaluate(self, *unused):
            return {"answers": {"movement": {"choice": "forward"}}, "latency_seconds": 0.6}
    ...
    experiment.task = {
        ...
        "kind": "go",
        "xyz": [3, 6, 2.8],
        "description": "Travel to a viewpoint.",
    }
    experiment.best = 10
    experiment.progress_at = 0
    start = experiment.sim.position
    experiment.move()
    assert experiment.sim.command_lease == 0.5
...
def test_high_approach_depends_on_observed_surface_not_hidden_furniture():
    ...
    obj = {
        "id": "seen",
        "room": "bedroom",
        "appearance": "An observed object.",
        "visible_surface_xyz": [9, 7, 2],
        "approach_xyz": [0, 0, 0],
    }
    a.memory["seen"] = obj.copy()
    b.memory["seen"] = obj.copy()
    high = a.options()["approach_high_seen"]
    assert high == b.options()["approach_high_seen"]
    assert high["xyz"][2] == 3 and high["altitude_tolerance"] == 0.25


def test_slow_choice_controls_actual_actuator_speed_without_veto():
    class Gateway:
        def evaluate(self, *unused):
            return {"answers": {"movement": {"choice": "slow_forward"}}, "latency_seconds": 0.1}
    ...
    experiment.progress_at = 0
    experiment.move()
    assert experiment.sim.command_speed == 0.3
...
def test_transient_control_error_brakes_then_uses_a_fresh_observation():
    ...
    class Gateway:
        ...
        def evaluate(self, state, *unused):
            ...
            if len(self.states) == 1:
                return {"error": "ReadTimeout", "latency_seconds": 3.0}
            return {"answers": {"movement": {"choice": "slow_forward"}}, "latency_seconds": 0.1}
    ...
    experiment.best = 10
    experiment.progress_at = 0
    experiment.sim.velocity = (0.8, 0, 0)
    experiment.sim.command("forward")
    experiment.move()
    assert experiment.sim.status == "running" and experiment.sim.velocity == (0.0, 0.0, 0.0)
    assert not experiment.calls[-1]["accepted"]
    ...
    assert gateway.states[1]["velocity_metres_per_second_xyz"] == [0.0, 0.0, 0.0]
